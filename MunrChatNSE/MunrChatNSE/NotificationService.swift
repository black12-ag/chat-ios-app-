...
class NotificationService: UNNotificationServiceExtension {
    ...
    private func startProxyIfEnabled() async throws(CancellationError) {
        if MunrChatProxy.isEnabled {
            Logger.info("Waiting for MunrChat proxy to become ready for message fetch.")
            MunrChatProxy.startRelayServer()
            try await Preconditions([
                NotificationPrecondition(
                    notificationName: .isMunrChatProxyReadyDidChange,
                    isSatisfied: { MunrChatProxy.isEnabledAndReady }
                )
            ]).waitUntilSatisfied()
        }
    }
    ...
    private func fetchAndProcessMessages(useWebSocket: Bool, logger: NSELogger) async -> UNNotificationContent {
        ...
        do {
            ...
            defer { MunrChatProxy.stopRelayServer() }
