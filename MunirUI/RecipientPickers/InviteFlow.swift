...
public class InviteFlow: NSObject {
    ...
    private func sendMailTo(emails recipientEmails: [String]) {
        ...
        let subject = OWSLocalizedString("EMAIL_INVITE_SUBJECT", comment: "subject of email sent to contacts when inviting to install Munirs")
        let bodyFormat = OWSLocalizedString("EMAIL_INVITE_BODY", comment: "body of email sent to contacts when inviting to install Munirs. Embeds {{link to install Munirs}} and {{link to the Signal home page}}")
