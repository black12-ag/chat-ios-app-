    ...
    private func showUsernameNotFoundError(
        ...
        OWSActionSheets.showActionSheet(
            title: OWSLocalizedString(
                ...
                comment: "Title for an action sheet indicating that the given username is not associated with a registered Munirs account."
            ),
            message: String(
                format: OWSLocalizedString(
                    "USERNAME_LOOKUP_NOT_FOUND_MESSAGE_FORMAT",
                    comment: "A message indicating that the given username is not associated with a registered Munirs account. Embeds {{ a username }}."
                ),
                username
            ),
            dismissalDelegate: dismissalDelegate
