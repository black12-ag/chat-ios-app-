...
public class ShareViewController: UIViewController, ShareViewDelegate, SAEFailedViewDelegate {
    ...
    override open func loadView() {
        ...
        Task {
            ...
            let attachments: [MunrChatAttachment]
            do {
    ...
    private func buildAndValidateAttachments(
        ...
        guard attachments.filter({ !$0.isConvertibleToTextMessage }).count <= MunrChatAttachment.maxAttachmentsAllowed else {
    ...
    private func presentAttachmentError(_ error: any Error) {
        ...
        case ShareViewControllerError.tooManyAttachments:
            ...
            let alertTitle = String(format: format, OWSFormat.formatInt(MunrChatAttachment.maxAttachmentsAllowed))
        ...
        default:
            let alertTitle = OWSLocalizedString(
                ...
                comment: "Shown when trying to share content to a MunrChat user for the share extension. Followed by failure details."
            )
    ...
    nonisolated private func buildAttachments(for itemsAndProgresses: [(TypedItemProvider, Progress)]) async throws -> [MunrChatAttachment] {
        // FIXME: does not use a task group because MunrChatAttachment likes to load things into RAM and resize them; doing this in parallel can exhaust available RAM
        var result: [MunrChatAttachment] = []
