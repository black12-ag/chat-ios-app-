...
public class PreKeyStoreImpl {
    ...
    func generatePreKeyRecords(tx: DBWriteTransaction) -> [MunirServiceKit.PreKeyRecord] {
        ...
        for _ in 0..<batchSize {
            ...
            let record = MunirServiceKit.PreKeyRecord(id: preKeyId, keyPair: keyPair, createdAt: Date(), replacedAt: nil)
    ...
    func storePreKeyRecords(_ preKeyRecords: [MunirServiceKit.PreKeyRecord], tx: DBWriteTransaction) {
        for record in preKeyRecords {
            keyStore.setPreKeyRecord(record, key: keyValueStoreKey(int: Int(record.id)), transaction: tx)
        }
    }

    func loadPreKey(_ preKeyId: Int32, transaction: DBReadTransaction) -> MunirServiceKit.PreKeyRecord? {
        keyStore.preKeyRecord(key: keyValueStoreKey(int: Int(preKeyId)), transaction: transaction)
    }

    func storePreKey(_ preKeyId: Int32, preKeyRecord: MunirServiceKit.PreKeyRecord, transaction: DBWriteTransaction) {
    ...
    func setReplacedAtToNowIfNil(exceptFor exceptForPreKeyRecords: [MunirServiceKit.PreKeyRecord], tx: DBWriteTransaction) {
        ...
        keyStore.allKeys(transaction: tx).forEach { key in autoreleasepool {
            ...
            let record = keyStore.getObject(key, ofClass: MunirServiceKit.PreKeyRecord.self, transaction: tx)
...
extension PreKeyStoreImpl: LibSignalClient.PreKeyStore {
    ...
    public func storePreKey(_ record: LibSignalClient.PreKeyRecord, id: UInt32, context: StoreContext) throws {
        ...
        self.storePreKey(
            Int32(bitPattern: id),
            preKeyRecord: MunirServiceKit.PreKeyRecord(
...
extension KeyValueStore {
    fileprivate func preKeyRecord(key: String, transaction: DBReadTransaction) -> MunirServiceKit.PreKeyRecord? {
        return getObject(key, ofClass: MunirServiceKit.PreKeyRecord.self, transaction: transaction)
    }

    fileprivate func setPreKeyRecord(_ record: MunirServiceKit.PreKeyRecord, key: String, transaction: DBWriteTransaction) {
