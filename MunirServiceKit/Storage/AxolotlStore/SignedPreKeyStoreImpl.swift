...
public class SignedPreKeyStoreImpl: LibSignalClient.SignedPreKeyStore {
    ...
    public func loadSignedPreKey(_ signedPreKeyId: Int32, transaction: DBReadTransaction) -> MunirServiceKit.SignedPreKeyRecord? {
        keyStore.signedPreKeyRecord(key: keyValueStoreKey(int: Int(signedPreKeyId)), transaction: transaction)
    }

    func storeSignedPreKey(_ signedPreKeyId: Int32, signedPreKeyRecord: MunirServiceKit.SignedPreKeyRecord, tx: DBWriteTransaction) {
    ...
    func setReplacedAtToNowIfNil(exceptFor justUploadedSignedPreKey: MunirServiceKit.SignedPreKeyRecord, tx: DBWriteTransaction) {
        ...
        keyStore.allKeys(transaction: tx).forEach { key in autoreleasepool {
            ...
            let record = keyStore.getObject(key, ofClass: MunirServiceKit.SignedPreKeyRecord.self, transaction: tx)
    ...
    ) -> MunirServiceKit.SignedPreKeyRecord {
...
extension LibSignalClient.SignedPreKeyRecord {
    func asSSKRecord() throws -> MunirServiceKit.SignedPreKeyRecord {
        let keyPair = IdentityKeyPair(
            publicKey: try self.publicKey(),
            privateKey: try self.privateKey()
        )

        return MunirServiceKit.SignedPreKeyRecord(
...
extension MunirServiceKit.SignedPreKeyRecord {
...
extension KeyValueStore {
    fileprivate func signedPreKeyRecord(key: String, transaction: DBReadTransaction) -> MunirServiceKit.SignedPreKeyRecord? {
        return getObject(key, ofClass: MunirServiceKit.SignedPreKeyRecord.self, transaction: transaction)
    }

    fileprivate func setSignedPreKeyRecord(_ record: MunirServiceKit.SignedPreKeyRecord, key: String, transaction: DBWriteTransaction) {
