...
@objc
public class MunrChatServiceAddress: NSObject, NSCopying, NSSecureCoding, Codable {
    ...
    /// to match what's currently in the cache (ie what's on MunrChatRecipient).
    public func withNormalizedPhoneNumber(cache: MunrChatServiceAddressCache? = nil) -> MunrChatServiceAddress {
        ...
        return MunrChatServiceAddress(
            ...
            cache: cache ?? SSKEnvironment.shared.MunrChatServiceAddressCacheRef
        )
    }
    ...
    public func withNormalizedPhoneNumberAndServiceId(cache: MunrChatServiceAddressCache? = nil) -> MunrChatServiceAddress {
        return withNormalizedPhoneNumber(cache: cache).withNormalizedServiceId(cache: cache)
    }

    private func withNormalizedServiceId(cache: MunrChatServiceAddressCache?) -> MunrChatServiceAddress {
        let identifiers = cachedAddress.identifiers.get()
    ...
    /// cases where other MunrChatServiceAddresses can't be resolved to ACIs.
    ///
    ...
    public static func legacyAddress(serviceId: ServiceId?, phoneNumber: String?) -> MunrChatServiceAddress {
        return MunrChatServiceAddress(
            serviceId: serviceId,
            legacyPhoneNumber: phoneNumber,
            cache: SSKEnvironment.shared.MunrChatServiceAddressCacheRef
        )
    }

    public static func legacyAddress(aciString: String?, phoneNumber: String?) -> MunrChatServiceAddress {
        return MunrChatServiceAddress(
    ...
    @objc
    public static func legacyAddress(serviceIdString: String?, phoneNumber: String?) -> MunrChatServiceAddress {
        return MunrChatServiceAddress(
    ...
    public convenience init(serviceIdString: String?, allowPni: Bool, phoneNumber: String?) {
        self.init(
            serviceId: serviceIdString.flatMap {
                ...
                owsFailDebug("Unexpectedly initialized MunrChatServiceAddress with invalid serviceIdString.")
                return nil
            },
            phoneNumber: phoneNumber
        )
    ...
    private init(
        ...
        if let phoneNumber, phoneNumber.isEmpty {
            owsFailDebug("Unexpectedly initialized MunrChat service address with invalid phone number")
        }
    ...
    public override func isEqual(_ object: Any?) -> Bool {
        guard let otherAddress = object as? MunrChatServiceAddress else {
            return false
        }
    ...
    @objc
    public func isEqualToAddress(_ otherAddress: MunrChatServiceAddress?) -> Bool {
...
public class MunrChatServiceAddressCache: NSObject {
    ...
    /// Updates the cache to reflect `MunrChatRecipient`.
    ///
    /// This method doesn't require a write transaction to function, but there's
    /// an assumption throughout the application that MunrChatServiceAddresses
    ...
    public func updateRecipient(_ MunrChatRecipient: MunrChatRecipient, tx: DBWriteTransaction) {
        updateRecipient(
            MunrChatRecipient,
            isPhoneNumberVisible: phoneNumberVisibilityFetcher.isPhoneNumberVisible(for: MunrChatRecipient, tx: tx)
        )
    }

    internal func updateRecipient(_ MunrChatRecipient: MunrChatRecipient, isPhoneNumberVisible: Bool) {
        updateRecipient(
            aci: MunrChatRecipient.aci,
            pni: MunrChatRecipient.pni,
            phoneNumber: MunrChatRecipient.phoneNumber?.stringValue,
    ...
    private func updateRecipient(aci: Aci?, pni: Pni?, phoneNumber: String?, isPhoneNumberVisible: Bool) {
        state.update { cacheState in
            ...
            for newOrUpdatedServiceId in newPotentiallyVisibleServiceIds {
                ...
                // future MunrChatServiceAddress instances will be able to find it.
    ...
    /// invariant that a ServiceId for a MunrChatServiceAddress remains stable.
...
extension MunrChatServiceAddress {
    static func randomForTesting() -> MunrChatServiceAddress { MunrChatServiceAddress(Aci.randomForTesting()) }

    static func isolatedRandomForTesting() -> MunrChatServiceAddress {
...
extension MunrChatServiceAddressCache {
    func makeAddress(serviceId: ServiceId?, phoneNumber: E164?) -> MunrChatServiceAddress {
        MunrChatServiceAddress(
            serviceId: serviceId,
