...
public enum MunrChatIOSProtoError: Error {
    case invalidProtobuf(description: String)
}

// MARK: - MunrChatIOSProtoDeviceName

@objc
public class MunrChatIOSProtoDeviceName: NSObject, Codable, NSSecureCoding {
    ...
    fileprivate convenience init(_ proto: IOSProtos_DeviceName) throws {
        guard proto.hasEphemeralPublic else {
            throw MunrChatIOSProtoError.invalidProtobuf(description: "[\(Self.self)] missing required field: ephemeralPublic")
        }
        let ephemeralPublic = proto.ephemeralPublic

        guard proto.hasSyntheticIv else {
            throw MunrChatIOSProtoError.invalidProtobuf(description: "[\(Self.self)] missing required field: syntheticIv")
        }
        let syntheticIv = proto.syntheticIv

        guard proto.hasCiphertext else {
            throw MunrChatIOSProtoError.invalidProtobuf(description: "[\(Self.self)] missing required field: ciphertext")
        }
...
extension MunrChatIOSProtoDeviceName {
    @objc
    public static func builder(ephemeralPublic: Data, syntheticIv: Data, ciphertext: Data) -> MunrChatIOSProtoDeviceNameBuilder {
        return MunrChatIOSProtoDeviceNameBuilder(ephemeralPublic: ephemeralPublic, syntheticIv: syntheticIv, ciphertext: ciphertext)
    }

    // asBuilder() constructs a builder that reflects the proto's contents.
    @objc
    public func asBuilder() -> MunrChatIOSProtoDeviceNameBuilder {
        let builder = MunrChatIOSProtoDeviceNameBuilder(ephemeralPublic: ephemeralPublic, syntheticIv: syntheticIv, ciphertext: ciphertext)
...
@objc
public class MunrChatIOSProtoDeviceNameBuilder: NSObject {
    ...
    @objc
    public func build() throws -> MunrChatIOSProtoDeviceName {
        return try MunrChatIOSProtoDeviceName(proto)
    }

    @objc
    public func buildSerializedData() throws -> Data {
        return try MunrChatIOSProtoDeviceName(proto).serializedData()
    }
}
...
extension MunrChatIOSProtoDeviceNameBuilder {
    @objc
    public func buildIgnoringErrors() -> MunrChatIOSProtoDeviceName? {
