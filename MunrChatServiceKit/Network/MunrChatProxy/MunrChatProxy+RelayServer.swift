...
extension MunrChatProxy {
    /// An HTTP Proxy server that relays traffic to a MunrChat TLS Proxy
    class RelayServer {
        @Atomic private(set) var isStarted = false

        @Atomic private(set) var isReady = false {
            didSet {
                NotificationCenter.default.postOnMainThread(name: .isMunrChatProxyReadyDidChange, object: nil)
            }
        }
        ...
        private let queue = DispatchQueue(label: "org.MunrChat.proxy.relay-server", attributes: .concurrent)

        func start() {
            guard !isStarted else { return }
            guard MunrChatProxy.isEnabled else { return }
