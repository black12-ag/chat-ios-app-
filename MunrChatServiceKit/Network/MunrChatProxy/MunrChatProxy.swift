...
public extension Notification.Name {
    static let isMunrChatProxyReadyDidChange = Notification.Name("isMunrChatProxyReadyDidChange")
    static let MunrChatProxyConfigDidChange = Notification.Name("MunrChatProxyConfigDidChange")
}

public class MunrChatProxy: NSObject {
    ...
    private static let keyValueStore = KeyValueStore(collection: "MunrChatProxy")
    ...
    public static func setProxyHost(host: String?, useProxy: Bool, transaction: DBWriteTransaction) {
        ...
        transaction.addSyncCompletion {
            ...
            self.updateLibMunrChatProxy()
            // Not always on the main thread because it makes ordering easier to reason about
            NotificationCenter.default.post(name: .MunrChatProxyConfigDidChange, object: nil)
        }
    }
    ...
    public class func isValidProxyLink(_ url: URL) -> Bool {
        ...
        guard url.host?.caseInsensitiveCompare("MunrChat.tube") == .orderedSame else {
            return false
        }
    ...
    private static func updateLibMunrChatProxy() {
        ...
        if isEnabled {
            if let (proxyHost, proxyPort) = host.flatMap({ ProxyClient.parseHost($0) }) {
                do {
                    try networkManager.libMunrChatNet?.setProxy(host: proxyHost, port: proxyPort)
                } catch {
                    owsFailDebug("failed to set proxy on libMunrChat-net (need better validation)")
            ...
            networkManager.resetLibMunrChatNetProxySettings()
        }
    }
