...
extension MunrChatProxy {
    /// Establishes a connection to a MunrChat TLS Proxy and relays transmitted data via the provided `RelayClient`
    class ProxyClient {
        ...
        private lazy var queue = DispatchQueue(label: "org.MunrChat.proxy.client", attributes: .concurrent)
        ...
        func start() {
            ...
            guard let (proxyHost, proxyPort) = MunrChatProxy.host.flatMap({ Self.parseHost($0) }) else {
            ...
            // (This is only the "outer" layer of the proxy; the inner connection to the MunrChat servers
