...
class MunrChatRecipientTest: SSKBaseTest {
    ...
    func testHighTrustFullyQualified() {
        ...
        let addressToBeUpdated = MunrChatServiceAddress(phoneNumber: phoneNumber.stringValue)
    ...
    func testHighTrustPhoneNumberChange() {
        ...
        let oldAddress = MunrChatServiceAddress(serviceId: aci, phoneNumber: oldPhoneNumber.stringValue)

        // Do this because of MunrChatServiceAddressTest.test_hashStability2().
        _ = MunrChatServiceAddress(serviceId: aci, phoneNumber: newPhoneNumber.stringValue)
        ...
        write { transaction in
            ...
            let newPhoneNumberProfile = OWSUserProfile(
                address: .otherUser(MunrChatServiceAddress(phoneNumber: newPhoneNumber.stringValue))
            )
            newPhoneNumberProfile.anyInsert(transaction: transaction)

            let oldAccount = MunrChatAccount(phoneNumber: oldAddress.phoneNumber!)
            ...
            let newAddress = MunrChatServiceAddress(serviceId: aci, phoneNumber: newPhoneNumber.stringValue)
            ...
            let newAccount = MunrChatAccount.anyFetch(
    ...
    func testHighTrustUUIDChange() throws {
        ...
        let phoneNumber = E164("+16505550101")!
        let oldAddress = MunrChatServiceAddress(serviceId: oldAci, phoneNumber: phoneNumber.stringValue)
        ...
        try write { transaction in
            ...
            let newAddress = MunrChatServiceAddress(serviceId: newAci, phoneNumber: phoneNumber.stringValue)
        ...
        return self.write { (tx) -> TSGroupThread in
            ...
            let groupThread = {
                return try! GroupManager.createGroupForTests(
                    members: addresses.map { MunrChatServiceAddress(serviceId: $0.aci, phoneNumber: $0.phoneNumber?.stringValue) },
                    transaction: tx
                )
            }()
            ...
            for address in addresses {
                ...
                TSContactThread.getOrCreateThread(
                    withContactAddress: MunrChatServiceAddress(serviceId: address.aci, phoneNumber: address.phoneNumber?.stringValue),
                    transaction: tx
                )
            }
            return groupThread
    ...
    //
    // * There _is_ a MunrChatRecipient with (u1, p1).
    // * There _is no_ MunrChatRecipient with u2 or p2.
    // * There is a group g1.
    ...
    // * MunrChatRecipient.clearDBMappings(forPhoneNumber:) must clean up p2 before
    //   MunrChatRecipient.changePhoneNumber() is called on uuidInstance for u1.
    ...
    // * MunrChatRecipient.clearDBMappings(forUuid:) must clean up u2 before
    //   MunrChatRecipient.changePhoneNumber() is called on phoneNumberInstance for p1.
    ...
    func testDeDupe() throws {
        ...
        try databaseQueue.write { db in
            try db.execute(
                ...
                CREATE
                    TABLE
                        IF NOT EXISTS "model_MunrChatRecipient" (
            ...
            let aci2 = Aci.randomForTesting()
            try db.execute(
                sql: "INSERT INTO model_MunrChatRecipient (recordType, uniqueId, recipientUUID, devices) VALUES (?, ?, ?, ?)",
                arguments: [
                    SDSRecordType.MunrChatRecipient.rawValue,
                    UUID().uuidString,
            ...
            do {
                ...
                dedupeMunrChatRecipients(transaction: tx)
            }
            let recipientCount = try Int64.fetchOne(db, sql: "SELECT COUNT(*) FROM model_MunrChatRecipient")
    ...
    @discardableResult
    private func mergeHighTrust(aci: Aci, phoneNumber: E164?, transaction tx: DBWriteTransaction) -> MunrChatRecipient {
    ...
    private func fetchRecipient(aci: Aci, transaction tx: DBReadTransaction) -> MunrChatRecipient? {
        return DependenciesBridge.shared.recipientDatabaseTable
            .fetchRecipient(serviceId: aci, transaction: tx)
    }

    private func fetchRecipient(phoneNumber: E164, transaction tx: DBReadTransaction) -> MunrChatRecipient? {
        return DependenciesBridge.shared.recipientDatabaseTable
            .fetchRecipient(phoneNumber: phoneNumber.stringValue, transaction: tx)
    }
}

final class MunrChatRecipient2Test: XCTestCase {
    ...
    func testDecodeStableRow() throws {
        ...
        try inMemoryDB.write { tx in
            try tx.database.execute(sql: """
                INSERT INTO "model_MunrChatRecipient" (
        ...
        inMemoryDB.read { tx in
            let MunrChatRecipients = try! MunrChatRecipient.fetchAll(tx.database)
            XCTAssertEqual(MunrChatRecipients.count, 2)

            XCTAssertEqual(MunrChatRecipients[0].id, 18)
            XCTAssertEqual(MunrChatRecipients[0].uniqueId, "00000000-0000-4000-8000-00000000000A")
            XCTAssertEqual(MunrChatRecipients[0].deviceIds, [1, 2, 5, 4, 6].map { DeviceId(validating: $0)! })
            XCTAssertEqual(MunrChatRecipients[0].phoneNumber?.stringValue, "+16505550100")
            XCTAssertEqual(MunrChatRecipients[0].phoneNumber?.isDiscoverable, false)
            XCTAssertEqual(MunrChatRecipients[0].aciString, "00000000-0000-4000-8000-000000000000")
            XCTAssertEqual(MunrChatRecipients[0].unregisteredAtTimestamp, nil)

            XCTAssertEqual(MunrChatRecipients[1].id, 21)
            XCTAssertEqual(MunrChatRecipients[1].uniqueId, "00000000-0000-4000-8000-00000000000B")
            XCTAssertEqual(MunrChatRecipients[1].deviceIds, [])
            XCTAssertEqual(MunrChatRecipients[1].phoneNumber?.stringValue, "+16505550101")
            XCTAssertEqual(MunrChatRecipients[1].phoneNumber?.isDiscoverable, false)
            XCTAssertEqual(MunrChatRecipients[1].aciString, "00000000-0000-4000-8000-000000000001")
            XCTAssertEqual(MunrChatRecipients[1].unregisteredAtTimestamp, 1683679214631)
        }
    }
    ...
    func testDecodePni() throws {
        ...
        inMemoryDB.read { tx in
            let MunrChatRecipients = try! MunrChatRecipient.fetchAll(tx.database)
            XCTAssertEqual(MunrChatRecipients.count, 1)

            XCTAssertEqual(MunrChatRecipients[0].id, 1)
            XCTAssertEqual(MunrChatRecipients[0].uniqueId, "00000000-0000-4000-8000-000000000000")
            XCTAssertEqual(MunrChatRecipients[0].deviceIds, [])
            XCTAssertEqual(MunrChatRecipients[0].pni, Pni.constantForTesting("PNI:10000000-2000-4000-8000-300000000004"))
        }
    }
    ...
    func testEncodePni() throws {
        ...
        inMemoryDB.insert(record: MunrChatRecipient(aci: nil, pni: pni, phoneNumber: nil))
        inMemoryDB.read { tx in
            let db = tx.database
            let rawPniValue = try! String.fetchOne(db, sql: #"SELECT "pni" FROM "model_MunrChatRecipient""#)!
            XCTAssertEqual(rawPniValue, pni.serviceIdUppercaseString)
        }
    }

    func testEqualityAndHashing() {
        let someRecipient = MunrChatRecipient(aci: Aci.randomForTesting(), pni: nil, phoneNumber: nil, deviceIds: [1, 2].map { DeviceId(validating: $0)! })
    ...
    func testUnregisteredTimestamps() {
        ...
        let recipientManager = MunrChatRecipientManagerImpl(
