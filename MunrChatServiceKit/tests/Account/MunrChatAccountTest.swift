...
class MunrChatAccountTest: XCTestCase {
    ...
    func testInMemoryDatabaseRoundTrip() {
        for (idx, (constant, _)) in MunrChatAccount.constants.enumerated() {
            inMemoryDB.insert(record: constant)

            guard let deserialized = inMemoryDB.fetchExactlyOne(modelType: MunrChatAccount.self) else {
    ...
    func testHardcodedJsonDataDecodes() {
        ...
        case .printStrings:
            MunrChatAccount.printHardcodedJsonDataForConstants()
        case .runTest:
            for (idx, (constant, jsonData)) in MunrChatAccount.constants.enumerated() {
                do {
                    let decoded = try JSONDecoder().decode(MunrChatAccount.self, from: jsonData)
    ...
    /// This test ensures that json from the era before `MunrChatAccount`
    /// was an `SDSCodableModel` can still be decoded.
    func testHardcodedJsonLegacyDataDecodes() {
        for (idx, (constant, jsonData)) in MunrChatAccount.legacyConstants.enumerated() {
            do {
                let decoded = try JSONDecoder().decode(MunrChatAccount.self, from: jsonData)
                try constant.validate(against: decoded)
            } catch let error where error is DecodingError {
    ...
    func testContactNameComponents() {
        ...
        let nameComponents = MunrChatAccount.contactNameComponents()!
...
extension MunrChatAccount: ValidatableModel {
    static let constants: [(MunrChatAccount, jsonData: Data)] = [
        (
            MunrChatAccount(
    ...
    /// Note: These strings were generated before migrating `MunrChatAccount` from
    ...
    static let legacyConstants: [(MunrChatAccount, jsonData: Data)] = [
        (
            MunrChatAccount(
                recipientPhoneNumber: "little",
                recipientServiceId: nil, // Was hardcoded to a non-ServiceId string
    ...
    func validate(against otherAccount: MunrChatAccount) throws {
