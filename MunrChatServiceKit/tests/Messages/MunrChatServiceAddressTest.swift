...
class MunrChatServiceAddressTest: XCTestCase {

    private lazy var cache = MunrChatServiceAddressCache()
    private lazy var mockDb = InMemoryDB()

    private func makeAddress(serviceId: ServiceId? = nil, phoneNumber: String? = nil) -> MunrChatServiceAddress {
    ...
    private func makeHighTrustAddress(aci: Aci? = nil, phoneNumber: String? = nil) -> MunrChatServiceAddress {
        cache.updateRecipient(
            MunrChatRecipient(aci: aci, pni: nil, phoneNumber: phoneNumber.flatMap { E164($0) }),
    ...
    @discardableResult
    private func updateMapping(aci: Aci, phoneNumber: String? = nil) -> MunrChatServiceAddress {
    ...
    func testInitializers() {
        ...
        cache.updateRecipient(MunrChatRecipient(aci: aci, pni: nil, phoneNumber: pn_a), isPhoneNumberVisible: true)

        let address1 = MunrChatServiceAddress(
            serviceId: aci,
            phoneNumber: nil,
            cache: cache
        )
        let address2 = MunrChatServiceAddress(
            serviceId: aci,
            phoneNumber: pn_a.stringValue,
            cache: cache
        )
        let address3 = MunrChatServiceAddress(
            serviceId: aci,
        ...
        cache.updateRecipient(MunrChatRecipient(aci: aci, pni: nil, phoneNumber: pn_b), isPhoneNumberVisible: true)
    ...
    func testInitializerPerformance() {
        ...
        cache.updateRecipient(MunrChatRecipient(aci: serviceId, pni: nil, phoneNumber: phoneNumber), isPhoneNumberVisible: true)

        var addresses = [MunrChatServiceAddress]()
        addresses.reserveCapacity(iterations)

        for _ in 0..<iterations {
            addresses.append(MunrChatServiceAddress(
                serviceId: serviceId,
    ...
    func testPotentiallyVisible() {
        ...
        var aciAddresses = [MunrChatServiceAddress]()
        var pniAddresses = [MunrChatServiceAddress]()

        func addBatch(isVisible: Bool) {
            let ambiguousAddress = MunrChatServiceAddress(
                serviceId: nil,
                phoneNumber: phoneNumber?.stringValue,
                cache: cache
            )
            ...
            aciAddresses.append(MunrChatServiceAddress(
                serviceId: nil,
            ...
            pniAddresses.append(MunrChatServiceAddress(
                serviceId: pni,
                phoneNumber: nil,
                cache: cache
            ))
        ...
        cache.updateRecipient(
            MunrChatRecipient(aci: aci, pni: pni, phoneNumber: phoneNumber),
...
class MunrChatServiceAddress2Test: SSKBaseTest {
    func testPersistence() throws {
        struct TestCase {
            var originalAddress: MunrChatServiceAddress
        ...
        let testCases: [TestCase] = [
            TestCase(
                originalAddress: MunrChatServiceAddress(serviceId: aci, phoneNumber: phoneNumber),
                decodedServiceId: aci,
                decodedPhoneNumber: nil
            ),
            TestCase(
                originalAddress: MunrChatServiceAddress(phoneNumber: phoneNumber),
                decodedServiceId: nil,
                decodedPhoneNumber: phoneNumber
            ),
            TestCase(
                originalAddress: MunrChatServiceAddress(serviceId: pni, phoneNumber: nil),
        ...
        for testCase in testCases {
            do {
                ...
                let decodedValue = try JSONDecoder().decode(MunrChatServiceAddress.self, from: encodedValue)
            ...
            do {
                ...
                let decodedValue = try NSKeyedUnarchiver.unarchivedObject(
                    ofClass: MunrChatServiceAddress.self,
                    from: encodedValue,
                    requiringSecureCoding: true
                )!
                XCTAssertEqual(decodedValue.serviceId, testCase.decodedServiceId)
