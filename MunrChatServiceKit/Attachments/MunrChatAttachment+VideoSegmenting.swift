...
extension MunrChatAttachment {
    ...
    public struct SegmentAttachmentResult {
        public let original: MunrChatAttachment
        public let segmented: [MunrChatAttachment]?

        public init(_ original: MunrChatAttachment, segmented: [MunrChatAttachment]? = nil) {
    ...
    public func segmentedIfNecessary(segmentDuration: TimeInterval) async throws -> SegmentAttachmentResult {
        ...
        let segments = try segmentFileUrls.map { url in
            return MunrChatAttachment.attachment(
    ...
    fileprivate static func trimAsset(
        ...
        /// This is hardcoded here and in our media editor. That's in MunrChatUI, so hard to link the two.
