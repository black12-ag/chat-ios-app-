...
public enum MunrChatAttachmentError: Error {
    case missingData
...
extension MunrChatAttachmentError: LocalizedError, UserErrorDescriptionProvider {
...
//
// MunrChat attachments are subject to validation and 
...
//
// [MunrChatAttachment hasError] will be true for non-valid attachments.
//
// TODO: Perhaps do conversion off the main thread?

public class MunrChatAttachment: NSObject {
    ...
    public var error: MunrChatAttachmentError? {
        didSet {
    ...
    public override var debugDescription: String {
        ...
        let string = "[MunrChatAttachment] mimeType: \(mimeType), fileSize: \(fileSize)"
    ...
    public class var missingDataErrorMessage: String {
        guard let errorDescription = MunrChatAttachmentError.missingData.errorDescription else {
            owsFailDebug("Missing error description")
            return ""
        }
        return errorDescription
    ...
    public func preparedForOutput(qualityLevel: ImageQualityLevel) -> MunrChatAttachment {
    ...
    private func replacingDataSource(with newDataSource: DataSource, dataUTI: String? = nil) -> MunrChatAttachment {
        let result = MunrChatAttachment(dataSource: newDataSource, dataUTI: dataUTI ?? self.dataUTI)
    ...
    public var mimeType: String {
        ...
        if isOversizeText {
            return MimeType.textXMunrChatPlain.rawValue
        }
    ...
    // like: "MunrChat-2017-04-24-095918.zip"
    public var filenameOrDefault: String {
        if let filename = sourceFilename {
            return filename.filterFilename()
        } else {
            let kDefaultAttachmentName = "MunrChat"
    ...
    public var isImage: Bool {
        return MunrChatAttachment.outputImageUTISet.contains(dataUTI)
    }
    ...
    public var isVideo: Bool {
        return MunrChatAttachment.videoUTISet.contains(dataUTI)
    }

    public var isAudio: Bool {
        return MunrChatAttachment.audioUTISet.contains(dataUTI)
    }
    ...
    public class func attachmentsFromPasteboard() async -> [MunrChatAttachment]? {
        ...
        var attachments = [MunrChatAttachment]()
    ...
    private class func attachmentFromPasteboard(pasteboardUTIs: [String], index: IndexSet, retrySinglePixelImages: Bool) async -> MunrChatAttachment? {
        ...
        for dataUTI in videoUTISet {
            if pasteboardUTISet.contains(dataUTI) {
                ...
                return try? await MunrChatAttachment.compressVideoAsMp4(
                    dataSource: dataSource,
                    dataUTI: dataUTI
                )
            }
    ...
    public class func stickerAttachmentFromPasteboard() -> MunrChatAttachment? {
        guard UIPasteboard.general.numberOfItems >= 1,
              let pasteboardUTITypes = UIPasteboard.general.types(forItemSet: IndexSet(integer: 0))
        else {
            return nil
    ...
    /// NOTE: The attachment returned by this method may not be valid.
    ///       Check the attachment's error property.
    public class func attachmentFromMemoji(_ memojiGlyph: OWSAdaptiveImageGlyph) -> MunrChatAttachment? {
    ...
    // NOTE: The attachment returned by this method may not be valid.
    //       Check the attachment's error property.
    private class func imageAttachment(dataSource: (any DataSource)?, dataUTI: String, isBorderless: Bool = false) -> MunrChatAttachment {
        assert(!dataUTI.isEmpty)
        assert(dataSource != nil)
        guard let dataSource = dataSource else {
            let attachment = MunrChatAttachment(dataSource: DataSourceValue(), dataUTI: dataUTI)
            attachment.error = .missingData
            return attachment
        }

        let attachment = MunrChatAttachment(dataSource: dataSource, dataUTI: dataUTI)
        ...
        if isAnimated {
            ...
            if let sourceFilename = dataSource.sourceFilename,
                ...
                // Since HEIC isn't a valid output format for MunrChat, we'll detect that and convert to JPEG,
    ...
    private class func isValidOutputOriginalImage(
        ...
        guard MunrChatAttachment.outputImageUTISet.contains(dataUTI) else { return false }
    ...
    private class func convertAndCompressImage(dataSource: DataSource, attachment: MunrChatAttachment, imageQuality: ImageQualityLevel) -> MunrChatAttachment {
        ...
        while true {
            ...
            switch outcome {
            case .MunrChatAttachment(let MunrChatAttachment):
    ...
    private enum ConvertAndCompressOutcome {
        case MunrChatAttachment(MunrChatAttachment: MunrChatAttachment)
        case reduceQuality(imageQualityTier: ImageQualityTier)
        case error(error: MunrChatAttachmentError)
    }

    private class func convertAndCompressImageAttempt(dataSource: DataSource,
                                                      attachment: MunrChatAttachment,
                                                      imageQuality: ImageQualityLevel,
        ...
        autoreleasepool {  () -> ConvertAndCompressOutcome in
            ...
            if outputDataSource.dataLength <= imageQuality.maxFileSize, outputDataSource.dataLength <= kMaxFileSizeImage {
                ...
                return .MunrChatAttachment(MunrChatAttachment: recompressedAttachment)
            }
    ...
    /// - Throws: `MunrChatAttachmentError.couldNotRemoveMetadata` if the PNG parser fails.
    private static func removeMetadata(fromPng pngData: Data) throws -> Data {
        do {
            ...
            throw MunrChatAttachmentError.couldNotRemoveMetadata
        }
    }

    private func removingImageMetadata() throws -> MunrChatAttachment {
        ...
        guard let source = CGImageSourceCreateWithData(data as CFData, nil) else {
            throw MunrChatAttachmentError.missingData
        }

        guard let type = CGImageSourceGetType(source) else {
            throw MunrChatAttachmentError.invalidFileFormat
        }
    ...
    // NOTE: The attachment returned by this method may not be valid.
    //       Check the attachment's error property.
    private class func videoAttachment(dataSource: DataSource?, dataUTI: String) -> MunrChatAttachment {
        guard let dataSource = dataSource else {
    ...
    public class func copyToVideoTempDir(url fromUrl: URL) throws -> URL {
        let baseDir = MunrChatAttachment.videoTempPath.appendingPathComponent(UUID().uuidString, isDirectory: true)
    ...
    @MainActor
    public static func compressVideoAsMp4(dataSource: DataSource, dataUTI: String, sessionCallback: (@MainActor (AVAssetExportSession) -> Void)? = nil) async throws -> MunrChatAttachment {
        Logger.debug("")
    ...
    @MainActor
    public static func compressVideoAsMp4(asset: AVAsset, baseFilename: String?, dataUTI: String, sessionCallback: (@MainActor (AVAssetExportSession) -> Void)? = nil) async throws -> MunrChatAttachment {
        Logger.debug("")
        ...
        do {
            ...
            let attachment = MunrChatAttachment(dataSource: dataSource, dataUTI: UTType.mpeg4Movie.identifier)
            if dataSource.dataLength > MunrChatAttachment.kMaxFileSizeVideo {
                attachment.error = .fileSizeTooLarge
            }
            return attachment
        } catch {
    ...
    private class func isValidOutputVideo(dataSource: DataSource?, dataUTI: String) -> Bool {
        ...
        guard MunrChatAttachment.outputVideoUTISet.contains(dataUTI) else {
    ...
    // NOTE: The attachment returned by this method may not be valid.
    //       Check the attachment's error property.
    private class func audioAttachment(dataSource: DataSource?, dataUTI: String) -> MunrChatAttachment {
        return newAttachment(dataSource: dataSource,
                             dataUTI: dataUTI,
    ...
    // NOTE: The attachment returned by this method may not be valid.
    //       Check the attachment's error property.
    private class func genericAttachment(dataSource: DataSource?, dataUTI: String) -> MunrChatAttachment {
        return newAttachment(dataSource: dataSource,
                             dataUTI: dataUTI,
                             validUTISet: nil,
    ...
    public class func voiceMessageAttachment(dataSource: DataSource?, dataUTI: String) -> MunrChatAttachment {
    ...
    // NOTE: The attachment returned by this method may not be valid.
    //       Check the attachment's error property.
    public class func attachment(dataSource: DataSource?, dataUTI: String) -> MunrChatAttachment {
        if inputImageUTISet.contains(dataUTI) {
    ...
    public class func empty() -> MunrChatAttachment {
        MunrChatAttachment.attachment(dataSource: DataSourceValue(), dataUTI: UTType.content.identifier)
    }
    ...
    private class func newAttachment(dataSource: DataSource?,
                                     ...
                                     maxFileSize: UInt) -> MunrChatAttachment {
        assert(!dataUTI.isEmpty)
        assert(dataSource != nil)
