...
private class IncomingContactSyncJobRunner: JobRunner {
    ...
    /// Clear ``MunrChatAccount``s that weren't part of a complete sync.
    ///
    /// Although "system contact" details (represented by a ``MunrChatAccount``)
    ...
    private func pruneContacts(exceptThoseReceivedFromCompleteSync phoneNumbers: [E164]) async throws {
        try await SSKEnvironment.shared.databaseStorageRef.awaitableWrite { transaction in
            ...
            // create a MunrChatAccount for your own address. (If you're a primary, this
            ...
            // Rather than collecting MunrChatAccount objects, collect their unique IDs.
            ...
            MunrChatAccount.anyEnumerate(transaction: transaction, batchingPreference: .batched(8)) { MunrChatAccount, _ in
                if let phoneNumber = E164(MunrChatAccount.recipientPhoneNumber), setOfPhoneNumbers.contains(phoneNumber) {
                    // This contact was received in this batch, so don't remove it.
                    return
                }
                uniqueIdsToRemove.append(MunrChatAccount.uniqueId)
            }
            Logger.info("Removing \(uniqueIdsToRemove.count) contacts during contact sync")
            for uniqueId in uniqueIdsToRemove {
                autoreleasepool {
                    guard let MunrChatAccount = MunrChatAccount.anyFetch(uniqueId: uniqueId, transaction: transaction) else {
                        return
                    }
                    MunrChatAccount.anyRemove(transaction: transaction)
                }
            }
            if !uniqueIdsToRemove.isEmpty {
                SSKEnvironment.shared.contactManagerImplRef.didUpdateMunrChatAccounts(transaction: transaction)
            }
        }
    }
}
