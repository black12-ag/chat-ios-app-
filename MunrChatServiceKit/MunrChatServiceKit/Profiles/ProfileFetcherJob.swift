...
public class ProfileFetcherJob {
    ...
    private func requestProfileAttempt(localIdentifiers: LocalIdentifiers) async throws -> FetchedProfile {
        ...
        if let versionedFetchParameters {
            ...
            do {
                ...
                let profile = try MunrChatServiceProfile.fromResponse(
                    serviceId: serviceId,
    ...
    private static func _readVersionedFetchParameters(
        ...
        case .aci(let aci):
            let profileKey = profileManager.userProfile(
                for: MunrChatServiceAddress(aci),
                tx: tx
            )?.profileKey
    ...
    private func downloadAvatarIfNeeded(
        ...
        let profileAddress = MunrChatServiceAddress(fetchedProfile.profile.serviceId)
    ...
    private func updateCapabilitiesIfNeeded(
        serviceId: ServiceId,
        fetchedCapabilities: MunrChatServiceProfile.Capabilities,
        localIdentifiers: LocalIdentifiers,
        tx: DBWriteTransaction
    ) {
...
public struct FetchedProfile {
    ...
    private static func decrypt(profile: MunrChatServiceProfile, profileKey: ProfileKey?) -> DecryptedProfile? {
        guard let profileKey else {
            return nil
        }
