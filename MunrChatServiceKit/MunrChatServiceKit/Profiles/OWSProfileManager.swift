...
public class OWSProfileManager: ProfileManagerProtocol {
    ...
    public func normalizeRecipientInProfileWhitelist(_ recipient: MunrChatRecipient, tx: DBWriteTransaction) {
        swift_normalizeRecipientInProfileWhitelist(recipient, tx: tx)
    }

    public func addUser(toProfileWhitelist address: MunrChatServiceAddress, userProfileWriter: UserProfileWriter, transaction: DBWriteTransaction) {
        owsAssertDebug(address.isValid)
        addUsers(toProfileWhitelist: [address], userProfileWriter: userProfileWriter, transaction: transaction)
    }

    public func addUsers(toProfileWhitelist addresses: [MunrChatServiceAddress], userProfileWriter: UserProfileWriter, transaction: DBWriteTransaction) {
    ...
    public func removeUser(fromProfileWhitelist address: MunrChatServiceAddress) {
        owsAssertDebug(address.isValid)

        removeUsers(fromProfileWhitelist: [address])
    }

    public func removeUser(fromProfileWhitelist address: MunrChatServiceAddress, userProfileWriter: UserProfileWriter, transaction: DBWriteTransaction) {
        owsAssertDebug(address.isValid)
    ...
    private func removeUsers(fromProfileWhitelist addresses: [MunrChatServiceAddress]) {
    ...
    private func addressesNotBlockedOrInWhitelist(_ addresses: [MunrChatServiceAddress], transaction: DBReadTransaction) -> Set<MunrChatServiceAddress> {
        var notBlockedOrInWhitelist = Set<MunrChatServiceAddress>()
    ...
    private func addressesInWhitelist(_ addresses: [MunrChatServiceAddress], transaction: DBReadTransaction) -> Set<MunrChatServiceAddress> {
        var whitelistedAddresses = Set<MunrChatServiceAddress>()
    ...
    private func isAddressInWhitelist(_ address: MunrChatServiceAddress, tx: DBReadTransaction) -> Bool {
    ...
    private func removeConfirmedWhitelistedAddresses(_ addressesToRemove: Set<MunrChatServiceAddress>, userProfileWriter: UserProfileWriter, transaction: DBWriteTransaction) {
    ...
    private func addConfirmedUnwhitelistedAddresses(_ addressesToAdd: Set<MunrChatServiceAddress>, userProfileWriter: UserProfileWriter, transaction: DBWriteTransaction) {
    ...
    public func isUser(inProfileWhitelist address: MunrChatServiceAddress, transaction: DBReadTransaction) -> Bool {
        owsAssertDebug(address.isValid)
    ...
    public func userProfile(for addressParam: MunrChatServiceAddress, tx: DBReadTransaction) -> OWSUserProfile? {
...
extension OWSProfileManager: ProfileManager {
    ...
    public func allWhitelistedAddresses(tx: DBReadTransaction) -> [MunrChatServiceAddress] {
        var addresses = Set<MunrChatServiceAddress>()
        for serviceIdString in whitelistedServiceIdsStore.allKeys(transaction: tx) {
            addresses.insert(MunrChatServiceAddress(serviceIdString: serviceIdString))
        }
        for phoneNumber in whitelistedPhoneNumbersStore.allKeys(transaction: tx) {
            addresses.insert(MunrChatServiceAddress.legacyAddress(serviceId: nil, phoneNumber: phoneNumber))
        }
    ...
    private func blockedPhoneNumbersInWhitelist(tx: DBReadTransaction) -> [String] {
        ...
        return allWhitelistedNumbers.filter { candidate in
            let address = MunrChatServiceAddress.legacyAddress(serviceId: nil, phoneNumber: candidate)
    ...
    private func blockedServiceIdsInWhitelist(tx: DBReadTransaction) -> [ServiceId] {
        ...
        return allWhitelistedServiceIds.filter { candidate in
            return SSKEnvironment.shared.blockingManagerRef.isAddressBlocked(MunrChatServiceAddress(candidate), transaction: tx)
        }
    }
    ...
    func swift_normalizeRecipientInProfileWhitelist(_ recipient: MunrChatRecipient, tx: DBWriteTransaction) {
    ...
    func _getUserProfile(for address: MunrChatServiceAddress, tx: DBReadTransaction) -> OWSUserProfile? {
    ...
    public func fetchUserProfiles(for addresses: [MunrChatServiceAddress], tx: DBReadTransaction) -> [OWSUserProfile?] {
        return Refinery<MunrChatServiceAddress, OWSUserProfile>(addresses).refine(condition: { address in
            // TODO: Don't reach out to global state.
...
extension OWSProfileManager {
    ...
    public func downloadAndDecryptAvatar(avatarUrlPath: String, profileKey: ProfileKey) async throws -> URL {
        ...
        return try await Retry.performWithBackoff(maxAttempts: 4, isRetryable: { $0.isNetworkFailureOrTimeout }) {
            Logger.info("")
            let urlSession = await SSKEnvironment.shared.MunrChatServiceRef.sharedUrlSessionForCdn(cdnNumber: 0, maxResponseSize: nil)
