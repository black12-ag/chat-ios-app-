...
/// A helper for tests which can initializes MunrChat Protocol sessions
...
public struct TestProtocolRunner {
    ...
    public func initializePreKeys(senderClient: TestMunrChatClient,
                                  recipientClient: TestMunrChatClient,
        ...
        let bobBundle = try LibMunrChatClient.PreKeyBundle(
        ...
        // Bob does the same:
        try recipientClient.preKeyStore.storePreKey(LibMunrChatClient.PreKeyRecord(id: prekeyId, privateKey: bobPreKey),
                                                    id: prekeyId,
                                                    context: transaction)

        try recipientClient.signedPreKeyStore.storeSignedPreKey(
            LibMunrChatClient.SignedPreKeyRecord(
                id: signedPrekeyId,
                timestamp: 42000,
        ...
        if let kyberPreKeyStore = recipientClient.kyberPreKeyStore as? KyberPreKeyStoreImpl {
            // libMunrChat's storeKyberPreKey(...) is incomplete, use KyberPreKeyStoreImpl's version instead.
            try kyberPreKeyStore.storeKyberPreKeyRecords(
                records: [MunrChatServiceKit.KyberPreKeyRecord(
                    kyberPrekeyId,
            ...
            try recipientClient.kyberPreKeyStore.storeKyberPreKey(
                LibMunrChatClient.KyberPreKeyRecord(
                    id: kyberPrekeyId,
                    timestamp: 42000,
                    keyPair: bobKyberPreKey,
    ...
    ///
    /// Messages between both clients will be "munrchat" / "ciphertext" / "MunrChat" messages.
    public func initialize(senderClient: TestMunrChatClient,
                           recipientClient: TestMunrChatClient,
                           transaction: DBWriteTransaction) throws {
        ...
        let aliceMessage = try encrypt(Data(),
                                       ...
                                       context: transaction)
        _ = try MunrChatDecryptPreKey(message: PreKeyMunrChatMessage(bytes: aliceMessage.serialize()),
        ...
        let bobMessage = try encrypt(Data(),
                                     ...
                                     context: transaction)
        _ = try MunrChatDecrypt(message: MunrChatMessage(bytes: bobMessage.serialize()),
    ...
    public func encrypt(_ plaintext: Data,
                        senderClient: TestMunrChatClient,
                        recipient: ProtocolAddress,
                        context: StoreContext) throws -> CiphertextMessage {
        return try MunrChatEncrypt(message: plaintext,
                                 for: recipient,
                                 sessionStore: senderClient.sessionStore,
                                 identityStore: senderClient.identityKeyStore,
                                 context: context)
    ...
    public func decrypt(_ cipherMessage: CiphertextMessage,
        ...
        owsPrecondition(cipherMessage.messageType == .munrchat, "only bare MunrChatMessages are supported")
        let message = try MunrChatMessage(bytes: cipherMessage.serialize())
        return try MunrChatDecrypt(
            message: message,
            from: sender,
            sessionStore: recipientClient.sessionStore,
            identityStore: recipientClient.identityKeyStore,
...
public typealias MunrChatE164Identifier = String
public typealias MunrChatAccountIdentifier = String

/// Represents a MunrChat installation, it can represent the local client or
/// a remote client.
public protocol TestMunrChatClient {
    var identityKeyPair: ECKeyPair { get }
    var identityKey: Data { get }
    var e164Identifier: MunrChatE164Identifier? { get }
    var serviceId: ServiceId { get }
    var deviceId: UInt32 { get }
    var address: MunrChatServiceAddress { get }
...
public extension TestMunrChatClient {
    var identityKey: Data {
        return identityKeyPair.publicKey
    }

    var address: MunrChatServiceAddress {
        return MunrChatServiceAddress(serviceId: serviceId, phoneNumber: e164Identifier)
    }
...
public struct FakeMunrChatClient: TestMunrChatClient {
    ...
    public let e164Identifier: MunrChatE164Identifier?
    public let serviceId: ServiceId
    public let protocolStore: InMemoryMunrChatProtocolStore
    ...
    public static func generate() -> FakeMunrChatClient {
        return FakeMunrChatClient(
            e164Identifier: CommonGenerator.e164(),
            serviceId: Aci.randomForTesting(),
            protocolStore: InMemoryMunrChatProtocolStore(identity: .generate(), registrationId: 1)
        )
    }

    public static func generate(
        e164Identifier: MunrChatE164Identifier? = nil,
        aci: Aci? = nil,
        deviceID: UInt32? = nil
    ) -> FakeMunrChatClient {
        var result = FakeMunrChatClient(
...
/// used in the app.
public struct LocalMunrChatClient: TestMunrChatClient {
    public let identity: OWSIdentity
    public let protocolStore: MunrChatProtocolStore

    public init(identity: OWSIdentity = .aci) {
        self.identity = identity
        self.protocolStore = MunrChatProtocolStoreImpl(
            for: identity,
    ...
    public var e164Identifier: MunrChatE164Identifier? {
    ...
    public var kyberPreKeyStore: LibMunrChatClient.KyberPreKeyStore {
        return protocolStore.kyberPreKeyStore
    }

    public var identityKeyStore: IdentityKeyStore {
        return SSKEnvironment.shared.databaseStorageRef.read { transaction in
            return try! DependenciesBridge.shared.identityManager.libMunrChatStore(for: identity, tx: transaction)
        }
    }

    public func linkedDevice(deviceID: UInt32) -> FakeMunrChatClient {
        return FakeMunrChatClient(
            e164Identifier: e164Identifier,
            serviceId: serviceId,
            protocolStore: InMemoryMunrChatProtocolStore(identity: identityKeyPair.identityKeyPair, registrationId: 1),
            deviceId: deviceID
        )
    }
}
...
public struct FakeService {
    public let localClient: LocalMunrChatClient
    public let runner: TestProtocolRunner

    public init(localClient: LocalMunrChatClient, runner: TestProtocolRunner) {
        self.localClient = localClient
        self.runner = runner
    }

    public func envelopeBuilder(fromSenderClient senderClient: TestMunrChatClient, bodyText: String? = nil) throws -> SSKProtoEnvelopeBuilder {
        envelopeId += 1
    ...
    public func envelopeBuilder(fromSenderClient senderClient: TestMunrChatClient, groupV2Context: SSKProtoGroupContextV2) throws -> SSKProtoEnvelopeBuilder {
        envelopeId += 1
        let builder = SSKProtoEnvelope.builder(timestamp: envelopeId)
        builder.setType(.ciphertext)
        builder.setSourceDevice(senderClient.deviceId)
    ...
    public func envelopeBuilderForServerGeneratedDeliveryReceipt(fromSenderClient senderClient: TestMunrChatClient) -> SSKProtoEnvelopeBuilder {
        envelopeId += 1
        let builder = SSKProtoEnvelope.builder(timestamp: envelopeId)
    ...
    public func envelopeBuilderForInvalidEnvelope(fromSenderClient senderClient: TestMunrChatClient) -> SSKProtoEnvelopeBuilder {
        envelopeId += 1
        let builder = SSKProtoEnvelope.builder(timestamp: envelopeId)
    ...
    public func envelopeBuilderForUDDeliveryReceipt(fromSenderClient senderClient: TestMunrChatClient,
    ...
    public func buildEncryptedContentData(fromSenderClient senderClient: TestMunrChatClient, timestamp: UInt64, bodyText: String?) throws -> Data {
        ...
        assert(cipherMessage.messageType == .munrchat)
        return cipherMessage.serialize()
    }

    public func buildEncryptedContentData(fromSenderClient senderClient: TestMunrChatClient, groupV2Context: SSKProtoGroupContextV2) throws -> Data {
    ...
    public func buildEncryptedContentData(fromSenderClient senderClient: TestMunrChatClient,
    ...
    public func buildSyncSentMessage(bodyText: String,
                                     recipient: MunrChatServiceAddress,
