...
struct StorageServiceContact {
    ...
    init?(_ MunrChatRecipient: MunrChatRecipient) {
        let unregisteredAtTimestamp: UInt64?
        if MunrChatRecipient.isRegistered {
            unregisteredAtTimestamp = nil
        } else {
            unregisteredAtTimestamp = (
                MunrChatRecipient.unregisteredAtTimestamp ?? MunrChatRecipient.Constants.distantPastUnregisteredTimestamp
            )
        }
        self.init(
            aci: MunrChatRecipient.aci,
            phoneNumber: E164.expectNilOrValid(stringValue: MunrChatRecipient.phoneNumber?.stringValue),
...
class StorageServiceContactRecordUpdater: StorageServiceRecordUpdater {
    ...
    func buildRecord(
        ...
        let anyAddress = MunrChatServiceAddress(contact.serviceIds.aciOrElsePni)
        ...
        let systemContact = { () -> MunrChatAccount? in
            guard let phoneNumber = contact.phoneNumber else {
                return nil
            }
            return contactsManager.fetchMunrChatAccount(
    ...
    private func _mergeRecord(
        ...
        let anyAddress = MunrChatServiceAddress(serviceIds.aciOrElsePni)
    ...
    private func mergeSystemContactNames(
        ...
        // the merging logic will delete the MunrChatAccount.
        guard let phoneNumber = recipient.phoneNumber?.stringValue else {
            return false
        }

        let localAccount = contactsManager.fetchMunrChatAccount(
        ...
        let newAccount: MunrChatAccount?
        ...
        if let systemFullName {
            ...
            // `MunrChatAccount`s are associated with the same system contact.
            // For example, Alice may have a work and mobile number, both of
            // of which are registered with MunrChat. This text could be (work)
            // or (mobile), to help disambiguate - otherwise, both MunrChat
            // accounts will present as just "Alice".
            let multipleAccountLabelText = ""

            newAccount = MunrChatAccount(
        ...
        default:
            ...
            // case and `didModifyMunrChatAccount` will remain false.
            var didModifyMunrChatAccount = false
            if let localAccount {
                localAccount.anyRemove(transaction: SDSDB.shimOnlyBridge(tx))
                didModifyMunrChatAccount = true
            }
            if let newAccount {
                newAccount.anyInsert(transaction: SDSDB.shimOnlyBridge(tx))
                didModifyMunrChatAccount = true
            }
            if didModifyMunrChatAccount {
                contactsManager.didUpdateMunrChatAccounts(transaction: SDSDB.shimOnlyBridge(tx))
            }
            ...
            if aciToUpdate != nil {
                ...
                MunrChatServiceAddressCache.updateRecipient(recipient, tx: tx)
            }
        }
    ...
    fileprivate func processPinnedConversationsProto(
        ...
        for pinnedConversation in pinnedConversations {
            ...
            case .contact(let contact)?:
                let address = MunrChatServiceAddress.legacyAddress(
