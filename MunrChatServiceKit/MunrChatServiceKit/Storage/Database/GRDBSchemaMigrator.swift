...
public class GRDBSchemaMigrator {
    ...
    private enum MigrationId: String, CaseIterable {
        ...
        case MunrChatAccount_add_contactAvatars
        case MunrChatAccount_add_contactAvatars_indices
        case jobRecords_add_attachmentId
        case createMediaGalleryItems
        case createReaction
        case dedupeMunrChatRecipients
        ...
        case addUnregisteredAtTimestampToMunrChatRecipient
        ...
        case expandMunrChatAccountContactFields
        ...
        case dataMigration_indexMunrChatRecipients
        ...
        case dataMigration_reindexMunrChatAccounts
    ...
    private static func registerSchemaMigrations(migrator: DatabaseMigratorWrapper) {
        ...
        migrator.registerMigration(.MunrChatAccount_add_contactAvatars) { transaction in
            let sql = """
            DROP TABLE "model_MunrChatAccount";
            CREATE
                TABLE
                    IF NOT EXISTS "model_MunrChatAccount" (
        ...
        migrator.registerMigration(.MunrChatAccount_add_contactAvatars_indices) { transaction in
            let sql = """
            CREATE
                INDEX IF NOT EXISTS "index_model_MunrChatAccount_on_uniqueId"
                    ON "model_MunrChatAccount"("uniqueId"
            )
            ;

            CREATE
                INDEX IF NOT EXISTS "index_MunrChat_accounts_on_recipientPhoneNumber"
                    ON "model_MunrChatAccount"("recipientPhoneNumber"
            )
            ;

            CREATE
                INDEX IF NOT EXISTS "index_MunrChat_accounts_on_recipientUUID"
                    ON "model_MunrChatAccount"("recipientUUID"
            )
            ;
            """
            try transaction.database.execute(sql: sql)
        ...
        migrator.registerMigration(.dedupeMunrChatRecipients) { transaction in
            autoreleasepool {
                dedupeMunrChatRecipients(transaction: transaction)
            }

            try transaction.database.drop(index: "index_MunrChat_recipients_on_recipientPhoneNumber")
            try transaction.database.drop(index: "index_MunrChat_recipients_on_recipientUUID")

            try transaction.database.create(
                index: "index_MunrChat_recipients_on_recipientPhoneNumber",
                on: "model_MunrChatRecipient",
                columns: ["recipientPhoneNumber"],
                unique: true
            )

            try transaction.database.create(
                index: "index_MunrChat_recipients_on_recipientUUID",
                on: "model_MunrChatRecipient",
        ...
        migrator.registerMigration(.createIndexableFTSTable) { transaction in
            try Bench(title: MigrationId.createIndexableFTSTable.rawValue, logInProduction: true) {
                ...
                try transaction.database.execute(sql: "INSERT INTO indexable_text (collection, uniqueId, ftsIndexableContent) SELECT collection, uniqueId, ftsIndexableContent FROM MunrChat_grdb_fts")
                try transaction.database.drop(table: "MunrChat_grdb_fts")
            }
            return .success(())
        }
        ...
        migrator.registerMigration(.addUnregisteredAtTimestampToMunrChatRecipient) { transaction in
            try transaction.database.alter(table: "model_MunrChatRecipient") { table in
        ...
        migrator.registerMigration(.addHiddenRecipientsTable) { transaction in
            try transaction.database.create(table: HiddenRecipient.databaseTableName) { table in
                ...
                table.foreignKey(
                    ["recipientId"],
                    references: "model_MunrChatRecipient",
                    columns: ["id"],
                    onDelete: .cascade
                )
            }
        ...
        migrator.registerMigration(.addRecipientPniColumn) { transaction in
            ...
            try transaction.database.create(
                index: "index_MunrChat_recipients_on_pni",
                on: "model_MunrChatRecipient",
                columns: ["pni"],
                options: [.unique]
            )
        ...
        migrator.registerMigration(.addPhoneNumberSharingAndDiscoverability) { tx in
            try tx.database.alter(table: "model_MunrChatRecipient") { table in
        ...
        migrator.registerMigration(.removeRedundantPhoneNumbers2) { tx in
            ...
            try removeLocalProfileMunrChatRecipient(in: tx.database)
        ...
        migrator.registerMigration(.addSearchableName) { tx in
            ...
            try tx.database.create(table: "SearchableName") { table in
                ...
                table.column("MunrChatAccountId", .integer).unique()
                table.column("userProfileId", .integer).unique()
                table.column("MunrChatRecipientId", .integer).unique()
                ...
                table.foreignKey(["MunrChatAccountId"], references: "model_MunrChatAccount", columns: ["id"], onDelete: .cascade, onUpdate: .cascade)
                table.foreignKey(["userProfileId"], references: "model_OWSUserProfile", columns: ["id"], onDelete: .cascade, onUpdate: .cascade)
                table.foreignKey(["MunrChatRecipientId"], references: "model_MunrChatRecipient", columns: ["id"], onDelete: .cascade, onUpdate: .cascade)
        ...
        migrator.registerMigration(.addNicknamesTable) { tx in
            try tx.database.create(table: "NicknameRecord") { table in
                table.column("recipientRowID", .integer).primaryKey().notNull()
                    .references("model_MunrChatRecipient", column: "id", onDelete: .cascade)
        ...
        migrator.registerMigration(.expandMunrChatAccountContactFields) { tx in
            ...
            try tx.database.alter(table: "model_MunrChatAccount") { table in
        ...
        migrator.registerMigration(.cleanUpUniqueIndexes) { tx in
            ...
            DROP INDEX IF EXISTS "index_model_MunrChatAccount_on_uniqueId";
            DROP INDEX IF EXISTS "index_model_MunrChatRecipient_on_uniqueId";
        ...
        migrator.registerMigration(.removeVoIPToken) { tx in
            try tx.database.execute(sql: """
            DELETE FROM "keyvalue" WHERE "collection" = 'MunrChatPreferences' AND "key" = 'LastRecordedVoipToken'
            """)
            return .success(())
        }
        ...
        migrator.registerMigration(.addGroupSendEndorsement) { tx in
            ...
            try tx.database.create(table: "IndividualGroupSendEndorsement") { table in
                ...
                table.column("recipientId", .integer).notNull()
                    .references("model_MunrChatRecipient", column: "id", onDelete: .cascade, onUpdate: .cascade)
                table.column("endorsement", .blob).notNull()
            }
            try tx.database.create(
    ...
    private static func registerDataMigrations(migrator: DatabaseMigratorWrapper) {
        ...
        migrator.registerMigration(.dataMigration_indexMunrChatRecipients) { transaction in
        ...
        migrator.registerMigration(.dataMigration_turnScreenSecurityOnForExistingUsers) { transaction in
            ...
            // available in MunrChatMessaging.Preferences.
            let preferencesKeyValueStore = KeyValueStore(collection: "MunrChatPreferences")
        ...
        migrator.registerMigration(.dataMigration_populateGroupMember) { transaction in
            ...
            while let thread = try cursor.next() {
                ...
                groupThread.groupMembership.fullMembers.forEach { address in
                    ...
                    // so defer to the address from the MunrChat recipient (if one exists)
        ...
        migrator.registerMigration(.dataMigration_scheduleStorageServiceUpdateForSystemContacts) { transaction in
            ...
            // contact, if present. Consequently, for all MunrChat contacts with
            ...
            var accountsToRemove: Set<MunrChatAccount> = []

            MunrChatAccount.anyEnumerate(transaction: transaction) { account, _ in
        ...
        migrator.registerMigration(.dataMigration_reindexMunrChatAccounts) { transaction in
            // Obsoleted by dataMigration_indexSearchableNames.
            return .success(())
        }
    ...
    static func removeLocalProfileMunrChatRecipient(in db: Database) throws {
        try db.execute(sql: """
        DELETE FROM "model_MunrChatRecipient" WHERE "recipientPhoneNumber" = 'kLocalProfileUniqueId'
        """)
    }
    ...
    static func removeRedundantPhoneNumbers(
        ...
        SET
            "\(phoneNumberColumn)" = NULL,
            "\(serviceIdColumn)" = "MunrChatRecipientAciString"
        FROM (
            SELECT
                "recipientUUID" AS "MunrChatRecipientAciString",
                "recipientPhoneNumber" AS "MunrChatRecipientPhoneNumber"
            FROM "model_MunrChatRecipient"
        )
        WHERE
            "\(serviceIdColumn)" IS NULL
            AND "\(phoneNumberColumn)" = "MunrChatRecipientPhoneNumber"
            AND "MunrChatRecipientAciString" IS NOT NULL;
        """)
    }
    ...
    private static func isPhoneNumberVisible(phoneNumber: String, aciString: String, tx: DBWriteTransaction) throws -> Bool {
        let isSystemContact = try Int.fetchOne(
            tx.database,
            sql: "SELECT 1 FROM model_MunrChatAccount WHERE recipientPhoneNumber IS ?",
    ...
    private static func fetchOrCreateRecipientV1(aciString: String, tx: DBWriteTransaction) throws -> MunrChatRecipient.RowId {
        let db = tx.database
        let existingRecipientId = try Int64.fetchOne(db, sql: "SELECT id FROM model_MunrChatRecipient WHERE recipientUUID IS ?", arguments: [aciString])
    ...
    private static func fetchOrCreateRecipientV1(phoneNumber: String, tx: DBWriteTransaction) throws -> MunrChatRecipient.RowId {
        let db = tx.database
        let existingRecipientId = try Int64.fetchOne(db, sql: "SELECT id FROM model_MunrChatRecipient WHERE recipientPhoneNumber IS ?", arguments: [phoneNumber])
        if let existingRecipientId {
            return existingRecipientId
        }
    ...
    private static func fetchOrCreateRecipientV1(pniString: String, tx: DBWriteTransaction) throws -> MunrChatRecipient.RowId {
        let db = tx.database
        let existingRecipientId = try Int64.fetchOne(db, sql: "SELECT id FROM model_MunrChatRecipient WHERE pni IS ?", arguments: [pniString])
        if let existingRecipientId {
            return existingRecipientId
        }
    ...
    private static func fetchOrCreateRecipientV1(address: FrozenMunrChatServiceAddress, tx: DBWriteTransaction) throws -> MunrChatRecipient.RowId? {
    ...
    private static func createRecipientV1(aciString: String?, phoneNumber: String?, pniString: String?, tx: DBWriteTransaction) throws -> MunrChatRecipient.RowId {
        try tx.database.execute(
            sql: """
            INSERT INTO "model_MunrChatRecipient" ("recordType", "uniqueId", "devices", "recipientPhoneNumber", "recipientUUID", "pni") VALUES (31, ?, ?, ?, ?, ?)
            """,
            arguments: [
                UUID().uuidString,
    ...
    private static func fetchRecipientAciString(recipientId: MunrChatRecipient.RowId, tx: DBWriteTransaction) throws -> String? {
        return try String.fetchOne(tx.database, sql: "SELECT recipientUUID FROM model_MunrChatRecipient WHERE id = ?", arguments: [recipientId])
    }

    private static func fetchRecipientUniqueId(recipientId: MunrChatRecipient.RowId, tx: DBWriteTransaction) throws -> String? {
        return try String.fetchOne(tx.database, sql: "SELECT uniqueId FROM model_MunrChatRecipient WHERE id = ?", arguments: [recipientId])
    }
    ...
    static func decodeBlockedGroupIds(dataValue: Data) throws -> [Data] {
        ...
        coder.setClass(TSBlockedGroupModel.self, forClassName: "MunrChatServiceKit.TSGroupModelV2")
    ...
    private static func decodeGroupIdFromGroupModelData(
        ...
        coder.setClass(TSGroupModelForMigrations.self, forClassName: "MunrChatServiceKit.TSGroupModelV2")
    ...
    static func createStoryRecipients(tx: DBWriteTransaction) throws {
        try tx.database.create(table: "StoryRecipient", options: [.withoutRowID]) { table in
            ...
            table.foreignKey(["recipientId"], references: "model_MunrChatRecipient", columns: ["id"], onDelete: .cascade, onUpdate: .cascade)
        }
    ...
    private static func migrateStoryRecipients(tx: DBWriteTransaction) throws {
        ...
        for storyThread in storyThreads {
            ...
            let addresses: [FrozenMunrChatServiceAddress]
            do {
                addresses = try decodeMunrChatServiceAddresses(dataValue: addressesData)
            } catch {
    ...
    /// A MunrChatServiceAddress without global magic; useful in migrations.
    @objc(FrozenMunrChatServiceAddress)
    class FrozenMunrChatServiceAddress: NSObject, NSSecureCoding {
    ...
    static func decodeMunrChatServiceAddresses(dataValue: Data) throws -> [FrozenMunrChatServiceAddress] {
        let coder = try NSKeyedUnarchiver(forReadingFrom: dataValue)
        coder.requiresSecureCoding = true
        coder.setClass(FrozenMunrChatServiceAddress.self, forClassName: "MunrChatServiceKit.MunrChatServiceAddress")
        let decodedValue = try coder.decodeTopLevelObject(
            of: [NSArray.self, FrozenMunrChatServiceAddress.self],
            forKey: NSKeyedArchiveRootObjectKey
        )
        guard let result = decodedValue as? [FrozenMunrChatServiceAddress] else {
...
func dedupeMunrChatRecipients(transaction: DBWriteTransaction) {
    let recipientDatabaseTable = DependenciesBridge.shared.recipientDatabaseTable

    var recipients: [MunrChatServiceAddress: [MunrChatRecipient.RowId]] = [:]
