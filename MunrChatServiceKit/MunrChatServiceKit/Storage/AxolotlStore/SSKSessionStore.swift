...
public final class SSKSessionStore: MunrChatSessionStore {
    ...
    public func mightContainSession(for recipient: MunrChatRecipient, tx: DBReadTransaction) -> Bool {
    ...
    public func archiveAllSessions(for address: MunrChatServiceAddress, tx: DBWriteTransaction) {
...
extension SSKSessionStore: LibMunrChatClient.SessionStore {
    ...
    public func loadExistingSessions(
        ...
        return try addresses.map { address in
            guard let session = try loadSession(for: address, context: context) else {
                throw MunrChatError.sessionNotFound("\(address)")
            }
            return session
        }
    }
