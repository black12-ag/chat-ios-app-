...
public class SenderKeyStore {
    ...
    func readyRecipients(
        ...
        let sentKeyInfo = { () -> [MunrChatServiceAddress: SKDMSendInfo]? in
        ...
        for intendedRecipient in intendedRecipients {
            let priorSendRecipientState: SenderKeySentToRecipient = (
                sentKeyInfo?[MunrChatServiceAddress(intendedRecipient)]?.keyRecipient
            ...
            do {
                ...
                if case MunrChatError.invalidState(_) = error {
...
// MARK: - <LibMunrChatClient.SenderKeyStore>

extension SenderKeyStore: LibMunrChatClient.SenderKeyStore {
...
extension SenderKeyStore {
    ...
    public func logSKDMInfo(for recipient: MunrChatServiceAddress, transaction: DBReadTransaction) {
...
private struct KeyMetadata {
    ...
    private(set) var sentKeyInfo: [MunrChatServiceAddress: SKDMSendInfo]
    var currentRecipients: Set<MunrChatServiceAddress> { Set(sentKeyInfo.keys) }
    ...
    mutating func resetDeliveryRecord(for serviceId: ServiceId) {
        sentKeyInfo[MunrChatServiceAddress(serviceId)] = nil
    }
    ...
    mutating func recordSentSenderKey(_ sentSenderKey: SentSenderKey) {
        ...
        sentKeyInfo[MunrChatServiceAddress(sentSenderKey.recipient)] = sendInfo
    }
}
...
extension KeyMetadata: Codable {
    ...
    init(from decoder: Decoder) throws {
        ...
        // - V2: Record a mapping of MunrChatServiceAddress -> SenderKeySentToRecipient. This allowed us to
        //       track additional info about the recipient of a key like registrationId
        // - V3: Record a mapping of MunrChatServiceAddress -> SKDMSendInfo. This allows us to
        ...
        if let sendInfo = try container.decodeIfPresent([MunrChatServiceAddress: SKDMSendInfo].self, forKey: .sentKeyInfo) {
            sentKeyInfo = sendInfo
        } else if let keyRecipients = try legacyValues.decodeIfPresent([MunrChatServiceAddress: SenderKeySentToRecipient].self, forKey: .keyRecipients) {
