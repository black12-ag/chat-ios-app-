...
public class PreKeyStoreImpl {
    ...
    func generatePreKeyRecords(tx: DBWriteTransaction) -> [MunrChatServiceKit.PreKeyRecord] {
        ...
        for _ in 0..<batchSize {
            ...
            let record = MunrChatServiceKit.PreKeyRecord(id: preKeyId, keyPair: keyPair, createdAt: Date(), replacedAt: nil)
    ...
    func storePreKeyRecords(_ preKeyRecords: [MunrChatServiceKit.PreKeyRecord], tx: DBWriteTransaction) {
        for record in preKeyRecords {
            keyStore.setPreKeyRecord(record, key: keyValueStoreKey(int: Int(record.id)), transaction: tx)
        }
    }

    func loadPreKey(_ preKeyId: Int32, transaction: DBReadTransaction) -> MunrChatServiceKit.PreKeyRecord? {
        keyStore.preKeyRecord(key: keyValueStoreKey(int: Int(preKeyId)), transaction: transaction)
    }

    func storePreKey(_ preKeyId: Int32, preKeyRecord: MunrChatServiceKit.PreKeyRecord, transaction: DBWriteTransaction) {
    ...
    func setReplacedAtToNowIfNil(exceptFor exceptForPreKeyRecords: [MunrChatServiceKit.PreKeyRecord], tx: DBWriteTransaction) {
        ...
        keyStore.allKeys(transaction: tx).forEach { key in autoreleasepool {
            ...
            let record = keyStore.getObject(key, ofClass: MunrChatServiceKit.PreKeyRecord.self, transaction: tx)
...
extension PreKeyStoreImpl: LibMunrChatClient.PreKeyStore {
    enum Error: Swift.Error {
        case noPreKeyWithId(UInt32)
    }

    public func loadPreKey(id: UInt32, context: StoreContext) throws -> LibMunrChatClient.PreKeyRecord {
    ...
    public func storePreKey(_ record: LibMunrChatClient.PreKeyRecord, id: UInt32, context: StoreContext) throws {
        ...
        self.storePreKey(
            Int32(bitPattern: id),
            preKeyRecord: MunrChatServiceKit.PreKeyRecord(
...
extension KeyValueStore {
    fileprivate func preKeyRecord(key: String, transaction: DBReadTransaction) -> MunrChatServiceKit.PreKeyRecord? {
        return getObject(key, ofClass: MunrChatServiceKit.PreKeyRecord.self, transaction: transaction)
    }

    fileprivate func setPreKeyRecord(_ record: MunrChatServiceKit.PreKeyRecord, key: String, transaction: DBWriteTransaction) {
