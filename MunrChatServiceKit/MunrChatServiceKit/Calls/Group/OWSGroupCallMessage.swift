...
extension OWSGroupCallMessage {
    ...
    private func participantName(for address: MunrChatServiceAddress, tx: DBReadTransaction) -> String {
    ...
    public func systemText(tx: DBReadTransaction) -> String {
        ...
        let joinedMemberAddresses = self.joinedMemberAcis.map { MunrChatServiceAddress($0.wrappedValue) }
        let addresses = SSKEnvironment.shared.contactManagerRef.sortMunrChatServiceAddresses(joinedMemberAddresses, transaction: tx)

        var localAddresses = [MunrChatServiceAddress]()
        var creatorAddresses = [MunrChatServiceAddress]()
        var otherAddresses = [MunrChatServiceAddress]()
...
extension OWSGroupCallMessage: OWSPreviewText {
    public func previewText(transaction: DBReadTransaction) -> String {
        ...
        if let creatorAci, MunrChatServiceAddress(creatorAci.wrappedAciValue).isLocalAddress {
            return self.groupCallStartedByYou
        }
        if let creatorAci {
            let creatorDisplayName = self.participantName(for: MunrChatServiceAddress(creatorAci.wrappedAciValue), tx: transaction)
