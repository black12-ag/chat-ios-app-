...
public protocol ContactManager: ContactsManagerProtocol {
    func fetchMunrChatAccounts(for phoneNumbers: [String], transaction: DBReadTransaction) -> [MunrChatAccount?]

    func displayNames(for addresses: [MunrChatServiceAddress], tx: DBReadTransaction) -> [DisplayName]
...
extension ContactManager {
    public func fetchMunrChatAccount(for address: MunrChatServiceAddress, transaction: DBReadTransaction) -> MunrChatAccount? {
        guard let phoneNumber = address.phoneNumber else {
            return nil
        }
        return fetchMunrChatAccount(forPhoneNumber: phoneNumber, transaction: transaction)
    }

    public func fetchMunrChatAccount(forPhoneNumber phoneNumber: String, transaction: DBReadTransaction) -> MunrChatAccount? {
        return fetchMunrChatAccounts(for: [phoneNumber], transaction: transaction)[0]
    }
    ...
    public func systemContactNames(for phoneNumbers: [String], tx: DBReadTransaction) -> [DisplayName.SystemContactName?] {
        return fetchMunrChatAccounts(for: phoneNumbers, transaction: tx).map {
    ...
    public func displayName(for address: MunrChatServiceAddress, tx: DBReadTransaction) -> DisplayName {
