...
private struct AppAttestManager {
    ...
    /// has been both attested by Apple and verified by MunrChat servers.
    ///
    ...
    /// MunrChat servers, then using that key to generate an assertion for a
    /// request to MunrChat servers to perform the given action. That assertion
    /// is sent alongside the request to MunrChat servers, who upon validating the
    ...
    /// MunrChat servers.
    ///
    /// This involves requesting a challenge from MunrChat, having Apple sign that
    /// challenge using our new key, and finally having MunrChat validate that
    ...
    private func attestAndRegisterKey(newKeyId: String) async throws(AttestationError) -> AttestedKey {
        // Get a challenge from MunrChat servers.
        ...
        // Sign the challenge-known-to-MunrChat-servers using our new key (aka,
        ...
        // Give the signed challenge to MunrChat servers, who will validate that
        // the signature/attestation (and therefore the key) is valid. If this
        // succeeds, the MunrChat servers will record this key so we can use it
        ...
        // Hurray! The key is valid, and reigstered with MunrChat servers. We can
    ...
    /// Get a challenge from MunrChat servers that we can use to attest that a new
    /// key is valid.
    ...
    /// Validate an attestation, or challenge signed by a new key, with MunrChat
    /// servers. If this succeeds, MunrChat servers will record this key so it can
    ...
    ///
    /// This involves requesting a challenge from MunrChat, merging the challenge
    ...
    /// Request a challenge from MunrChat servers to generate an assertion to
