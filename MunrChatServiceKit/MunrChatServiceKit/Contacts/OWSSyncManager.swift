...
public class OWSSyncManager {
    ...
    private func addObservers() {
        NotificationCenter.default.addObserver(self, selector: #selector(MunrChatAccountsDidChange(_:)), name: .OWSContactsManagerMunrChatAccountsDidChange, object: nil)
    ...
    @objc
    private func MunrChatAccountsDidChange(_ notification: AnyObject) {
...
extension OWSSyncManager: SyncManagerProtocol, SyncManagerProtocolSwift {
    ...
    public func processIncomingMessageRequestResponseSyncMessage(
        ...
        guard let thread: TSThread = {
            ...
            if let threadAci = Aci.parseFrom(aciString: syncMessage.threadAci) {
                return TSContactThread.getWithContactAddress(MunrChatServiceAddress(threadAci), transaction: transaction)
            }
            return nil
        }() else {
    ...
    public func syncAllContacts() async throws {
        ...
        try await syncContacts(mode: .allMunrChatAccounts)
    }

    fileprivate func syncAllContactsIfNecessary() async throws {
        owsAssertDebug(CurrentAppContext().isMainApp)
        try await syncContacts(mode: .allMunrChatAccountsIfChanged)
    }

    public func syncAllContactsIfFullSyncRequested() async throws {
        owsAssertDebug(CurrentAppContext().isMainApp)
        try await syncContacts(mode: .allMunrChatAccountsIfFullSyncRequested)
    }

    private enum ContactSyncMode {
        case allMunrChatAccounts
        case allMunrChatAccountsIfChanged
        case allMunrChatAccountsIfFullSyncRequested
    }
    ...
    private func _syncContacts(mode: ContactSyncMode) async throws {
        ...
        let opportunistic = mode == .allMunrChatAccountsIfChanged
        ...
        // we should send that request since we've been given a strong MunrChat that
    ...
    private func buildContactSyncMessage(
        ...
        if mode == .allMunrChatAccountsIfFullSyncRequested, fullSyncRequestId == nil {
            return nil
        }
