...
public protocol RecipientMerger {
    ...
    func splitUnregisteredRecipientIfNeeded(
        localIdentifiers: LocalIdentifiers,
        unregisteredRecipient: MunrChatRecipient,
        tx: DBWriteTransaction
    )
}
...
struct MergedRecipient {
    ...
    let oldRecipient: MunrChatRecipient?
    let newRecipient: MunrChatRecipient
}

class RecipientMergerImpl: RecipientMerger {
    private let aciSessionStore: MunrChatSessionStore
    ...
    init(
        aciSessionStore: MunrChatSessionStore,
    ...
    static func buildObservers(
        ...
        return Observers(
            preThreadMerger: [
                MunrChatServiceAddressCache,
                AuthorMergeObserver(authorMergeHelper: authorMergeHelper),
                MunrChatAccountMergeObserver(),
            ...
            postThreadMerger: [
                // The group member MergeObserver depends on `MunrChatServiceAddressCache`,
    ...
    private func _applyValidatedMergeFromStorageService(
        ...
        let aciPhoneNumberRecipient: MunrChatRecipient? = {
            guard let aci = serviceIds.aci, let phoneNumber else {
                return nil
            }
            // Explicit cast to guarantee this method doesn't return an Optional.
            return mergeAlways(aci: aci, phoneNumber: phoneNumber, isLocalRecipient: false, tx: tx) as MunrChatRecipient
        }()
        let phoneNumberPniRecipient: MunrChatRecipient? = {
            guard let phoneNumber, let pni = serviceIds.pni else {
                return nil
            }
            // Explicit cast to guarantee this method doesn't return an Optional.
            return mergeAlways(phoneNumber: phoneNumber, pni: pni, isLocalRecipient: false, tx: tx) as MunrChatRecipient
        }()
    ...
    func splitUnregisteredRecipientIfNeeded(
        ...
        mergeAndNotify(
            ...
            let splitRecipient = MunrChatRecipient.buildEmptyRecipient(unregisteredAt: NSDate.ows_millisecondTimeStamp())
    ...
    ///
    /// There may be a ``MunrChatRecipient`` for one or more of the provided
    ...
    /// the ACI of a ``MunrChatRecipient`` from one ACI to another; instead we
    /// create a new ``MunrChatRecipient``. (However, the ACI *may* change from a
    ...
    private func mergeAlways(
        ...
        // remainder of this method is to ensure there's a `MunrChatRecipient` such
        ...
        return mergeAndNotify(
            ...
            return existingRecipient ?? MunrChatRecipient(aci: aci, pni: alreadyKnownPni, phoneNumber: phoneNumber)
        }
    }
    ...
    private func _mergeHighTrust(
        aci: Aci,
        phoneNumber: E164,
        aciRecipient: MunrChatRecipient?,
        phoneNumberRecipient: MunrChatRecipient?,
        tx: DBWriteTransaction
    ) -> MunrChatRecipient? {
        if let aciRecipient {
        ...
        if let phoneNumberRecipient {
            if phoneNumberRecipient.aciString != nil {
                ...
                // a new MunrChatRecipient. We clear the phone number here since it will
                // belong to the new MunrChatRecipient.
                phoneNumberRecipient.phoneNumber = nil
                phoneNumberRecipient.pni = nil
                return nil
            }
    ...
    private func mergeAlways(
        ...
        return mergeAndNotify(
            ...
            return existingRecipient ?? MunrChatRecipient(aci: nil, pni: pni, phoneNumber: phoneNumber)
        }
    }
    ...
    private func _mergeAlways(
        ...
        pni: Pni,
        phoneNumberRecipient: MunrChatRecipient?,
        pniRecipient: MunrChatRecipient?,
        tx: DBWriteTransaction
    ) -> MunrChatRecipient? {
    ...
    private func mergeAlwaysFromStorageService(
        ...
        return mergeAndNotify(
            ...
            return existingRecipient ?? MunrChatRecipient(aci: aci, pni: pni, phoneNumber: nil)
        }
    }
    ...
    private enum SessionEvent {
        case sessionSwitchover(MunrChatRecipient, phoneNumber: String?)
        case safetyNumberChange(MunrChatRecipient, wasIdentityVerified: Bool)
    }

    private func prepareSessionEventsToInsert(
        oldRecipients: [MunrChatRecipient],
        newRecipients: [MunrChatRecipient],
        mergedRecipient: MunrChatRecipient,
        tx: DBWriteTransaction
    ) -> [SessionEvent] {
    ...
    private func prepareSessionSwitchoverEvent(
        recipientPair: MergePair<MunrChatRecipient>,
        tx: DBWriteTransaction
    ) -> SessionEvent {
    ...
    /// numbers to ACIs.
    private func sessionIdentifier(for recipient: MunrChatRecipient) -> some Equatable {
...
// MARK: - MunrChatServiceAddressCache

extension MunrChatServiceAddressCache: RecipientMergeObserver {
    func willBreakAssociation(for recipient: MunrChatRecipient, mightReplaceNonnilPhoneNumber: Bool, tx: DBWriteTransaction) {}
