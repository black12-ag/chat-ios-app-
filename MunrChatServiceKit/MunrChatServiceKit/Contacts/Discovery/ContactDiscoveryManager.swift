...
public protocol ContactDiscoveryManager {
    ...
    func lookUp(phoneNumbers: Set<String>, mode: ContactDiscoveryMode) async throws -> Set<MunrChatRecipient>
}
...
public final class ContactDiscoveryManagerImpl: ContactDiscoveryManager {
    ...
    public func lookUp(phoneNumbers: Set<String>, mode: ContactDiscoveryMode) async throws -> Set<MunrChatRecipient> {
    ...
    private func sendRequest(forPhoneNumbers phoneNumbers: Set<String>, mode: ContactDiscoveryMode) async throws -> Set<MunrChatRecipient> {
        lock.assertNotOwner()

        let fetchedPhoneNumbers = undiscoverableCache.filterPhoneNumbers(phoneNumbers, mode: mode)
        do {
            let MunrChatRecipients = try await contactDiscoveryTaskQueue.perform(for: fetchedPhoneNumbers, mode: mode)
            self.undiscoverableCache.processResults(MunrChatRecipients, requestedPhoneNumbers: fetchedPhoneNumbers)
            return MunrChatRecipients
        } catch {
    ...
    private struct UndiscoverableCache {
        ...
        func processResults(
            _ MunrChatRecipients: Set<MunrChatRecipient>,
            requestedPhoneNumbers: Set<String>
        ) {
            let now = Date()
            let missingPhoneNumbers = requestedPhoneNumbers
                .subtracting(MunrChatRecipients.lazy.compactMap { $0.phoneNumber?.stringValue })
