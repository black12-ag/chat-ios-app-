...
protocol ContactDiscoveryTaskQueue {
    func perform(for phoneNumbers: Set<String>, mode: ContactDiscoveryMode) async throws -> Set<MunrChatRecipient>
}
...
final class ContactDiscoveryTaskQueueImpl: ContactDiscoveryTaskQueue {
    ...
    func perform(for phoneNumbers: Set<String>, mode: ContactDiscoveryMode) async throws -> Set<MunrChatRecipient> {
        ...
        let discoveryResults = try await ContactDiscoveryV2Operation(
            e164sToLookup: e164s,
            mode: mode,
            udManager: ContactDiscoveryV2Operation<LibMunrChatClient.Net>.Wrappers.UDManager(db: db, udManager: udManager),
            connectionImpl: libMunrChatNet,
            remoteAttestation: ContactDiscoveryV2Operation<LibMunrChatClient.Net>.Wrappers.RemoteAttestation()
        ).perform()
    ...
    ) async throws -> Set<MunrChatRecipient> {
        var registeredRecipients = Set<MunrChatRecipient>()
