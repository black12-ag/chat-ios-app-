...
extension LibMunrChatClient.Net: ContactDiscoveryConnection {
...
extension LibMunrChatClient.CdsiLookup: ContactDiscoveryTokenResult {}
...
final class ContactDiscoveryV2Operation<ConnectionType: ContactDiscoveryConnection> {
    ...
    func perform() async throws -> [ContactDiscoveryResult] {
        do {
            ...
            let auth = LibMunrChatClient.Auth(username: cdsiAuth.username, password: cdsiAuth.password)
    ...
    private func buildRequest() throws -> ContactDiscoveryLookupRequest {
        ...
        let acisAndAccessKeys = udManager.fetchAllAciUakPairsWithSneakyTransaction().map { aci, uak in
            LibMunrChatClient.AciAndAccessKey(aci: aci, accessKey: uak.keyData)
        }
    ...
    private func handle(error: Error) async -> Error {
        switch error {
        case let libMunrChatError as LibMunrChatClient.MunrChatError:
            return await handle(libMunrChatError: libMunrChatError)
        default:
            return error
        }
    }

    private func handle(libMunrChatError: LibMunrChatClient.MunrChatError) async -> ContactDiscoveryError {
        switch libMunrChatError {
        ...
        default:
            return ContactDiscoveryError.terminalError("libMunrChat-net error: \(libMunrChatError)")
        }
    }
}
