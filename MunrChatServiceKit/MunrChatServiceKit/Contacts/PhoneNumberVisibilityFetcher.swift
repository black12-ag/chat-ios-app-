...
public protocol PhoneNumberVisibilityFetcher {
    ...
    func isPhoneNumberVisible(for recipient: MunrChatRecipient, tx: DBReadTransaction) -> Bool

    /// Pre-fetches state about visible phone numbers.
    ///
    /// This is useful during `MunrChatServiceAddressCache`'s `warmCaches` method
...
public final class PhoneNumberVisibilityFetcherImpl: PhoneNumberVisibilityFetcher {
    ...
    public func isPhoneNumberVisible(for recipient: MunrChatRecipient, tx: DBReadTransaction) -> Bool {
        return _isPhoneNumberVisible(
            ...
            isSystemContact: {
                contactsManager.fetchMunrChatAccount(forPhoneNumber: $0, transaction: SDSDB.shimOnlyBridge(tx)) != nil
            }
        )
    }
    ...
    public func fetchAll(tx: DBReadTransaction) throws -> BulkPhoneNumberVisibilityFetcher {
        return BulkPhoneNumberVisibilityFetcher(
            ...
            phoneNumbersWithSystemContacts: Set(
                try MunrChatAccountFinder().fetchPhoneNumbers(tx: SDSDB.shimOnlyBridge(tx))
            )
        )
    }
}
...
public final class BulkPhoneNumberVisibilityFetcher {
    ...
    func isPhoneNumberVisible(for recipient: MunrChatRecipient) -> Bool {
        return _isPhoneNumberVisible(
            for: recipient,
            localAci: { localAci },
