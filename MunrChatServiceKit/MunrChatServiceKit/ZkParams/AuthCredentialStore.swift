...
class AuthCredentialStore {
    ...
    ) throws -> LibMunrChatClient.CallLinkAuthCredential? {
        return try callLinkAuthCredentialStore.getData(
            Self.callLinkAuthCredentialKey(for: redemptionTime),
            transaction: tx
        ).map {
            return try LibMunrChatClient.CallLinkAuthCredential(contents: $0)
        }
    }

    func setCallLinkAuthCredential(
        _ credential: LibMunrChatClient.CallLinkAuthCredential,
        for redemptionTime: UInt64,
    ...
    func svr🐝AuthCredential(tx: DBReadTransaction) -> LibMunrChatClient.Auth? {
        guard
    ...
    func setSvr🐝AuthCredential(
        _ credential: LibMunrChatClient.Auth?,
        tx: DBWriteTransaction
    ) {
