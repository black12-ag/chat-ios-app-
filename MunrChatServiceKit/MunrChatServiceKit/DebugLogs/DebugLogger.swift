...
public final class DebugLogger {
    ...
    public static func registerLibMunrChat() {
        LibMunrChatLoggerImpl().setUpLibMunrChatLogging(level: { () -> LibMunrChatLogLevel in
            if ShouldLogVerbose() {
                return .trace
            }
            if ShouldLogDebug() {
...
private extension LibMunrChatLogLevel {
...
final class LibMunrChatLoggerImpl: LibMunrChatLogger {
    func log(level: LibMunrChatLogLevel, file: UnsafePointer<CChar>?, line: UInt32, message: UnsafePointer<CChar>) {
        Logger.log(
