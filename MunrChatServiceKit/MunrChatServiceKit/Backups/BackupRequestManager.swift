...
public protocol BackupRequestManager {
    ...
    func fetchSvr🐝AuthCredential(
        ...
        forceRefresh: Bool,
    ) async throws -> LibMunrChatClient.Auth
}
...
public class BackupRequestManagerMock: BackupRequestManager {
    ...
    public func fetchBackupServiceAuth(
        for key: MunrChatServiceKit.BackupKeyMaterial,
        localAci: LibMunrChatClient.Aci,
        auth: MunrChatServiceKit.ChatServiceAuth,
        forceRefreshUnlessCachedPaidCredential: Bool
    ) async throws -> MunrChatServiceKit.BackupServiceAuth {
        return BackupServiceAuth.mock(type: .media, backupLevel: .paid)
    }

    public func fetchBackupUploadForm(
        backupByteLength: UInt32,
        auth: MunrChatServiceKit.BackupServiceAuth
    ) async throws -> MunrChatServiceKit.Upload.Form {
    ...
    public func fetchMediaTierCdnRequestMetadata(
        cdn: Int32,
        auth: MunrChatServiceKit.BackupServiceAuth
    ) async throws -> MunrChatServiceKit.MediaTierReadCredential {
        fatalError("Unimplemented")
    }

    public func fetchBackupRequestMetadata(
        auth: MunrChatServiceKit.BackupServiceAuth
    ) async throws -> MunrChatServiceKit.BackupReadCredential {
        fatalError("Unimplemented")
    }

    public func copyToMediaTier(
        item: MunrChatServiceKit.BackupArchive.Request.MediaItem,
        auth: MunrChatServiceKit.BackupServiceAuth
    ) async throws -> UInt32 {
        fatalError("Unimplemented")
    }
    ...
    public func copyToMediaTier(
        items: [MunrChatServiceKit.BackupArchive.Request.MediaItem],
        auth: MunrChatServiceKit.BackupServiceAuth
    ) async throws -> [MunrChatServiceKit.BackupArchive.Response.BatchedBackupMediaResult] {
        fatalError("Unimplemented")
    }
    ...
    public func listMediaObjects(
        ...
        limit: UInt32?,
        auth: MunrChatServiceKit.BackupServiceAuth
    ) async throws -> MunrChatServiceKit.BackupArchive.Response.ListMediaResult {
        return listMediaResults.popFirst()!
    }

    public func deleteMediaObjects(
        objects: [MunrChatServiceKit.BackupArchive.Request.DeleteMediaTarget],
        auth: MunrChatServiceKit.BackupServiceAuth
    ) async throws {
        fatalError("Unimplemented")
    }
    ...
    public func fetchSvr🐝AuthCredential(
        key: MunrChatServiceKit.MessageRootBackupKey,
        chatServiceAuth auth: MunrChatServiceKit.ChatServiceAuth,
        forceRefresh: Bool
    ) async throws -> LibMunrChatClient.Auth {
        return LibMunrChatClient.Auth(username: "", password: "")
    }
}
