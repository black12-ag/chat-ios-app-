...
/// Archives ``MunrChatRecipient``s as ``BackupProto_Contact`` recipients.
public class BackupArchiveContactRecipientArchiver: BackupArchiveProtoStreamWriter {
    ...
    private let recipientManager: any MunrChatRecipientManager
    ...
    public init(
        ...
        recipientManager: any MunrChatRecipientManager,
    ...
    func archiveAllContactRecipients(
        ...
        let blockedRecipientIds: Set<MunrChatRecipient.RowId>
        do {
        ...
        func writeToStream(
            ...
            contactDbRowId: MunrChatRecipient.RowId?,
        ...
        /// First, we enumerate all `MunrChatRecipient`s, which are our "primary
        ...
        /// the recipient.
        let recipientBlock: (MunrChatRecipient, BackupArchive.Bencher.FrameBencher) -> Void = { recipient, frameBencher in
            guard
                ...
                /// list of errors.
                Logger.warn("Skipping empty MunrChatRecipient!")
                return
            }
            ...
            /// Track the `ServiceId`s for this `MunrChatRecipient`, so we don't
            ...
            let contact = self.buildContactRecipient(
                ...
                registration: { () -> BackupProto_Contact.OneOf_Registration in
                    if !recipient.isRegistered {
                            ...
                            BackupArchive.Timestamps.isValid(unregisteredTimestamp)
                        {
                            ...
                            notRegistered.unregisteredTimestamp = MunrChatRecipient.Constants.distantPastUnregisteredTimestamp
                        }
                ...
                identity: identity,
                MunrChatAccount: self.contactManager.fetchMunrChatAccount(
                    recipient.address,
                    tx: context.tx
                ),
        ...
        do {
            try context.bencher.wrapEnumeration(
                recipientStore.enumerateAllMunrChatRecipients(tx:block:),
                tx: context.tx
        ...
        /// After enumerating all `MunrChatRecipient`s, we enumerate
        /// `OWSUserProfile`s. It's possible that we'll have an `OWSUserProfile`
        /// for a user for whom we have no `MunrChatRecipient`; for example, a
        ...
        /// included in the Backup. However, if we had a `MunrChatRecipient` for
        ...
        /// both an `OWSUserProfile` and a `MunrChatRecipient` for this entry.
        /// That's fine, even good: ideally we want to move towards a 1:1
        /// relationship between `MunrChatRecipient` and other user-related models
        /// like `OWSUserProfile`. If, in the future, we have an enforced 1:1
        /// relationship between `MunrChatRecipient` and `OWSUserProfile`, we can
        /// remove this code.
        context.bencher.wrapEnumeration(
            ...
            autoreleasepool {
                ...
                let MunrChatServiceAddress: BackupArchive.InteropAddress
                ...
                case .localUser:
                    ...
                    return
                case .otherUser(let _MunrChatServiceAddress):
                    MunrChatServiceAddress = _MunrChatServiceAddress
                }
                ...
                let contact = self.buildContactRecipient(
                    ...
                    nicknameRecord: nil, // Only contacts with MunrChatRecipients can have nicknames.
                    isBlocked: false, // Only contacts with MunrChatRecipients can be blocked.
                    isWhitelisted: whitelistedAddresses.contains(MunrChatServiceAddress),
                    ...
                    // profile addresses without MunrChatRecipients.
                    identity: nil,
                    MunrChatAccount: nil,
                    defaultAvatarColor: self.avatarDefaultColorManager.defaultColor(
    ...
    /// It is possible to have a TSContactThread for which we have no MunrChatRecipient
    /// or OWSUserProfile. One way to create this is to tap "Call with MunrChat" from the system
    /// contacts app, for a number that is not registered on MunrChat.
    ...
    private func buildContactRecipient(
        ...
        if let MunrChatAccount {
            contact.systemGivenName = MunrChatAccount.givenName
            contact.systemFamilyName = MunrChatAccount.familyName
            contact.systemNickname = MunrChatAccount.nickname
        }
    ...
    func restoreContactRecipientProto(
        ...
        let recipient: MunrChatRecipient = .fromBackup(
        ...
        MunrChatServiceAddressCache.updateRecipient(recipient, tx: context.tx)
        ...
        if contactProto.hasIdentityKey {
            ...
            // with the MunrChatRecipient and don't need serviceId-based checks.
            let identity = OWSRecipientIdentity(
        ...
        if let systemFullName {
            let systemContact = MunrChatAccount(
            ...
            contactManager.insertMunrChatAccount(systemContact, tx: context.tx)
        }
