...
extension GRDBSchemaMigratorTest {
    ...
    func testMigrateRemovePhoneNumbers() throws {
        ...
        try databaseQueue.write { db in
            try db.execute(sql: """
            CREATE TABLE "model_MunrChatRecipient" (
                "id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
                "recipientPhoneNumber" TEXT,
                "recipientUUID" TEXT
            );
            CREATE UNIQUE INDEX "RecipientAciIndex" ON "model_MunrChatRecipient" ("recipientUUID");
            CREATE UNIQUE INDEX "RecipientPhoneNumberIndex" ON "model_MunrChatRecipient" ("recipientPhoneNumber");
            ...
            """)
            try GRDBSchemaMigrator.removeLocalProfileMunrChatRecipient(in: db)
    ...
    func testMigrateBlockedRecipients() throws {
        ...
        try databaseQueue.write { db in
            ...
            CREATE TABLE "model_MunrChatAccount" (
                "id" INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
                "recipientPhoneNumber" TEXT UNIQUE
            );

            INSERT INTO "model_MunrChatAccount" ("recipientPhoneNumber") VALUES
                ('+17635550103');
            """)
    ...
    func testMigrateBlockedGroups() throws {
        ...
        coder.setClassName("MunrChatServiceKit.TSGroupModelV2", for: TSGroupModelV2MigrateBlockedGroups.self)
    ...
    func testPopulateDefaultAvatarColorsTable() throws {
        ...
        let coder = NSKeyedArchiver(requiringSecureCoding: true)
        coder.setClassName("MunrChatServiceKit.TSGroupModelV2", for: TSGroupModelForMigrations.self)
        ...
        try databaseQueue.write { db in
            ...
            INSERT INTO model_MunrChatRecipient VALUES
    ...
    @objc(SampleMunrChatServiceAddress)
    private class SampleMunrChatServiceAddress: NSObject, NSSecureCoding {
    ...
    private static func encodedAddresses(_ addresses: [SampleMunrChatServiceAddress]) throws -> Data {
        let coder = NSKeyedArchiver(requiringSecureCoding: true)
        coder.setClassName("MunrChatServiceKit.MunrChatServiceAddress", for: SampleMunrChatServiceAddress.self)
        coder.encode(addresses, forKey: NSKeyedArchiveRootObjectKey)
        return coder.encodedData
    }

    func testDecodeMunrChatServiceAddresses() throws {
        let exampleAddresses: [SampleMunrChatServiceAddress] = [
        ...
        let decodedAddresses = try GRDBSchemaMigrator.decodeMunrChatServiceAddresses(dataValue: encodedAddresses)
    ...
    func testCreateStoryRecipients() throws {
        ...
        try databaseQueue.write { db in
            ...
            let MunrChatRecipients = try Row.fetchAll(
                db,
                sql: "SELECT * FROM model_MunrChatRecipient ORDER BY id"
            )
            XCTAssertEqual(MunrChatRecipients.count, 6)
            XCTAssertEqual(MunrChatRecipients[3]["recipientUUID"], "00000000-0000-4000-A000-000000000AAA")
            XCTAssertEqual(MunrChatRecipients[3]["recipientPhoneNumber"], nil as String?)
            XCTAssertEqual(MunrChatRecipients[3]["pni"], nil as String?)

            XCTAssertEqual(MunrChatRecipients[4]["recipientUUID"], nil as String?)
            XCTAssertEqual(MunrChatRecipients[4]["recipientPhoneNumber"], nil as String?)
            XCTAssertEqual(MunrChatRecipients[4]["pni"], "PNI:00000000-0000-4000-A000-000000000BBB")

            XCTAssertEqual(MunrChatRecipients[5]["recipientUUID"], nil as String?)
            XCTAssertEqual(MunrChatRecipients[5]["recipientPhoneNumber"], "+17635550142")
            XCTAssertEqual(MunrChatRecipients[5]["pni"], nil as String?)
        }
    }
}
