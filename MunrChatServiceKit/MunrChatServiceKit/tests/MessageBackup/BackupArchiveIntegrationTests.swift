...
class BackupArchiveIntegrationTests: XCTestCase {
    ...
    /// Describes what output to log if LibMunrChat reports a test failure.
    private enum LibMunrChatComparisonFailureLogOutput {
        /// Log the full backup JSONs returned by LibMunrChat, for external
        /// analysis via `parse-libMunrChat-comparator-failure.py`.
        case fullLibMunrChatJSON

        /// Log a minimal diff of the backup JSONs returned by LibMunrChat, for
    ...
    /// runs. Toggle to `.fullLibMunrChatJSON` if desired during local
    /// development, for more thorough inspection of the failure case.
    private let preferredFailureLogOutput: LibMunrChatComparisonFailureLogOutput = .minimalDiff
    ...
    /// by LibMunrChat. They should be equivalent; any disparity indicates that
    ...
    private func runRoundTripTest(
        testCaseFileUrl: URL,
        failureLogOutput: LibMunrChatComparisonFailureLogOutput
    ) async throws {
    ...
    private func compareViaLibMunrChat(
        ...
        if sharedTestCaseBackupString != exportedBackupString {
            ...
            case .fullLibMunrChatJSON:
                ...
                Copy the JSON lines below and run `pbpaste | parse-libMunrChat-comparator-failure.py`.
...
#else
        throw XCTSkip("LibMunrChatClient.ComparableBackup is only available in the simulator.")
#endif
    }
    ...
    private func initializeApp(dateProvider: DateProvider?) async {
        ...
        /// are bound to the MunrChatUI target.
        await MockSSKEnvironment.activate(
            ...
            testDependencies: AppSetup.TestDependencies(
                ...
                networkManager: CrashyMocks.MockNetworkManager(appReadiness: appReadiness, libMunrChatNet: nil),
...
#if targetEnvironment(simulator)
private extension LibMunrChatClient.ComparableBackup {
