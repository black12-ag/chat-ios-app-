...
class MessageDecryptionTest: SSKBaseTest {
    ...
    lazy var remoteClient: TestMunrChatClient = FakeMunrChatClient.generate(e164Identifier: remoteE164Identifier)

    private lazy var localClient = LocalMunrChatClient()
    private lazy var localPniClient = LocalMunrChatClient(identity: .pni)
    ...
    private func generateAndDecrypt(
        ...
        prepareForDecryption: (MunrChatProtocolStore, DBWriteTransaction) -> Void = { _, _ in },
        handleResult: (Result<DecryptedIncomingEnvelope, Error>, SSKProtoEnvelope) -> Void
    ) {
        write { transaction in
            let localClient: TestMunrChatClient
            let localDestinationServiceId: ServiceId
            let localProtocolStore: MunrChatProtocolStore
            ...
            var contentProto = MunrChatServiceProtos_Content()
