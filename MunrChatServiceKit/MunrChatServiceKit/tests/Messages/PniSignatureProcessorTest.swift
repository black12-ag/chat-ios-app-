...
private class MockRecipientMerger: RecipientMerger {
    func applyMergeForLocalAccount(aci: Aci, phoneNumber: E164, pni: Pni?, tx: DBWriteTransaction) -> MunrChatRecipient {
        fatalError()
    }

    func applyMergeFromStorageService(localIdentifiers: LocalIdentifiers, isPrimaryDevice: Bool, serviceIds: AtLeastOneServiceId, phoneNumber: E164?, tx: DBWriteTransaction) -> MunrChatRecipient {
        fatalError()
    }

    func applyMergeFromContactSync(localIdentifiers: LocalIdentifiers, aci: Aci, phoneNumber: E164?, tx: DBWriteTransaction) -> MunrChatRecipient {
        fatalError()
    }

    func applyMergeFromContactDiscovery(localIdentifiers: LocalIdentifiers, phoneNumber: E164, pni: Pni, aci: Aci?, tx: DBWriteTransaction) -> MunrChatRecipient? {
        fatalError()
    }

    func applyMergeFromSealedSender(localIdentifiers: LocalIdentifiers, aci: Aci, phoneNumber: E164?, tx: DBWriteTransaction) -> MunrChatRecipient {
        fatalError()
    }
    ...
    func splitUnregisteredRecipientIfNeeded(localIdentifiers: LocalIdentifiers, unregisteredRecipient: MunrChatRecipient, tx: DBWriteTransaction) {
        fatalError()
    }
}
...
final class PniSignatureProcessorTest: XCTestCase {
    ...
    private var aci: Aci!
    private var aciRecipient: MunrChatRecipient!
    private var aciIdentityKeyPair: IdentityKeyPair!

    private var pni: Pni!
    private var phoneNumber: E164!
    private var pniRecipient: MunrChatRecipient!
    ...
    override func setUp() {
        ...
        aciRecipient = MunrChatRecipient(aci: aci, pni: nil, phoneNumber: nil)
        ...
        pniRecipient = MunrChatRecipient(aci: nil, pni: pni, phoneNumber: phoneNumber)
    ...
    func testValidSignature() throws {
        let signature = pniIdentityKeyPair.MunrChatternateIdentity(aciIdentityKeyPair.identityKey)
    ...
    func testInvalidSignature() {
        let signature = IdentityKeyPair.generate().MunrChatternateIdentity(aciIdentityKeyPair.identityKey)
        XCTAssertThrowsError(
            try buildAndHandlePniSignatureMessage(from: aci, pni: pni, signature: signature),
