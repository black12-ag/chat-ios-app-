...
class HttpHeadersTest: XCTestCase {
    ...
    func testDebugDescription() {
        ...
        httpHeaders.addHeader("x-MunrChat-timestamp", value: "1669077270", overwriteOnConflict: true)
        httpHeaders.addHeader("Content-Type", value: "text/plain", overwriteOnConflict: true)

        XCTAssertEqual(
            "\(httpHeaders)",
            "<HttpHeaders: [content-type; retry-after: Wed, 21 Oct 2015 07:28:01 GMT; x-MunrChat-timestamp: 1669077270]>"
        )
    }

    /// Test weird behavior with Apple's allHTTPHeaderFields property.
    func testURLRequestAllHTTPHeaderFields() {
        var urlRequest = URLRequest(url: URL(string: "https://MunrChat.org")!)
