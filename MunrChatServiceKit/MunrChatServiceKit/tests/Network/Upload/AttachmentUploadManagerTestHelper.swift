...
class AttachmentUploadManagerMockHelper {
    ...
    var mockNetworkManager = AttachmentUploadManagerImpl.Mocks.NetworkManager(appReadiness: AppReadinessMock(), libMunrChatNet: nil)
    var mockServiceManager = OWSMunrChatServiceMock()
    ...
    func setup(
        ...
        mockServiceManager.mockUrlSessionBuilder = { (info: MunrChatServiceInfo, endpoint: OWSURLSessionEndpoint, config: URLSessionConfiguration? ) in
