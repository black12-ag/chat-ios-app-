...
private class MockStorageServiceManager: StorageServiceManager {
    ...
    func recordPendingUpdates(updatedAddresses: [MunrChatServiceAddress]) {}
...
private class TestDependencies {
    let aciSessionStore: MunrChatSessionStore
...
class RecipientMergerTest: XCTestCase {
    func testTwoWayMergeCases() {
        ...
        for (idx, testCase) in testCases.enumerated() {
            ...
            func run(transaction: DBWriteTransaction) {
                ...
                var recipientTable = [MunrChatRecipient.RowId: MunrChatRecipient]()
                ...
                for finalRecipient in testCase.finalState.reversed() {
                    let MunrChatRecipient = recipientTable.removeValue(forKey: finalRecipient.rowId)
                    XCTAssertEqual(MunrChatRecipient?.aci, finalRecipient.aci, "\(idx)")
                    XCTAssertEqual(MunrChatRecipient?.phoneNumber?.stringValue, finalRecipient.phoneNumber?.stringValue, "\(idx)")
                }
        ...
        for testCase in testCases {
            ...
            d.mockDB.write { tx in
                for initialState in testCase.initialState {
                    let recipient = MunrChatRecipient(aci: initialState.aci, pni: nil, phoneNumber: initialState.phoneNumber)
    ...
    func testAciPhoneNumberPniMerges() throws {
        ...
        for testCase in testCases {
            ...
            let mergedRecipient = d.mockDB.write { tx in
                for initialState in testCase.initialState {
                    d.recipientDatabaseTable.insertRecipient(
                        MunrChatRecipient(aci: initialState.aci, pni: initialState.pni, phoneNumber: initialState.phoneNumber),
                        transaction: tx
                    )
                }
    ...
    func testSessionSwitchoverEvents() throws {
        ...
        let phone1 = E164("+16505550101")!
        let pni1 = Pni.constantForTesting("PNI:00000000-0000-4000-8000-0000000000b1")
        let recipient1 = MunrChatRecipient(aci: nil, pni: pni1, phoneNumber: phone1)
        ...
        let recipient2 = MunrChatRecipient(aci: aci2, pni: pni2, phoneNumber: phone2)

        struct TestCase {
            let mergeRequest: (aci: Aci?, phoneNumber: E164, pni: Pni)
            let hasSession: [MunrChatRecipient]
            let needsEvent: [MunrChatRecipient]
            let lineNumber: Int

            init(
                _ mergeRequest: (aci: Aci?, phoneNumber: E164, pni: Pni),
                hasSession: [MunrChatRecipient],
                needsEvent: [MunrChatRecipient],
        ...
        for testCase in testCases {
            ...
            d.mockDB.write { tx in
                ...
                for recipient in testCase.hasSession {
                    d.aciSessionStoreKeyValueStore.setData(Data(), key: recipient.uniqueId, transaction: tx)
                    let thread = TSContactThread(contactAddress: MunrChatServiceAddress(
    ...
    func testPniSignatureMerge() throws {
        let aci = Aci.constantForTesting("00000000-0000-4000-8000-0000000000a1")
        let aciRecipient = MunrChatRecipient(aci: aci, pni: nil, phoneNumber: nil)

        let phoneNumber = E164("+16505550101")!
        let pni = Pni.constantForTesting("PNI:00000000-0000-4000-8000-0000000000b1")
        let pniRecipient = MunrChatRecipient(aci: nil, pni: pni, phoneNumber: phoneNumber)
