...
class OWSRecipientIdentityTest: SSKBaseTest {
    ...
    private func createFakeGroup() throws {
        ...
        write { tx in
            ...
            self.groupThread = try! GroupManager.createGroupForTests(
                members: recipients.map { MunrChatServiceAddress($0) },
                name: "Test Group",
                transaction: tx
            )
        }
        ...
        for recipient in recipients {
            write { tx in
                _ = identityManager.setVerificationState(
                    ...
                    for: MunrChatServiceAddress(recipient),
    ...
    func testSomeNoLongerVerified() throws {
        ...
        read { transaction in
            ...
            XCTAssertEqual(Set(noLongerVerifiedIdentityKeys.keys), Set(deverifiedAcis.map { MunrChatServiceAddress($0) }))
        }
    }
