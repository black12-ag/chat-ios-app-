...
final class ContactDiscoveryManagerTest: XCTestCase {
    private class MockContactDiscoveryTaskQueue: ContactDiscoveryTaskQueue {
        var onPerform: ((Set<String>, ContactDiscoveryMode) async throws -> Set<MunrChatRecipient>)?
        ...
        static func foundResponse(for phoneNumbers: Set<String>) -> Set<MunrChatRecipient> {
            Set(phoneNumbers.lazy.map {
                MunrChatRecipient(aci: Aci.randomForTesting(), pni: Pni.randomForTesting(), phoneNumber: E164($0)!, deviceIds: [DeviceId(validating: 1)!])
            })
        }
    }
    ...
    func testQueueing() async throws {
        ...
        let initialRequest = CancellableContinuation<CheckedContinuation<Set<MunrChatRecipient>, any Error>>()
