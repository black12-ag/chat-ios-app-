...
class BlockingManagerTests: SSKBaseTest {
    ...
    func testAddBlockedAddress() {
        ...
        SSKEnvironment.shared.databaseStorageRef.read { tx in
            ...
            XCTAssertTrue(blockingManager.isAddressBlocked(MunrChatServiceAddress(aci), transaction: tx))
    ...
    func testRemoveBlockedAddress() {
        ...
        // Test
        SSKEnvironment.shared.databaseStorageRef.write { tx in
            blockingManager.removeBlockedAddress(MunrChatServiceAddress(unblockedAci), wasLocallyInitiated: true, transaction: tx)
        }

        // Verify
        SSKEnvironment.shared.databaseStorageRef.read { tx in
            XCTAssertTrue(blockingManager.isAddressBlocked(MunrChatServiceAddress(blockedAci), transaction: tx))
            XCTAssertFalse(blockingManager.isAddressBlocked(MunrChatServiceAddress(unblockedAci), transaction: tx))
    ...
    func testIncomingSyncMessage() throws {
        ...
        try SSKEnvironment.shared.databaseStorageRef.write { tx in
            blockingManager.addBlockedAddress(
                MunrChatServiceAddress(noLongerBlockedAci),
                blockMode: .localShouldNotLeaveGroups,
                transaction: tx
            )
            blockingManager.addBlockedAddress(
                MunrChatServiceAddress(noLongerBlockedPhoneNumber),
                blockMode: .localShouldNotLeaveGroups,
                transaction: tx
            )
            ...
            blockingManager.addBlockedAddress(
                MunrChatServiceAddress(stillBlockedAci),
                blockMode: .localShouldNotLeaveGroups,
                transaction: tx
            )
            blockingManager.addBlockedAddress(
                MunrChatServiceAddress(stillBlockedPhoneNumber),
                blockMode: .localShouldNotLeaveGroups,
                transaction: tx
            )
            TSGroupThread.forUnitTest(
        ...
        try SSKEnvironment.shared.databaseStorageRef.read { readTx in
            ...
            XCTAssertFalse(blockingManager.isAddressBlocked(MunrChatServiceAddress(noLongerBlockedAci), transaction: readTx))
            XCTAssertFalse(blockingManager.isAddressBlocked(MunrChatServiceAddress(noLongerBlockedPhoneNumber), transaction: readTx))
            ...
            XCTAssertTrue(blockingManager.isAddressBlocked(MunrChatServiceAddress(stillBlockedAci), transaction: readTx))
            XCTAssertTrue(blockingManager.isAddressBlocked(MunrChatServiceAddress(stillBlockedPhoneNumber), transaction: readTx))
            ...
            XCTAssertTrue(blockingManager.isAddressBlocked(MunrChatServiceAddress(newlyBlockedAci), transaction: readTx))
            XCTAssertTrue(blockingManager.isAddressBlocked(MunrChatServiceAddress(newlyBlockedPhoneNumber), transaction: readTx))
            ...
            let expectedBlockedAddresses = [
                MunrChatServiceAddress(stillBlockedAci),
                MunrChatServiceAddress(stillBlockedPhoneNumber),
                MunrChatServiceAddress(newlyBlockedAci),
                MunrChatServiceAddress(newlyBlockedPhoneNumber),
            ]
