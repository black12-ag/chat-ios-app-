...
class StoryManagerTest: SSKBaseTest {
    ...
    func testProcessIncomingStoryMessage_createsPrivateStoryWithWhitelistedAuthor() throws {
        ...
        profileManager.fakeUserProfiles = [
            MunrChatServiceAddress(author): OWSUserProfile(address: .otherUser(MunrChatServiceAddress(author))),
        ]

        try write {
            SSKEnvironment.shared.profileManagerRef.addUser(
                toProfileWhitelist: MunrChatServiceAddress(author),
    ...
    func testProcessIncomingStoryMessage_storeAuthorProfileKey() throws {
        ...
        try write {
            ...
            let profileKey = profileManager.userProfile(
                for: MunrChatServiceAddress(author),
                tx: $0
            )?.profileKey
    ...
    class StoryManager: MunrChatServiceKit.StoryManager {
