...
class ProfileManagerTest: XCTestCase {
    func testNormalizeRecipientInProfileWhitelist() {
        ...
        func normalizeRecipient(_ recipient: MunrChatRecipient, tx: DBWriteTransaction) {
        ...
        db.write { tx in
            normalizeRecipient(MunrChatRecipient(aci: aci, pni: pni, phoneNumber: phoneNumber), tx: tx)
        ...
        db.write { tx in
            ...
            normalizeRecipient(MunrChatRecipient(aci: nil, pni: pni, phoneNumber: phoneNumber), tx: tx)
            XCTAssertFalse(serviceIdStore.hasValue(aci.serviceIdUppercaseString, transaction: tx))
