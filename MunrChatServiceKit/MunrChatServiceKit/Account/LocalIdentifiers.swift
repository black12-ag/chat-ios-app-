...
public final class LocalIdentifiers {
    ...
    public func contains(address: MunrChatServiceAddress) -> Bool {
        ...
        // in production because we populate `MunrChatServiceAddressCache` with our
        // own identifiers.)
    ...
    public func isAciAddressEqualToAddress(_ address: MunrChatServiceAddress) -> Bool {
        if let serviceId = address.serviceId {
...
public extension LocalIdentifiers {
    var aciAddress: MunrChatServiceAddress {
        MunrChatServiceAddress(serviceId: aci, phoneNumber: phoneNumber)
    }
}
