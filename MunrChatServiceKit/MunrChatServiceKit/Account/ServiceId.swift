...
extension Pni {
    ...
    public static func parseFrom(ambiguousString: String?) -> Pni? {
        ...
        // Give LibMunrChat the first pass at parsing a "PNI:"-prefixed value.
...
extension ProtocolAddress {
    ...
    public var deviceIdObj: DeviceId {
        // LibMunrChat also enforces [1, 127], so this can't fail.
...
/// `MunrChatRecipient`, but existing types may store a ServiceId/E164 pair.
///
...
public struct NormalizedDatabaseRecordAddress {
    ...
    public init?(address: MunrChatServiceAddress?) {
