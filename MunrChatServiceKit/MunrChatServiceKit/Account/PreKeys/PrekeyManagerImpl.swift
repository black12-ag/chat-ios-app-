...
public class PreKeyManagerImpl: PreKeyManager {
    ...
    public enum Constants {
        ...
        /// `MAX_UNACKNOWLEDGED_SESSION_AGE` (from LibMunrChatClient) value currently
    ...
    private func needsSignedPreKeyRotation(identity: OWSIdentity, tx: DBReadTransaction) -> Bool {
        let store = protocolStoreManager.MunrChatProtocolStore(for: identity).signedPreKeyStore
    ...
    private func needsLastResortPreKeyRotation(identity: OWSIdentity, tx: DBReadTransaction) -> Bool {
        let store = protocolStoreManager.MunrChatProtocolStore(for: identity).kyberPreKeyStore
