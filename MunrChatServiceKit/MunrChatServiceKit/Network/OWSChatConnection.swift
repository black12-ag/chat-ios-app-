...
public class OWSChatConnection {
    ...
    public init(
        ...
        self.serialQueue = DispatchQueue(label: "org.MunrChat.chat-connection-\(type)")
        self.type = type
...
internal class OWSChatConnectionUsingLibMunrChat<Connection: ChatConnection & Sendable>: OWSChatConnection, ConnectionEventsListener {
    fileprivate let libMunrChatNet: Net
    ...
    internal init(
        ...
        self.libMunrChatNet = libMunrChatNet
    ...
    fileprivate override func appDidBecomeReady() {
        ...
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(MunrChatProxyConfigDidChange),
            name: .MunrChatProxyConfigDidChange,
            object: nil,
        )
    }
    ...
    @objc
    private func MunrChatProxyConfigDidChange(_ notification: NSNotification) {
        // The libMunrChat connection needs to be recreated whether the proxy is going up,
    ...
    fileprivate override func ensureWebsocketExists() {
        ...
        connection = .connecting(token: token, task: Task { [token] () -> Connection? in
            ...
            } catch MunrChatError.appExpired(_) {
                await appExpiry.setHasAppExpiredAtCurrentVersion(db: db)
            } catch MunrChatError.deviceDeregistered(_) {
    ...
    fileprivate override func makeRequestInternal(_ request: TSRequest, requestId: UInt64) async throws -> any HTTPResponse {
        ...
        let libMunrChatRequest = ChatConnection.Request(method: httpMethod, pathAndQuery: "/\(requestUrl.relativeString)", headers: httpHeaders.headers, body: body, timeout: request.timeoutInterval)
        ...
        do {
            guard let chatService else {
                throw MunrChatError.chatServiceInactive("no connection to chat server")
            }

            connectionInfo = chatService.info()
            response = try await chatService.send(libMunrChatRequest)
        } catch {
            switch error {
            case MunrChatError.connectionTimeoutError(_), MunrChatError.requestTimeoutError(_):
            ...
            case MunrChatError.webSocketError(_), MunrChatError.connectionFailed(_), MunrChatError.chatServiceInactive(_):
                throw OWSHTTPError.networkFailure(.genericFailure)
            case MunrChatError.connectionInvalidated(_):
    ...
    func connectionWasInterrupted(_ service: Connection, error: Error?) {
        self.serialQueue.async { [self] in
            ...
            if let error {
                ...
                owsFailDebug("\(logPrefix) libMunrChat disconnected us without being asked")
            }
...
internal class OWSUnauthConnectionUsingLibMunrChat: OWSChatConnectionUsingLibMunrChat<UnauthenticatedChatConnection> {
    init(libMunrChatNet: Net, appExpiry: AppExpiry, appReadiness: AppReadiness, db: any DB) {
        super.init(libMunrChatNet: libMunrChatNet, type: .unidentified, appExpiry: appExpiry, appReadiness: appReadiness, db: db)
    }
    ...
    override func connectChatService(token: NSObject) async throws -> UnauthenticatedChatConnection {
        return try await libMunrChatNet.connectUnauthenticatedChat(languages: Array(HttpHeaders.topPreferredLanguages()))
    }
}

internal class OWSAuthConnectionUsingLibMunrChat: OWSChatConnectionUsingLibMunrChat<AuthenticatedChatConnection>, ChatConnectionListener {
    ...
    init(
        ...
        super.init(libMunrChatNet: libMunrChatNet, type: .identified, appExpiry: appExpiry, appReadiness: appReadiness, db: db)
    }
    ...
    fileprivate override func connectChatService(token: NSObject) async throws -> AuthenticatedChatConnection {
        ...
        do {
            return try await libMunrChatNet.connectAuthenticatedChat(
                username: username ?? "",
                password: password ?? "",
            ...
            switch error {
            case MunrChatError.deviceDeregistered(_):
                serialQueue.async {
    ...
    ///
    /// This is on top of the websocket pings libMunrChat already uses to keep connections alive.
            ...
            while true {
                do {
                    ...
                    _ = try await chat.send(request)
                        ...
                        MunrChatError.chatServiceInactive(_) {
                    // No action necessary, we're done with this service.
                    return
                } catch MunrChatError.rateLimitedError(retryAfter: let delay, message: _) {
                    ...
                    // but continue to rely on libMunrChat reporting disconnects via delegate callback.
