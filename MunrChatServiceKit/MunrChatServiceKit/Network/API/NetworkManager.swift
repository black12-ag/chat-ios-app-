...
public class NetworkManager: NetworkManagerProtocol {
    ...
    public let libMunrChatNet: Net?

    public init(appReadiness: AppReadiness, libMunrChatNet: Net?) {
        self.appReadiness = appReadiness
        self.libMunrChatNet = libMunrChatNet
        if let libMunrChatNet {
                ...
                for await notification in NotificationCenter.default.notifications(named: .reachabilityChanged) {
                    ...
                    do {
                        if !MunrChatProxy.isEnabled {
                            Self.resetLibMunrChatNetProxySettings(libMunrChatNet, appReadiness: appReadiness)
                        }
                        try libMunrChatNet.networkDidChange()
                    } catch {
                        owsFailDebug("error notify libMunrChat of network change: \(error)")
                    }
                }
            }

            self.resetLibMunrChatNetProxySettings()
    ...
    func resetLibMunrChatNetProxySettings() {
        guard let libMunrChatNet else {
            // In tests without a libMunrChat Net instance, no action is needed.
            return
        }
        Self.resetLibMunrChatNetProxySettings(libMunrChatNet, appReadiness: appReadiness)
    }

    private static func resetLibMunrChatNetProxySettings(_ libMunrChatNet: Net, appReadiness: AppReadiness) {
        if let systemProxy = ProxyConfig.fromCFNetwork() {
            Logger.info("System '\(systemProxy.scheme)' proxy detected")
            do {
                try libMunrChatNet.setProxy(scheme: systemProxy.scheme, host: systemProxy.host, port: systemProxy.port, username: systemProxy.username, password: systemProxy.password)
                return
            } catch {
    ...
    static func fromCFNetwork() -> Self? {
        ...
        for proxyConfig in proxies {
            ...
            case kCFProxyTypeNone:
                ...
                // But libMunrChat only supports one global proxy setting,
            ...
            case kCFProxyTypeSOCKS:
                // iOS doesn't distinguish between SOCKS4 and SOCKS5. Defer to libMunrChat's default.
                return ProxyConfig(
                    scheme: "socks",
                    host: proxyConfig[kCFProxyHostNameKey] as! String,
                    port: proxyConfig[kCFProxyPortNumberKey] as! UInt16?,
