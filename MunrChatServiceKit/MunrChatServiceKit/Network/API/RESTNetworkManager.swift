...
private class RESTSessionManager {
    ...
    init() {
        urlSession = SSKEnvironment.shared.MunrChatServiceRef.urlSessionForMainMunrChatService()
    }

    public func performRequest(_ request: TSRequest) async throws -> any HTTPResponse {
        // We should only use the RESTSessionManager for requests to the MunrChat main service.
...
private class OWSSessionManagerPool {
    ...
    init() {
        ...
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(OWSSessionManagerPool.isMunrChatProxyReadyDidChange),
            name: Notification.Name.isMunrChatProxyReadyDidChange,
            object: nil)
    }
