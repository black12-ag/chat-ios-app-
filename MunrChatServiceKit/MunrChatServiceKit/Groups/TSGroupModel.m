...
const NSUInteger kMaxAvatarSize = (kMaxEncryptedAvatarSize
    /* The length of the padding. See GroupSecretParams:encrypt_blob_with_padding in LibMunrChat. */
    - sizeof(uint32_t)
    ...
    - 0
    /* Encryption tag & nonce. See GroupSecretParams:encrypt_blob in LibMunrChat. */
    - 16
    - 12
    /* Reserved byte. See GroupSecretParams:encrypt_blob in LibMunrChat. */
    - 1);
...
- (instancetype)initWithGroupId:(NSData *)groupId
                     ...
                     avatarData:(nullable NSData *)avatarData
                        members:(NSArray<MunrChatServiceAddress *> *)members
                 addedByAddress:(nullable MunrChatServiceAddress *)addedByAddress
{
    self = [super init];
    if (!self) {
        return self;
...
#pragma clang diagnostic pop
    ...
    if (_groupModelSchemaVersion < 1) {
        ...
        if (memberE164s) {
            NSMutableArray<MunrChatServiceAddress *> *memberAddresses = [NSMutableArray new];
            for (NSString *phoneNumber in memberE164s) {
                [memberAddresses addObject:[MunrChatServiceAddress legacyAddressWithServiceIdString:nil
