...
public class GroupMembership: MTLModel {
    ...
    fileprivate typealias MemberStateMap = [MunrChatServiceAddress: GroupMemberState]
    fileprivate typealias InvalidInviteMap = [Data: InvalidInviteModel]

    private typealias LegacyMemberStateMap = [MunrChatServiceAddress: LegacyMemberState]
    ...
    @objc
    init(v1Members: [MunrChatServiceAddress]) {
        var builder = Builder()
    ...
    convenience init(membersForTest: [MunrChatServiceAddress]) {
    ...
    public func role(for serviceId: ServiceId) -> TSGroupMemberRole? {
        return role(for: MunrChatServiceAddress(serviceId))
    }

    public func role(for address: MunrChatServiceAddress) -> TSGroupMemberRole? {
        guard let memberState = memberStates[address] else {
            return nil
        }
    ...
    public func isFullOrInvitedAdministrator(_ address: MunrChatServiceAddress) -> Bool {
        guard let memberState = memberStates[address] else {
            return false
        }
        switch memberState {
    ...
    public func isFullOrInvitedAdministrator(_ serviceId: ServiceId) -> Bool {
        return isFullOrInvitedAdministrator(MunrChatServiceAddress(serviceId))
    }

    public func isFullMemberAndAdministrator(_ address: MunrChatServiceAddress) -> Bool {
        guard let memberState = memberStates[address] else {
            return false
        }
    ...
    public func isFullMemberAndAdministrator(_ serviceId: ServiceId) -> Bool {
        return isFullMemberAndAdministrator(MunrChatServiceAddress(serviceId))
    }

    @objc
    public func isFullMember(_ address: MunrChatServiceAddress) -> Bool {
        guard let memberState = memberStates[address] else {
            return false
        }
    ...
    public func isFullMember(_ serviceId: ServiceId) -> Bool {
        return isFullMember(MunrChatServiceAddress(serviceId))
    }

    public func isInvitedMember(_ address: MunrChatServiceAddress) -> Bool {
        guard let memberState = memberStates[address] else {
            return false
        }
    ...
    public func isInvitedMember(_ serviceId: ServiceId) -> Bool {
        return isInvitedMember(MunrChatServiceAddress(serviceId))
    }

    public func isRequestingMember(_ address: MunrChatServiceAddress) -> Bool {
        guard let memberState = memberStates[address] else {
            return false
        }
    ...
    public func isRequestingMember(_ serviceId: ServiceId) -> Bool {
        return isRequestingMember(MunrChatServiceAddress(serviceId))
    }

    public func isMemberOfAnyKind(_ address: MunrChatServiceAddress) -> Bool {
        return memberStates[address] != nil
    }

    public func isMemberOfAnyKind(_ serviceId: ServiceId) -> Bool {
        return isMemberOfAnyKind(MunrChatServiceAddress(serviceId))
    }
    ...
    public func addedByAci(forInvitedMember address: MunrChatServiceAddress) -> Aci? {
        guard let memberState = memberStates[address] else {
            return nil
        }
        switch memberState {
    ...
    public func addedByAci(forInvitedMember serviceId: ServiceId) -> Aci? {
        return addedByAci(forInvitedMember: MunrChatServiceAddress(serviceId))
    }

    /// This method should only be called for full members.
    public func didJoinFromInviteLink(forFullMember address: MunrChatServiceAddress) -> Bool {
        guard let memberState = memberStates[address] else {
    ...
    /// this method should only be called for full members.
    public func didJoinFromAcceptedJoinRequest(forFullMember address: MunrChatServiceAddress) -> Bool {
        guard let memberState = memberStates[address] else {
            owsFailDebug("Missing member: \(address)")
            return false
        }
    ...
    public func hasProfileKeyInGroup(serviceId: ServiceId) -> Bool {
        guard let memberState = memberStates[MunrChatServiceAddress(serviceId)] else {
            return false
        }
    ...
    public struct Builder {
        ...
        public mutating func remove(_ serviceId: ServiceId) {
            remove(MunrChatServiceAddress(serviceId))
        }

        public mutating func remove(_ address: MunrChatServiceAddress) {
            remove([address])
        }

        public mutating func remove(_ addresses: Set<MunrChatServiceAddress>) {
        ...
        public mutating func addFullMembers(
            _ addresses: Set<MunrChatServiceAddress>,
            role: TSGroupMemberRole,
            didJoinFromInviteLink: Bool = false,
            didJoinFromAcceptedJoinRequest: Bool = false
        ) {
        ...
        public mutating func addInvitedMember(_ serviceId: ServiceId, role: TSGroupMemberRole, addedByAci: Aci) {
            addInvitedMember(MunrChatServiceAddress(serviceId), role: role, addedByAci: addedByAci)
        }
        ...
        public mutating func addRequestingMember(_ aci: Aci) {
            addRequestingMember(MunrChatServiceAddress(aci))
        }

        public mutating func addRequestingMember(_ address: MunrChatServiceAddress) {
            addRequestingMembers([address])
        }

        public mutating func addRequestingMembers(_ addresses: Set<MunrChatServiceAddress>) {
        ...
        public func hasMemberOfAnyKind(_ address: MunrChatServiceAddress) -> Bool {
        ...
        public func build() -> GroupMembership {
            owsAssertDebug(Set(bannedMembers.keys.lazy.map { MunrChatServiceAddress($0) })
...
@objc(_TtCC16MunrChatServiceKit15GroupMembership11MemberState)
