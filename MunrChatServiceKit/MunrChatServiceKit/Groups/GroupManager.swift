...
public class GroupManager: NSObject {
    ...
    public static func doesUserSupportGroupsV2(address: MunrChatServiceAddress) -> Bool {
    ...
    public static func localCreateNewGroup(
        seed: NewGroupSeed,
        members membersParam: [MunrChatServiceAddress],
    ...
    public static func createGroupForTests(
        members: [MunrChatServiceAddress],
            ...
            let remoteContactName: String? = {
                    ...
                    changeAuthor != localIdentifiers.aci
                {
                    return SSKEnvironment.shared.contactManagerRef.displayName(
                        for: MunrChatServiceAddress(changeAuthor),
                        tx: transaction
                    ).resolvedValue()
                }
    ...
    public static func isPossibleGroupInviteLink(_ url: URL) -> Bool {
        ...
        if url.scheme == "https" {
            possibleHosts = ["MunrChat.group"]
        } else if url.scheme == "sgnl" {
            possibleHosts = ["MunrChat.group", "joingroup"]
        } else {
            return false
        }
        ...
        do {
                ...
                !newGroupModel.membership.hasProfileKeyInGroup(serviceId: localIdentifiers.aci)
            {
                ...
                for member in oldMembers {
                    let memberAddress = MunrChatServiceAddress(member)
    ...
    private static func autoWhitelistGroupIfNecessary(
        ...
        case .aci(let aci):
            shouldAddToWhitelist = SSKEnvironment.shared.profileManagerRef.isUser(inProfileWhitelist: MunrChatServiceAddress(aci), transaction: tx)
        case .localUser:
