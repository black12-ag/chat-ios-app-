...
public final class OWSUserProfile: NSObject, NSCopying, SDSCodableModel, Decodable {
    ...
    public enum Address: Hashable {
        case localUser
        case otherUser(MunrChatServiceAddress)
    }
    ...
    public var internalAddress: Address {
        if phoneNumber == Constants.localProfilePhoneNumber {
            ...
            return .otherUser(MunrChatServiceAddress.legacyAddress(serviceIdString: serviceIdString, phoneNumber: phoneNumber))
        }
    }
    ...
    public func publicAddress(localIdentifiers: LocalIdentifiers) -> MunrChatServiceAddress {
    ...
    public static func internalAddress(for publicAddress: MunrChatServiceAddress, localIdentifiers: LocalIdentifiers) -> Address {
    ...
    private static func publicAddress(for internalAddress: Address, localIdentifiers: LocalIdentifiers) -> MunrChatServiceAddress {
    ...
    public class func getOrBuildUserProfile(
        ...
        case .otherUser(let serviceId):
            address = .otherUser(MunrChatServiceAddress(serviceId))
        case .legacyUserPhoneNumberFromBackupRestore(let phoneNumber):
            address = .otherUser(MunrChatServiceAddress(phoneNumber: phoneNumber.stringValue))
        }
    ...
    ///
    /// We should only have one UserProfile for each MunrChatRecipient. However,
...
extension OWSUserProfile {
    ...
    private func displayNameBeforeLearningProfileNameIfNecessary(
        ...
        let MunrChatServiceAddress: MunrChatServiceAddress
        switch internalAddress {
        case .localUser:
            return nil
        case .otherUser(let _address):
            MunrChatServiceAddress = _address
        }
        ...
        if
            let aci = MunrChatServiceAddress.serviceId as? Aci,
            let username = usernameLookupManager.fetchUsername(forAci: aci, transaction: tx)
        {
            ...
            let serviceId = MunrChatServiceAddress.serviceId,
            ...
            let phoneNumber = recipient.phoneNumber
        {
            ...
            /// exclusively in the MunrChat ecosystem (i.e., not including names
