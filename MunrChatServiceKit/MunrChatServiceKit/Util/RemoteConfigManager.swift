...
private enum IsEnabledFlag: String, FlagType {
    ...
    case libMunrChatEnforceMinTlsVersion = "ios.libMunrChatEnforceMinTlsVersion"
    ...
    var isHotSwappable: Bool {
        ...
        case .libMunrChatEnforceMinTlsVersion: true // cached during launch, so not hot-swapped in practice
...
public class RemoteConfigManagerImpl: RemoteConfigManager {
    ...
    private func _refresh() async throws {
        ...
        let serverEpochTimeMs = headers["x-MunrChat-timestamp"].flatMap(UInt64.init(_:))
        owsAssertDebug(serverEpochTimeMs != nil, "Must have X-MunrChat-Timestamp.")
        ...
        let libMunrChatEnforceMinTlsVersion = mergedConfig.isEnabled(.libMunrChatEnforceMinTlsVersion, defaultValue: FeatureFlags.libMunrChatEnforceMinTlsVersion)

        LibMunrChatUserDefaults.saveShouldEnforceMinTlsVersion(libMunrChatEnforceMinTlsVersion, in: CurrentAppContext().appUserDefaults())
...
enum LibMunrChatUserDefaults {

    private static var shouldEnforceMinTlsVersionKey: String = "LibMunrChatEnforceMinTlsVersion"
    ...
    /// add the arguments "-UseLibMunrChatForUnidentifiedWebsocket YES" to the invocation of the app.
