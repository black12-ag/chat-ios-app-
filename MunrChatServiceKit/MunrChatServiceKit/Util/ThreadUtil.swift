...
extension TSThread {
    ...
    public enum IntentContext {
        case senderAddress(MunrChatServiceAddress)
    ...
    private func _generateSendMessageIntent(
        ...
        let senderAddress: MunrChatServiceAddress
        let message: TSMessage?
        let recipientAddress: MunrChatServiceAddress?
        switch context {
    ...
    func generateIncomingCallIntent(
        ...
        if self.isGroupThread {
            ...
            return self.generateSendMessageIntent(
                context: .senderAddress(MunrChatServiceAddress(callerAci)),
                transaction: tx
            )
        } else {
            ...
            return ResolvableDisplayNameBuilder(
                displayNameForAddress: MunrChatServiceAddress(callerAci),
                transformedBy: { displayName, tx in
                    let caller = Self.buildPerson(address: MunrChatServiceAddress(callerAci), displayName: displayName, tx: tx)
    ...
    private static func intentRecipientAvatarImage(recipient: MunrChatServiceAddress, transaction: DBReadTransaction) -> INImage? {
        // Generate avatar
        let image: INImage
