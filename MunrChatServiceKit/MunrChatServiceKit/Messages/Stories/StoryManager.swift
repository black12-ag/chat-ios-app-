...
public class StoryManager {
    ...
    public class func processIncomingStoryMessage(
        ...
        guard !SSKEnvironment.shared.blockingManagerRef.isAddressBlocked(MunrChatServiceAddress(author), transaction: transaction) else {
        ...
        if let masterKey = storyMessage.group?.masterKey {
            ...
            guard SSKEnvironment.shared.profileManagerRef.isUser(inProfileWhitelist: MunrChatServiceAddress(author), transaction: transaction) else {
        ...
        case .authorAci(let authorAci):
            ...
            _ = TSContactThread.getOrCreateThread(withContactAddress: MunrChatServiceAddress(authorAci), transaction: transaction)
...
extension StoryManager {
    ...
    public static func buildStoryHeaders() -> HttpHeaders {
        ["X-MunrChat-Receive-Stories": areStoriesEnabled ? "true" : "false"]
    }
    ...
    func threadUniqueId(transaction: DBReadTransaction) -> String? {
        ...
        case .authorAci(let authorAci):
            return TSContactThread.getWithContactAddress(
                MunrChatServiceAddress(authorAci),
                transaction: transaction
            )?.uniqueId
