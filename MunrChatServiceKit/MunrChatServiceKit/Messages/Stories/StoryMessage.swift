...
public final class StoryMessage: NSObject, SDSCodableModel, Decodable {
    ...
    @objc
    public var authorAddress: MunrChatServiceAddress { MunrChatServiceAddress(authorAci) }
    ...
    public static func create(
        ...
        if let groupId = groupId, SSKEnvironment.shared.blockingManagerRef.isGroupIdBlocked(groupId, transaction: transaction) {
            ...
            if SSKEnvironment.shared.blockingManagerRef.isAddressBlocked(MunrChatServiceAddress(author), transaction: transaction) {
                Logger.warn("Ignoring StoryMessage from blocked author.")
                return nil
            }
            if DependenciesBridge.shared.recipientHidingManager.isHiddenAddress(MunrChatServiceAddress(author), tx: transaction) {
    ...
    // The "MunrChat account" used for e.g. the onboarding story has a fixed UUID
    ...
    public func updateRecipientStatesWithOutgoingMessageStates(
        _ outgoingMessageStates: [MunrChatServiceAddress: TSOutgoingMessageRecipientState]?,
        transaction: DBWriteTransaction
    ) {
    ...
    public func threads(transaction: DBReadTransaction) -> [TSThread] {
        ...
        case .incoming:
            if let groupId = groupId, let groupThread = TSGroupThread.fetch(groupId: groupId, transaction: transaction) {
                return [groupThread]
            } else if let contactThread = TSContactThread.getWithContactAddress(MunrChatServiceAddress(authorAci), transaction: transaction) {
    ...
    public func failedRecipientAddresses(errorCode: Int) -> [MunrChatServiceAddress] {
        ...
        }.map { MunrChatServiceAddress($0.key) }
    }
    ...
    public func resendMessageToFailedRecipients(transaction: DBWriteTransaction) {
        ...
        for (serviceId, state) in recipientStates {
            ...
            messages.forEach { $0.updateWithSkippedRecipients([MunrChatServiceAddress(serviceId)], tx: transaction) }
        }
