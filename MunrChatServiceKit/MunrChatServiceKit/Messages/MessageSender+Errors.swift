...
extension Error {
    public var isFatalError: Bool {
        ...
        case is UntrustedIdentityError:
            return true
        case is MunrChatServiceRateLimitedError:
...
public class MessageSenderNoSuchMunrChatRecipientError: CustomNSError, IsRetryableProvider, UserErrorDescriptionProvider {
    // NSError bridging: the domain of the error.
    public static let errorDomain = OWSError.errorDomain

    // NSError bridging: the error code within the given domain.
    public var errorCode: Int { OWSErrorCode.noSuchMunrChatRecipient.rawValue }
    ...
    public class func isNoSuchMunrChatRecipientError(_ error: Error?) -> Bool {
        error is MessageSenderNoSuchMunrChatRecipientError
    }
...
public class UntrustedIdentityError: CustomNSError, IsRetryableProvider, UserErrorDescriptionProvider {
    ...
    public var localizedDescription: String {
        ...
        return String(format: format, SSKEnvironment.shared.databaseStorageRef.read { tx in
            return SSKEnvironment.shared.contactManagerRef.displayName(for: MunrChatServiceAddress(serviceId), tx: tx).resolvedValue()
        })
    }
...
class MunrChatServiceRateLimitedError: CustomNSError, IsRetryableProvider, UserErrorDescriptionProvider {
    // NSError bridging: the domain of the error.
    public static let errorDomain = OWSError.errorDomain
    ...
    // NSError bridging: the error code within the given domain.
    public var errorCode: Int { OWSErrorCode.MunrChatServiceRateLimited.rawValue }
