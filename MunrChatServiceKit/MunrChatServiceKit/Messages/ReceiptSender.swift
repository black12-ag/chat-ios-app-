    ...
    private func sendReceipts(
        ...
        repeat {
            let sendResult = await SSKEnvironment.shared.databaseStorageRef.awaitableWrite { tx -> (sendPromise: Promise<Void>, batchEndIndex: Int)? in
                ...
                if SSKEnvironment.shared.blockingManagerRef.isAddressBlocked(MunrChatServiceAddress(aci), transaction: tx) {
                    Logger.warn("Dropping receipts for blocked \(aci)")
                    return (.value(()), remainingTimestamps.endIndex)
                }

                let recipientHidingManager = DependenciesBridge.shared.recipientHidingManager
                if recipientHidingManager.isHiddenAddress(MunrChatServiceAddress(aci), tx: tx) {
                ...
                guard identityManager.untrustedIdentityForSending(to: MunrChatServiceAddress(aci), untrustedThreshold: nil, tx: tx) == nil else {
                ...
                let thread = TSContactThread.getOrCreateThread(withContactAddress: MunrChatServiceAddress(aci), transaction: tx)
            ...
            } catch let error as MessageSenderNoSuchMunrChatRecipientError {
