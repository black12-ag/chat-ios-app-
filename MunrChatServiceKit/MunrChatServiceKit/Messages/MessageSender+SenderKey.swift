...
extension MessageSender {
    ...
    func prepareSenderKeyMessageSend(
        ...
        var eligibleRecipients = Set(recipients.filter { serviceId in
            ...
            // TODO: Remove this & handle MunrChatError.invalidRegistrationId.
    ...
    private func sendSenderKeyCiphertext(
        ...
        return await SSKEnvironment.shared.databaseStorageRef.awaitableWrite { tx in
            let failedRecipients = sendResult.unregisteredServiceIds.map { serviceId in
                ...
                return (serviceId, MessageSenderNoSuchMunrChatRecipientError())
            }
    ...
    private func senderKeyMessageBody(
        ...
        if let groupThread = thread as? TSGroupThread {
            ...
            // TODO: Eventually LibMunrChatClient could allow passing `nil` in this case
        ...
        let secretCipher = try SMKSecretSessionCipher(
            sessionStore: MunrChatProtocolStoreManager.MunrChatProtocolStore(for: .aci).sessionStore,
            preKeyStore: MunrChatProtocolStoreManager.MunrChatProtocolStore(for: .aci).preKeyStore,
            signedPreKeyStore: MunrChatProtocolStoreManager.MunrChatProtocolStore(for: .aci).signedPreKeyStore,
            kyberPreKeyStore: MunrChatProtocolStoreManager.MunrChatProtocolStore(for: .aci).kyberPreKeyStore,
            identityStore: identityManager.libMunrChatStore(for: .aci, tx: writeTx),
        ...
        let ciphertext = try secretCipher.groupEncryptMessage(
            ...
            contentHint: message.contentHint.MunrChatClientHint,
...
private extension MessageSender {
    ...
    enum RegistrationIdStatus {
        ...
        case valid
        /// LibMunrChatClient expects registrationIds to fit in 15 bits for multiRecipientEncrypt,
