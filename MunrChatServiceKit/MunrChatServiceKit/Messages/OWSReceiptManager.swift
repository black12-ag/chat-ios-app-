...
struct ReceiptForLinkedDevice: Codable {
    ...
    init(senderAddress: MunrChatServiceAddress, messageUniqueId: String, messageIdTimestamp: UInt64, timestamp: UInt64) {
...
extension OWSReceiptManager {
    ...
    private func enqueueLinkedDeviceViewedReceipt(
        messageAuthorAddress: MunrChatServiceAddress,
        ...
        return processReceiptsForMessages(sentAt: sentTimestamps, tx: tx) { _, messages in
            if !messages.isEmpty {
                for message in messages {
                    message.update(
                        withDeliveredRecipient: MunrChatServiceAddress(recipientServiceId),
    ...
    func processReadReceipts(
        ...
        return processReceiptsForMessages(sentAt: sentTimestamps, tx: tx) { _, messages in
            if !messages.isEmpty {
                ...
                for message in messages {
                    message.update(
                        withReadRecipient: MunrChatServiceAddress(recipientAci),
                        deviceId: recipientDeviceId,
                        readTimestamp: readTimestamp,
                        tx: tx
                    )
        ...
        return processReceiptsForMessages(sentAt: sentTimestamps, tx: tx) { sentTimestamp, messages in
            if !messages.isEmpty {
                if self.areReadReceiptsEnabled() {
                    for message in messages {
                        message.update(
                            withViewedRecipient: MunrChatServiceAddress(recipientAci),
                            deviceId: recipientDeviceId,
