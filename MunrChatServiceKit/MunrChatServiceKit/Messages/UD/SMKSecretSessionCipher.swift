...
public class SMKSecretSessionCipher: NSObject {
    ...
    private let currentSenderKeyStore: LibMunrChatClient.SenderKeyStore

    // public SecretSessionCipher(MunrChatProtocolStore MunrChatProtocolStore) {
    public init(
        ...
        senderKeyStore: LibMunrChatClient.SenderKeyStore
    ) {
    ...
    public func encryptMessage(
        ...
        let ciphertextMessage = try MunrChatEncrypt(
    ...
    // public Pair<MunrChatProtocolAddress, byte[]> decrypt(CertificateValidator validator, byte[] ciphertext, long timestamp)
    ...
    public func decryptMessage(
        ...
        do {
            ...
            // return new Pair<>(new MunrChatProtocolAddress(content.getSenderCertificate().getSender(),
    ...
    private func decrypt(messageContent: UnidentifiedSenderMessageContent, context: StoreContext) throws -> Data {
        ...
        // MunrChatProtocolAddress sender = new MunrChatProtocolAddress(message.getSenderCertificate().getSender(),
        ...
        // case CiphertextMessage.munrchat_TYPE: return new SessionCipher(MunrChatProtocolStore, sender).decrypt(new
        // MunrChatMessage(message.getContent())); case CiphertextMessage.PREKEY_TYPE: return new
        // SessionCipher(MunrChatProtocolStore, sender).decrypt(new PreKeyMunrChatMessage(message.getContent())); default: throw
        // new InvalidMessageException("Unknown type: " + message.getType());
        // }
        let plaintextData: Data
        switch messageContent.messageType {
        case .munrchat:
            let cipherMessage = try MunrChatMessage(bytes: messageContent.contents)
            plaintextData = try MunrChatDecrypt(
                ...
                context: context)
        case .preKey:
            let cipherMessage = try PreKeyMunrChatMessage(bytes: messageContent.contents)
            plaintextData = try MunrChatDecryptPreKey(
                message: cipherMessage,
                from: ProtocolAddress(from: sender),
                sessionStore: currentSessionStore,
                identityStore: currentIdentityStore,
