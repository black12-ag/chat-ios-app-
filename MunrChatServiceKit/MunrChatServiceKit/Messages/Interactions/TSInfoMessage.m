...
- (nullable instancetype)initWithCoder:(NSCoder *)coder
{
    ...
    if (self.infoMessageSchemaVersion < 2) {
        ...
        if (phoneNumber) {
            _unregisteredAddress = [MunrChatServiceAddress legacyAddressWithServiceIdString:nil phoneNumber:phoneNumber];
        }
    }
...
- (NSString *)infoMessagePreviewTextWithTransaction:(DBReadTransaction *)transaction
{
    switch (_messageType) {
        ...
        case TSInfoMessageUserNotRegistered:
            if (self.unregisteredAddress.isValid) {
                ...
                return [NSString stringWithFormat:OWSLocalizedString(@"ERROR_UNREGISTERED_USER_FORMAT",
                                                      ...
                                                      @"unregistered user's name or MunrChat id}}."),
                    recipientName];
            } else {
        ...
        case TSInfoMessageUnknownProtocolVersion:
            break;
        case TSInfoMessageUserJoinedMunrChat: {
            MunrChatServiceAddress *address = [TSContactThread contactAddressFromThreadId:self.uniqueThreadId
                                                                            transaction:transaction];
            NSString *recipientName =
                [SSKEnvironment.shared.contactManagerObjcRef displayNameStringForAddress:address
                                                                             transaction:transaction];
            NSString *format = OWSLocalizedString(@"INFO_MESSAGE_USER_JOINED_MunrChat_BODY_FORMAT",
                @"Shown in inbox and conversation when a user joins MunrChat, embeds the new user's {{contact "
                @"name}}");
        ...
        case TSInfoMessagePhoneNumberChange: {
            ...
            MunrChatServiceAddress *address = [[MunrChatServiceAddress alloc] initWithServiceIdObjC:aci];
