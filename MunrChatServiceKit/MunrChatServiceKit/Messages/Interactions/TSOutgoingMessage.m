                                   ...
                                   transaction:(DBReadTransaction *)transaction
{
    ...
    NSMutableSet<MunrChatServiceAddress *> *recipientAddresses = [NSMutableSet new];
    if ([self isKindOfClass:[OWSOutgoingSyncMessage class]]) {
        // Sync messages should only be sent to linked devices.
        MunrChatServiceAddress *localAddress = [TSAccountManagerObjcBridge localAciAddressWith:transaction];
        ...
        if (explicitRecipients.count > 0) {
            NSMutableSet<MunrChatServiceAddress *> *explicitRecipientAddresses = [[NSMutableSet alloc] init];
            for (AciObjC *recipientAci in explicitRecipients) {
                [explicitRecipientAddresses
                    addObject:[[MunrChatServiceAddress alloc] initWithServiceIdObjC:recipientAci]];
            }
        ...
        if (additionalRecipients.count > 0) {
            for (ServiceIdObjC *serviceId in additionalRecipients) {
                [recipientAddresses addObject:[[MunrChatServiceAddress alloc] initWithServiceIdObjC:serviceId]];
            }
        }
    }

    NSSet<ServiceIdObjC *> *skippedRecipientsSet = [NSSet setWithArray:skippedRecipients];
    NSMutableDictionary<MunrChatServiceAddress *, TSOutgoingMessageRecipientState *> *recipientAddressStates =
        [NSMutableDictionary new];
    for (MunrChatServiceAddress *recipientAddress in recipientAddresses) {
