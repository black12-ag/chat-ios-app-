...
extension TSOutgoingMessage {
    @objc
    open func recipientState(for address: MunrChatServiceAddress) -> TSOutgoingMessageRecipientState? {
        return recipientAddressStates?[address]
    }

    /// All recipients of this message.
    @objc
    public func recipientAddresses() -> [MunrChatServiceAddress] {
    ...
    @objc
    public func sendingRecipientAddresses() -> [MunrChatServiceAddress] {
    ...
    @objc
    public func sentRecipientAddresses() -> [MunrChatServiceAddress] {
        return filterRecipientAddresses { state in
            switch state.status {
    ...
    /// - Note: we only learn "read" status if read receipts are enabled.
    @objc
    public func deliveredRecipientAddresses() -> [MunrChatServiceAddress] {
        return filterRecipientAddresses { state in
            switch state.status {
    ...
    /// - Note: we only learn "read" status if read receipts are enabled.
    @objc
    open func readRecipientAddresses() -> [MunrChatServiceAddress] {
        return filterRecipientAddresses { state in
            switch state.status {
    ...
    /// viewed.
    @objc
    public func viewedRecipientAddresses() -> [MunrChatServiceAddress] {
        return filterRecipientAddresses { state in
            switch state.status {
    ...
    @objc
    public func failedRecipientAddresses(errorCode: Int) -> [MunrChatServiceAddress] {
        return filterRecipientAddresses { state in
...
public extension TSOutgoingMessage {
    ...
    func updateWithRecipientAddressStates(
        _ recipientAddressStates: [MunrChatServiceAddress: TSOutgoingMessageRecipientState]?,
    ...
    func updateRecipientsFromNonLocalDevice(
        _ nonLocalRecipientStates: [MunrChatServiceAddress: TSOutgoingMessageRecipientState],
        isSentUpdate: Bool,
        transaction tx: DBWriteTransaction
    ) {
        anyUpdateOutgoingMessage(transaction: tx) { outgoingMessage in
    ...
    func updateWithFailedRecipients(
        ...
        self.anyUpdateOutgoingMessage(transaction: tx) {
            for (serviceId, error) in recipientErrors {
                guard let recipientState = $0.recipientAddressStates?[MunrChatServiceAddress(serviceId)] else {
...
public extension TSOutgoingMessage {
    ...
    func buildPniSignatureMessageIfNeeded(transaction tx: DBReadTransaction) -> SSKProtoPniSignatureMessage? {
        ...
        let signature = pniIdentityKeyPair.identityKeyPair.MunrChatternateIdentity(
    ...
    fileprivate func maybeClearShouldSharePhoneNumber(
        for recipientAddress: MunrChatServiceAddress,
...
extension TSOutgoingMessage {
    public func update(
        withDeliveredRecipient recipientAddress: MunrChatServiceAddress,
        deviceId: DeviceId,
    ...
    public func update(
        withReadRecipient recipientAddress: MunrChatServiceAddress,
        deviceId: DeviceId,
    ...
    public func update(
        withViewedRecipient recipientAddress: MunrChatServiceAddress,
        deviceId: DeviceId,
    ...
    private func handleReceipt(
        from recipientAddress: MunrChatServiceAddress,
        deviceId: DeviceId,
...
extension TSOutgoingMessage {
    ...
    /// Used in MessageSender to MunrChat how a message should be encrypted before sending
    /// Currently only overridden by OWSOutgoingResendRequest (this is asserted in the MessageSender implementation)
    @objc
    var encryptionStyle: EncryptionStyle { .munrchat }

    func clearMessageSendLogEntry(forRecipient address: MunrChatServiceAddress, deviceId: DeviceId, tx: DBWriteTransaction) {
