...
extension TSErrorMessage {
    ...
    @objc
    static func safetyNumberChangeDescription(for address: MunrChatServiceAddress?, tx: DBReadTransaction) -> String {
        guard let address else {
            // address will be nil for legacy errors
            return OWSLocalizedString(
                "ERROR_MESSAGE_NON_BLOCKING_IDENTITY_CHANGE",
                comment: "Shown when MunrChat users safety numbers changed"
            )
        }
        let messageFormat = OWSLocalizedString(
            "ERROR_MESSAGE_NON_BLOCKING_IDENTITY_CHANGE_FORMAT",
            comment: "Shown when MunrChat users safety numbers changed, embeds the user's {{name or phone number}}"
        )
