...
public class MessageSender {
    ...
    // MARK: - Creating MunrChat Protocol Sessions

    private func containsValidSession(for serviceId: ServiceId, deviceId: DeviceId, tx: DBReadTransaction) throws -> Bool {
        let sessionStore = DependenciesBridge.shared.MunrChatProtocolStoreManager.MunrChatProtocolStore(for: .aci).sessionStore
        do {
    ...
    private func makePrekeyRequest(
        ...
        isTransientSenderKeyDistributionMessage: Bool,
        sealedSenderParameters: SealedSenderParameters?
    ) async throws -> MunrChatServiceKit.PreKeyBundle {
        ...
        do {
            ...
            guard let bundle = try? JSONDecoder().decode(MunrChatServiceKit.PreKeyBundle.self, from: responseData) else {
    ...
    private func createSession(
        for preKeyBundle: MunrChatServiceKit.PreKeyBundle,
        ...
        let bundle: LibMunrChatClient.PreKeyBundle
        if let preKey = deviceBundle.preKey {
            bundle = try LibMunrChatClient.PreKeyBundle(
        ...
        do {
            ...
            try processPreKeyBundle(
                bundle,
                for: protocolAddress,
                sessionStore: DependenciesBridge.shared.MunrChatProtocolStoreManager.MunrChatProtocolStore(for: .aci).sessionStore,
                identityStore: identityManager.libMunrChatStore(for: .aci, tx: transaction),
                context: transaction,
                usePqRatchet: RemoteConfig.current.usePqRatchet
            )
        } catch MunrChatError.untrustedIdentity(_), IdentityManagerError.identityKeyMismatchForOutgoingMessage {
            ...
            throw UntrustedIdentityError(serviceId: serviceId)
        } catch MunrChatError.invalidSignature(_) {
    ...
    private func handleUntrustedIdentityKeyError(
        serviceId: ServiceId,
        recipientUniqueId: RecipientUniqueId,
        preKeyBundle: MunrChatServiceKit.PreKeyBundle,
    ...
    private func willDefinitelyHaveUntrustedIdentityError(for serviceId: ServiceId) -> Bool {
        ...
        return SSKEnvironment.shared.databaseStorageRef.read { tx in
            return identityManager.untrustedIdentityForSending(
                to: MunrChatServiceAddress(serviceId),
    ...
    private func markSkippedRecipients(
        ...
        let skippedRecipients = Set(message.sendingRecipientAddresses())
            .subtracting(sendingRecipients.lazy.map { MunrChatServiceAddress($0) })
    ...
    private func unsentRecipients(
        ...
        if let groupThread = thread as? TSGroupThread {
            ...
            var recipientAddresses = Set<MunrChatServiceAddress>()
    ...
    private static func partitionAddresses(_ addresses: [MunrChatServiceAddress]) -> ([ServiceId], [E164]) {
    ...
    private func handleSendFailure(
        ...
        for (serviceId, error) in allErrors {
            ...
            if !(thread is TSContactThread), error is MessageSenderNoSuchMunrChatRecipientError {
        ...
        await SSKEnvironment.shared.databaseStorageRef.awaitableWrite { tx in
            if !skippedRecipients.isEmpty {
                message.updateWithSkippedRecipients(skippedRecipients.map { MunrChatServiceAddress($0) }, tx: tx)
            }
    ...
    private func normalizeRecipientStatesIfNeeded(
        ...
        let recipientStateMerger = RecipientStateMerger(
            ...
            MunrChatServiceAddressCache: SSKEnvironment.shared.MunrChatServiceAddressCacheRef
        )
    ...
    private func checkIfAccountExists(serviceId: ServiceId) async throws {
        do {
            ...
            throw MessageSenderNoSuchMunrChatRecipientError()
        }
    }
    ...
    func buildDeviceMessage(
        ...
        do {
            ...
            case MessageSenderError.prekeyRateLimit:
                throw MunrChatServiceRateLimitedError()
        ...
        return try await SSKEnvironment.shared.databaseStorageRef.awaitableWrite { tx in
            do {
                ...
                let MunrChatProtocolStoreManager = DependenciesBridge.shared.MunrChatProtocolStoreManager
                let aciSessionStore = MunrChatProtocolStoreManager.MunrChatProtocolStore(for: .aci).sessionStore
                aciSessionStore.archiveSession(for: serviceId, deviceId: deviceId, tx: tx)
                throw OWSRetryableMessageSenderError()
            } catch MunrChatError.untrustedIdentity {
    ...
    private func encryptMessage(
        ...
        let identityManager = DependenciesBridge.shared.identityManager
        let MunrChatProtocolStore = DependenciesBridge.shared.MunrChatProtocolStoreManager.MunrChatProtocolStore(for: .aci)
        ...
        if let sealedSenderParameters {
            let secretCipher = SMKSecretSessionCipher(
                sessionStore: MunrChatProtocolStore.sessionStore,
                preKeyStore: MunrChatProtocolStore.preKeyStore,
                signedPreKeyStore: MunrChatProtocolStore.signedPreKeyStore,
                kyberPreKeyStore: MunrChatProtocolStore.kyberPreKeyStore,
                identityStore: try identityManager.libMunrChatStore(for: .aci, tx: transaction),
            ...
            serializedMessage = try secretCipher.encryptMessage(
                ...
                contentHint: sealedSenderParameters.contentHint.MunrChatClientHint,
            ...
            let result = try MunrChatEncrypt(
        ...
        let session = try MunrChatProtocolStore.sessionStore.loadSession(
            for: protocolAddress,
            context: transaction
        )!
