...
public class EarlyMessageManager {
    ...
    private enum EarlyReceipt: Codable, Hashable {
        ...
        init(from decoder: Decoder) throws {
            ...
            let sender = try container.decodeIfPresent(MunrChatServiceAddress.self, forKey: .sender)
        ...
        case outgoingMessageRead(sender: MunrChatServiceAddress, deviceId: UInt32, timestamp: UInt64)
        case outgoingMessageDelivered(sender: MunrChatServiceAddress, deviceId: UInt32, timestamp: UInt64)
        case outgoingMessageViewed(sender: MunrChatServiceAddress, deviceId: UInt32, timestamp: UInt64)
        ...
        init(receiptType: SSKProtoReceiptMessageType, sender: ServiceId, senderDeviceId: DeviceId, timestamp: UInt64) {
            switch receiptType {
            case .delivery: self = .outgoingMessageDelivered(sender: MunrChatServiceAddress(sender), deviceId: senderDeviceId.uint32Value, timestamp: timestamp)
            case .read: self = .outgoingMessageRead(sender: MunrChatServiceAddress(sender), deviceId: senderDeviceId.uint32Value, timestamp: timestamp)
            case .viewed: self = .outgoingMessageViewed(sender: MunrChatServiceAddress(sender), deviceId: senderDeviceId.uint32Value, timestamp: timestamp)
            }
        }
    }
