...
public enum SealedSenderContentHint: Int, Codable, CustomStringConvertible {
    ...
    init(_ MunrChatClientHint: UnidentifiedSenderMessageContent.ContentHint) {
        switch MunrChatClientHint {
        case .default: self = .default
        case .resendable: self = .resendable
        case .implicit: self = .implicit
        default:
            owsFailDebug("Unspecified case \(MunrChatClientHint)")
            self = .default
        }
    }

    public var MunrChatClientHint: UnidentifiedSenderMessageContent.ContentHint {
        switch self {
...
public final class MessageReceiver {
    ...
    public func processEnvelope(
        ...
        do {
            ...
            case .unidentifiedSender, .identifiedSender:
                ...
                if SSKEnvironment.shared.blockingManagerRef.isAddressBlocked(MunrChatServiceAddress(sourceAci), transaction: tx) {
                    return
                }
            ...
            if let dataMessage = sent.message {
                ...
                if dataMessage.hasProfileKey {
                    if let groupId {
                        ...
                        let destinationAddress = MunrChatServiceAddress.legacyAddress(
    ...
    private func preprocessDataMessage(
        ...
        guard let groupContext = dataMessage.groupV2 else {
            let contactAddress = MunrChatServiceAddress(envelope.sourceAci)
    ...
    private func processFlaglessDataMessage(
        ...
        guard !dataMessage.hasRequiredProtocolVersion || dataMessage.requiredProtocolVersion <= SSKProtos.currentProtocolVersion else {
            ...
            OWSUnknownProtocolVersionMessage(
                ...
                sender: MunrChatServiceAddress(envelope.sourceAci),
        ...
        message.insertOrReplacePlaceholder(from: MunrChatServiceAddress(envelope.sourceAci), transaction: tx)
    ...
    private func handleIncomingEnvelope(
        ...
        if let groupId = typingMessage.groupID {
            ...
            let sourceAddress = MunrChatServiceAddress(envelope.sourceAci)
    ...
    private func handleIncomingEnvelope(
        ...
        do {
            ...
            if let ratchetKey = errorMessage.ratchetKey {
                ...
                SSKEnvironment.shared.senderKeyStoreRef.logSKDMInfo(for: MunrChatServiceAddress(sourceAci), transaction: tx)
                didPerformSessionReset = false
            }
    ...
    func checkForUnknownLinkedDevice(in envelope: DecryptedIncomingEnvelope, tx: DBWriteTransaction) {
        ...
        // Check if the MunrChatRecipient (used for sending messages) knows about
        // this device.
        ...
        if !recipient.deviceIds.contains(deviceId) {
            ...
            Logger.info("Message received from unknown linked device; adding to local MunrChatRecipient: \(deviceId).")
    ...
    private func handleIncomingEndSessionEnvelope(
        ...
        let thread = TSContactThread.getOrCreateThread(
            withContactAddress: MunrChatServiceAddress(decryptedEnvelope.sourceAci),
            transaction: tx
        )
