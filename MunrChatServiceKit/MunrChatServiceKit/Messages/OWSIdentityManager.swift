...
public protocol OWSIdentityManager {
    func libMunrChatStore(for identity: OWSIdentity, tx: DBReadTransaction) throws -> IdentityStore
    ...
    func recipientIdentity(for address: MunrChatServiceAddress, tx: DBReadTransaction) -> OWSRecipientIdentity?
    ...
    func identityKey(for address: MunrChatServiceAddress, tx: DBReadTransaction) -> Data?
    ...
    func insertSessionSwitchoverEvent(for recipient: MunrChatRecipient, phoneNumber: String?, tx: DBWriteTransaction)
    ...
    func untrustedIdentityForSending(
        to address: MunrChatServiceAddress,
    ...
    func verificationState(for address: MunrChatServiceAddress, tx: DBReadTransaction) -> VerificationState
...
public class IdentityStore: IdentityKeyStore {
    ...
    public func identity(for address: ProtocolAddress, context: StoreContext) throws -> LibMunrChatClient.IdentityKey? {
...
public class OWSIdentityManagerImpl: OWSIdentityManager {
    ...
    public func libMunrChatStore(for identity: OWSIdentity, tx: DBReadTransaction) throws -> IdentityStore {
    ...
    public func recipientIdentity(for address: MunrChatServiceAddress, tx: DBReadTransaction) -> OWSRecipientIdentity? {
    ...
    public func identityKey(for address: MunrChatServiceAddress, tx: DBReadTransaction) -> Data? {
    ...
    public func insertIdentityChangeInfoMessage(
        ...
        let contactThread = TSContactThread.getOrCreateThread(
            withContactAddress: MunrChatServiceAddress(serviceId),
            transaction: tx
        )
        let contactThreadMessage: TSErrorMessage = .nonblockingIdentityChange(
            thread: contactThread,
            address: MunrChatServiceAddress(serviceId),
            wasIdentityVerified: wasIdentityVerified
        )
        contactThreadMessage.anyInsert(transaction: tx)

        for groupThread in TSGroupThread.groupThreads(with: MunrChatServiceAddress(serviceId), transaction: tx) {
    ...
    public func mergeRecipient(_ recipient: MunrChatRecipient, into targetRecipient: MunrChatRecipient, tx: DBWriteTransaction) {
    ...
    private func buildVerificationStateSyncMessage(
        ...
        let value: Any? = queuedVerificationStateSyncMessagesKeyValueStore.getObject(
            key,
            ofClasses: [NSNumber.self, NSString.self, MunrChatServiceAddress.self],
            transaction: tx
        )
        ...
        case is MunrChatServiceAddress:
            recipientUniqueId = key
        case let stringValue as NSString:
            // Previously, we stored phone numbers in this KV store.
            let address = MunrChatServiceAddress.legacyAddress(serviceId: nil, phoneNumber: stringValue as String)
    ...
    private func sendVerificationStateSyncMessage(for recipientUniqueId: RecipientUniqueId, message: OWSVerificationStateSyncMessage) {
        ...
        nullMessagePromise.done(on: DispatchQueue.global()) {
            ...
            if error is MessageSenderNoSuchMunrChatRecipientError {
    ...
    public func verificationState(for address: MunrChatServiceAddress, tx: DBReadTransaction) -> VerificationState {
        ...
        default:
            if isUserInitiatedChange {
                ...
                case .verified:
                    ...
                    // to the profile whitelist so they become a "MunrChat
...
class OWSIdentityManagerObjCBridge: NSObject {
    ...
    @objc
    static func identityKey(forAddress address: MunrChatServiceAddress) -> Data? {
