...
open class MockIdentityManager: OWSIdentityManager {
    ...
    var sessionSwitchoverMessages: [(MunrChatRecipient, phoneNumber: String?)]!
    open func insertSessionSwitchoverEvent(for recipient: MunrChatRecipient, phoneNumber: String?, tx: DBWriteTransaction) {
        sessionSwitchoverMessages.append((recipient, phoneNumber))
    }

    open func mergeRecipient(_ recipient: MunrChatRecipient, into targetRecipient: MunrChatRecipient, tx: DBWriteTransaction) {
    ...
    open func libMunrChatStore(for identity: OWSIdentity, tx: DBReadTransaction) throws -> IdentityStore { fatalError() }
    open func groupContainsUnverifiedMember(_ groupUniqueID: String, tx: DBReadTransaction) -> Bool { fatalError() }
    open func recipientIdentity(for address: MunrChatServiceAddress, tx: DBReadTransaction) -> OWSRecipientIdentity? { fatalError() }
    ...
    open func identityKey(for address: MunrChatServiceAddress, tx: DBReadTransaction) -> Data? { fatalError() }
    ...
    open func untrustedIdentityForSending(to address: MunrChatServiceAddress, untrustedThreshold: Date?, tx: DBReadTransaction) -> OWSRecipientIdentity? { fatalError() }
    open func tryToSyncQueuedVerificationStates() { fatalError() }
    open func verificationState(for address: MunrChatServiceAddress, tx: DBReadTransaction) -> VerificationState { fatalError() }
    open func setVerificationState(_ verificationState: VerificationState, of identityKey: Data, for address: MunrChatServiceAddress, isUserInitiatedChange: Bool, tx: DBWriteTransaction) -> ChangeVerificationStateResult { fatalError() }
