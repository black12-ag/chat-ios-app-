...
public class GroupMessageProcessorManager {
    ...
    public static func discardMode(
        ...
        let isBlocked: Bool = (
            blockingManager.isAddressBlocked(MunrChatServiceAddress(sourceAci), transaction: tx)
        ...
        if shouldCheckGroupModel {
            ...
            guard groupModel.groupMembership.isFullMember(MunrChatServiceAddress(sourceAci)) else {
            ...
            if groupModel.isAnnouncementsOnly {
                guard groupModel.groupMembership.isFullMemberAndAdministrator(MunrChatServiceAddress(sourceAci)) else {
