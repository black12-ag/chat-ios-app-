...
public class OWSMessageDecrypter {
    ...
    private func trySendReactiveProfileKey(to sourceAci: Aci, tx transaction: DBWriteTransaction) {
        ...
        let contactThread = TSContactThread.getOrCreateThread(
            withContactAddress: MunrChatServiceAddress(sourceAci),
            transaction: transaction
        )
    ...
    private func processError(
        ...
        if case MunrChatError.duplicatedMessage(_) = error {
        ...
        let sourceAddress = MunrChatServiceAddress(sourceAci)
        ...
        default: // another MunrChatError, or another kind of Error altogether
            break
        }
    ...
    private func buildResendRequestDecryptionError(
        ...
        guard [.munrchat, .senderKey, .preKey, .plaintext].contains(unsealedEnvelope.cipherType) else {
            return nil
        }
    ...
    func decryptIdentifiedEnvelope(
        ...
        do {
            ...
            let MunrChatProtocolStore = DependenciesBridge.shared.MunrChatProtocolStoreManager.MunrChatProtocolStore(for: validatedEnvelope.localIdentity)

            let plaintext: Data
            switch cipherType {
            case .munrchat:
                let message = try MunrChatMessage(bytes: encryptedData)
                plaintext = try MunrChatDecrypt(
                    message: message,
                    from: protocolAddress,
                    sessionStore: MunrChatProtocolStore.sessionStore,
                    identityStore: identityManager.libMunrChatStore(for: localIdentity, tx: transaction),
                    context: transaction
                )
            ...
            case .preKey:
                ...
                let message = try PreKeyMunrChatMessage(bytes: encryptedData)
                plaintext = try MunrChatDecryptPreKey(
                    message: message,
                    from: protocolAddress,
                    sessionStore: MunrChatProtocolStore.sessionStore,
                    identityStore: identityManager.libMunrChatStore(for: localIdentity, tx: transaction),
    ...
    private func sendReactiveProfileKeyIfNecessary(to sourceAci: Aci, tx transaction: DBWriteTransaction) {
        ...
        if SSKEnvironment.shared.blockingManagerRef.isAddressBlocked(MunrChatServiceAddress(sourceAci), transaction: transaction) {
            ...
            Task {
                ...
                let needsReactiveProfileKeyMessage: Bool = db.read { transaction in
                    ...
                    if profileManager.isUser(
                        inProfileWhitelist: MunrChatServiceAddress(sourceAci),
                        transaction: transaction
                    ) {
                        return true
                    }
                    ...
                    TSGroupThread.enumerateGroupThreads(
                        with: MunrChatServiceAddress(sourceAci),
                        transaction: transaction
                    ) { thread, stop in
    ...
    func decryptUnidentifiedSenderEnvelope(
        ...
        let identityManager = DependenciesBridge.shared.identityManager
        let MunrChatProtocolStore = DependenciesBridge.shared.MunrChatProtocolStoreManager.MunrChatProtocolStore(for: localIdentity)
    ...
    private func processDecryptedEnvelope(
        ...
        localIdentifiers: LocalIdentifiers,
        mergeRecipient: (DBWriteTransaction) -> MunrChatRecipient,
    ...
    private static func descriptionForEnvelopeType(_ envelope: SSKProtoEnvelope) -> String {
        ...
        case .unknown:
            ...
            return "Unknown"
        case .ciphertext:
            return "MunrChatEncryptedMessage"
        case .prekeyBundle:
