...
public protocol RecipientHidingManager {
    ...
    func hiddenRecipients(tx: DBReadTransaction) -> Set<MunrChatRecipient>

    /// Fetch the hidden-recipient state for the given `MunrChatRecipient`, if the
    /// `MunrChatRecipient` is currently hidden.
    func fetchHiddenRecipient(
        MunrChatRecipient: MunrChatRecipient,
    ...
    /// Inserts hidden-recipient state for the given `MunrChatRecipient`.
    ///
    ...
    ///
    /// - Parameter recipient: A ``MunrChatRecipient``.
...
public extension RecipientHidingManager {
    ...
    /// Whether the given `MunrChatRecipient` is currently hidden.
    func isHiddenRecipient(
        _ recipient: MunrChatRecipient,
        tx: DBReadTransaction
    ) -> Bool {
        return fetchHiddenRecipient(MunrChatRecipient: recipient, tx: tx) != nil
    }
}

// MARK: - Record

/// A database record denoting a hidden ``MunrChatRecipient`` by their row ID.
...
public struct HiddenRecipient: Codable, FetchableRecord, PersistableRecord {
    ...
    public enum CodingKeys: String, CodingKey {
        case MunrChatRecipientRowId = "recipientId"
        case inKnownMessageRequestState
    }

    /// The hidden recipient's ``MunrChatRecipient.id``.
    let MunrChatRecipientRowId: Int64
...
public final class RecipientHidingManagerImpl: RecipientHidingManager {
    ...
    public func hiddenRecipients(tx: DBReadTransaction) -> Set<MunrChatRecipient> {
        do {
            let sql = """
                SELECT \(MunrChatRecipient.databaseTableName).*
                FROM \(MunrChatRecipient.databaseTableName)
                INNER JOIN \(HiddenRecipient.databaseTableName)
                    AS hiddenRecipient
                    ON hiddenRecipient.recipientId = \(MunrChatRecipientColumn: .id)
            """
            return Set(
                try MunrChatRecipient.fetchAll(tx.database, sql: sql)
            )
        } catch {
    ...
    public func fetchHiddenRecipient(
        ...
        guard let MunrChatRecipientRowId = MunrChatRecipient.id else {
            return nil
        }
        ...
        do {
            return try HiddenRecipient.fetchOne(db, key: MunrChatRecipientRowId)
        } catch {
    ...
    public func addHiddenRecipient(
        ...
        guard let MunrChatRecipientRowId = recipient.id else {
            throw RecipientHidingError.recipientIdNotFound
        }

        let record = HiddenRecipient(
            MunrChatRecipientRowId: MunrChatRecipientRowId,
    ...
    public func removeHiddenRecipient(
        ...
        if let id = recipient.id, isHiddenRecipient(recipient, tx: tx) {
            ...
            let sql = """
                ...
                WHERE \(HiddenRecipient.CodingKeys.MunrChatRecipientRowId.stringValue) = ?
            """
...
private extension RecipientHidingManagerImpl {
    ...
    /// to the database.
    ///
    /// - Parameter recipient: The ``MunrChatRecipient`` who was just hidden.
    /// - Parameter wasLocallyInitiated: Whether the user initiated
    ///   the hide on this device (true) or a linked device (false).
    /// - Parameter tx: The transaction to use for database operations.
    func didSetAsHidden(
        recipient: MunrChatRecipient,
        wasLocallyInitiated: Bool,
        tx: DBWriteTransaction
    ) {
    ...
    /// from the database.
    ///
    /// - Parameter recipient: The ``MunrChatRecipient`` who was just unhidden.
    /// - Parameter wasLocallyInitiated: Whether the user initiated
    ///   the hide on this device (true) or a linked device (false).
    /// - Parameter tx: The transaction to use for database operations.
    ///
    /// Note: If a ``MunrChatRecipient`` is deleted, a cascade
    /// rule is in place that will also delete the corresponding
    /// `HiddenRecipient` entry. This method does not get hit in
    /// that case.
    func didSetAsUnhidden(recipient: MunrChatRecipient, wasLocallyInitiated: Bool, tx: DBWriteTransaction) {
        // Triggers UI updates of recipient lists.
        NotificationCenter.default.postOnMainThread(name: Self.hideListDidChange, object: nil)
...
public enum RecipientHidingError: Error, CustomStringConvertible {
    ...
    case invalidRecipientAddress(MunrChatServiceAddress)
...
public class RecipientHidingManagerObjcBridge: NSObject {
    ...
    @objc
    public static func isHiddenAddress(_ address: MunrChatServiceAddress, tx: DBReadTransaction) -> Bool {
