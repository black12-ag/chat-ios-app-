...
public class BlockingManager {
    ...
    public func isAddressBlocked(_ address: MunrChatServiceAddress, transaction: DBReadTransaction) -> Bool {
    ...
    public func blockedAddresses(transaction: DBReadTransaction) -> Set<MunrChatServiceAddress> {
        let recipientDatabaseTable = DependenciesBridge.shared.recipientDatabaseTable
    ...
    public func addBlockedAci(_ aci: Aci, blockMode: BlockMode, tx: DBWriteTransaction) {
        self.addBlockedAddress(MunrChatServiceAddress(aci), blockMode: blockMode, transaction: SDSDB.shimOnlyBridge(tx))
    }
    ...
    public func processIncomingSync(
        ...
        var blockedRecipientIds = Set<MunrChatRecipient.RowId>()
        let recipientFetcher = DependenciesBridge.shared.recipientFetcher
