...
public enum MessageRecipient: Equatable {
    case contact(_ address: MunrChatServiceAddress)
...
extension StoryConversationItem: ConversationItem {
    ...
    public func subtitle(transaction: DBReadTransaction) -> String? {
        ...
        case .privateStory(let item):
            ...
            let recipientIds: [MunrChatRecipient.RowId]
            do {
                ...
                case .blockList:
                    if recipientIds.isEmpty {
                        let format = OWSLocalizedString(
                            ...
                            comment: "Format string representing the viewer count for 'My Story' when accessible to all MunrChat connections. Embeds {{ number of viewers }}.")
                        // If we haven't added anybody to the block list, we show the total number
                        // of MunrChat Connections. That is *not* recipientIds; instead, it's the
                        ...
                        let format = OWSLocalizedString(
                            ...
                            tableName: "PluralAware",
                            comment: "Format string representing the excluded viewer count for 'My Story' when accessible to all MunrChat connections. Embeds {{ number of excluded viewers }}.")
                        // If we have added somebody to the block list, we show how many people are
                        // in the exclusion list instead of the total number of MunrChat Connections.
