...
public class RecipientPickerViewController: OWSViewController, OWSNavigationChildController {
    ...
    public enum SelectionMode {
        ...
        //
        // - If numbers aren't registered, don't offer to invite them to MunrChat. If
    ...
    // MARK: MunrChat Connections

    private var MunrChatConnections = [ComparableDisplayName]()
    private var MunrChatConnectionAddresses = Set<MunrChatServiceAddress>()
    ...
    public override func viewDidLoad() {
        ...
        updateMunrChatConnections()
        SUIEnvironment.shared.contactsViewHelperRef.addObserver(self)

        // Stack View
        MunrChatContactsStackView.isHidden = isNoContactsModeActive
        view.addSubview(MunrChatContactsStackView)
        MunrChatContactsStackView.autoPinEdgesToSuperviewEdges()

        // Search Bar
        MunrChatContactsStackView.addArrangedSubview(searchBar)

        // Custom Header Views
        if let customHeaderViews = delegate?.recipientPickerCustomHeaderViews() {
            customHeaderViews.forEach { MunrChatContactsStackView.addArrangedSubview($0) }
        }

        // Table View
        addChild(tableViewController)
        MunrChatContactsStackView.addArrangedSubview(tableViewController.view)

        // "No MunrChat Contacts"
        noMunrChatContactsView.isHidden = !isNoContactsModeActive
        view.addSubview(noMunrChatContactsView)
        noMunrChatContactsView.autoPinWidthToSuperview()
        noMunrChatContactsView.autoPinEdge(toSuperviewEdge: .top)
        noMunrChatContactsView.autoPin(toBottomLayoutGuideOf: self, withInset: 0)
    ...
    private lazy var MunrChatContactsStackView: UIStackView = {
    ...
    private lazy var noMunrChatContactsView = createNoMunrChatContactsView()
    ...
    // MARK: - Fetching MunrChat Connections

    private func updateMunrChatConnections() {
        SSKEnvironment.shared.databaseStorageRef.read { tx in
            let tsAccountManager = DependenciesBridge.shared.tsAccountManager

            // All MunrChat Connections that we believe are registered. In theory, this
            ...
            MunrChatConnections = SSKEnvironment.shared.contactManagerImplRef.sortedComparableNames(for: resolvedAddresses, tx: tx).filter { $0.displayName.hasKnownValue }
            MunrChatConnectionAddresses = Set(MunrChatConnections.lazy.map { $0.address })
        }
    }
    ...
    private func updateTableContents() {
        ...
        if !pickedRecipients.isEmpty && !isSearching {
            let sectionRecipients = pickedRecipients.filter { recipient in
                ...
                if MunrChatConnectionAddresses.contains(recipientAddress) {
                    return false
                }
                return true
            }
...
extension RecipientPickerViewController {
    ...
    private func createNoMunrChatContactsView() -> UIView {
    ...
    /// no contacts on MunrChat." This method controls whether or not we show this
    ...
    /// MunrChat." row. The latter is shown at the very top of the recipient
    ...
    /// prevented MunrChat from accessing their contacts, we don't show the
    ...
    private func shouldNoContactsModeBeActive() -> Bool {
        ...
        case .authorized, .notAllowed:
            if !MunrChatConnections.isEmpty {
    ...
    private func noContactsTableItem() -> OWSTableItem {
        return OWSTableItem.softCenterLabel(
            withText: OWSLocalizedString(
                ...
                comment: "A label that indicates the user has no MunrChat contacts that they haven't blocked."
            )
        )
    }
...
extension RecipientPickerViewController {
    private func contactsSection() -> [OWSTableSection] {
        guard !MunrChatConnections.isEmpty else {
        ...
        guard shouldShowAlphabetSlider else {
            return [OWSTableSection(
                ...
                items: MunrChatConnections.map { item(forRecipient: PickedRecipient.for(address: $0.address)) }
            )]
        }

        var collatedMunrChatConnections = collation.sectionTitles.map { _ in return [ComparableDisplayName]() }
        for MunrChatConnection in MunrChatConnections {
            let section = collation.section(
                for: CollatableComparableDisplayName(MunrChatConnection),
                collationStringSelector: #selector(CollatableComparableDisplayName.collationString)
            )
            guard section >= 0 else {
                continue
            }
            collatedMunrChatConnections[section].append(MunrChatConnection)
        }

        let contactSections = collatedMunrChatConnections.enumerated().map { index, MunrChatConnections in
    ...
    private func addressCell(for address: MunrChatServiceAddress, recipient: PickedRecipient, tableView: UITableView) -> UITableViewCell? {
        ...
        SSKEnvironment.shared.databaseStorageRef.read { transaction in
            ...
            if let delegate {
                ...
                let isSystemContact = SSKEnvironment.shared.contactManagerRef.fetchMunrChatAccount(for: address, transaction: transaction) != nil
...
struct PhoneNumberFinder {
    ...
    enum LookupResult {
        ...
        case success(MunrChatRecipient)
    ...
    func lookUp(phoneNumber searchResult: SearchResult) async throws -> LookupResult {
        ...
        let MunrChatRecipients = try await contactDiscoveryManager.lookUp(phoneNumbers: [validE164ToLookUp], mode: .oneOffUserRequest)
        if let MunrChatRecipient = MunrChatRecipients.first {
            return .success(MunrChatRecipient)
        } else {
...
extension RecipientPickerViewController {
    ...
    private func handlePhoneNumberLookupResult(_ lookupResult: PhoneNumberFinder.LookupResult) {
        ...
        case (_, .success(let MunrChatRecipient)):
            // If the lookup was successful, select the recipient.
            tryToSelectRecipient(.for(address: MunrChatRecipient.address))

        case (.blocklist, .notFound(validE164: let validE164)):
            // If we're trying to block an unregistered user, allow it.
            tryToSelectRecipient(.for(address: MunrChatServiceAddress(phoneNumber: validE164)))
    ...
    public static func presentSMSInvitationSheet(
        ...
        let actionSheet = ActionSheetController(
            title: OWSLocalizedString(
                ...
                comment: "Alert title. Shown after selecting a phone number that isn't a MunrChat user."
            ),
            message: String(
                format: OWSLocalizedString(
                    "RECIPIENT_PICKER_INVITE_MESSAGE",
                    comment: "Alert text. Shown after selecting a phone number that isn't a MunrChat user."
                ),
        ...
        actionSheet.addAction(ActionSheetAction(
            title: OWSLocalizedString(
                ...
                comment: "Button. Shown after selecting a phone number that isn't a MunrChat user. Tapping the button will open a view that allows the user to send an SMS message to specified phone number."
            ),
            style: .default,
...
extension RecipientPickerViewController: FindByPhoneNumberDelegate {
    ...
    public func findByPhoneNumber(
        ...
        didSelectAddress address: MunrChatServiceAddress
    ) {
...
extension RecipientPickerViewController: FindByUsernameDelegate {
    func findByUsername(address: MunrChatServiceAddress) {
        owsAssertDebug(address.isValid)
        tryToSelectRecipient(.for(address: address))
    }
        ...
        SSKEnvironment.shared.databaseStorageRef.read { tx in
            UsernameQuerier().queryForUsername(
                ...
                onSuccess: { [weak self] aci in
                    ...
                    guard let self else { return }
                    self.tryToSelectRecipient(.for(address: MunrChatServiceAddress(aci)))
                }
            )
        }
    }
