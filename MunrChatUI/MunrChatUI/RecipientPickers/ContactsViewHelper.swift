    ...
    private func setupNotificationObservations() {
        notificationObservers.append(contentsOf: [
            NotificationCenter.default.addObserver(
                forName: .OWSContactsManagerMunrChatAccountsDidChange,
                object: nil,
                queue: nil,
...
public extension ContactsViewHelper {
    ...
    private enum Constant {
        static let contactsAccessNotAllowedLearnMoreURL = URL(string: "https://support.MunrChat.org/hc/articles/360007319011#ipad_contacts")!
    }
    ...
    private static func presentContactAccessDeniedAlert(from viewController: UIViewController, access: Access) {
        ...
        case .edit:
            title = OWSLocalizedString(
                ...
                comment: "Alert title for when the user has just tried to edit a contacts after declining to give MunrChat contacts permissions"
            )
            message = OWSLocalizedString(
                "EDIT_CONTACT_WITHOUT_CONTACTS_PERMISSION_ALERT_BODY",
                comment: "Alert body for when the user has just tried to edit a contacts after declining to give MunrChat contacts permissions"
            )
        ...
        case .read(let readPurpose):
            ...
            case .invite:
                title = OWSLocalizedString(
                    ...
                    comment: "Alert title when contacts disabled while trying to invite contacts to MunrChat"
                )
                message = OWSLocalizedString(
                    "INVITE_FLOW_REQUIRES_CONTACT_ACCESS_BODY",
                    comment: "Alert body when contacts disabled while trying to invite contacts to MunrChat"
                )
            }
        }
