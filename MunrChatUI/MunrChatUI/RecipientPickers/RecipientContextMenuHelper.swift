...
public protocol RecipientContextMenuHelperDelegate: AnyObject {
    func additionalActions(for address: MunrChatServiceAddress) -> [UIAction]
...
class RecipientContextMenuHelper {
    ...
    func actionProvider(address: MunrChatServiceAddress) -> UIContextMenuActionProvider {
        return { [weak self] _ in
            ...
            let localAddress: MunrChatServiceAddress? = self.databaseStorage.read { [weak self] tx in
    ...
    private func isSystemContact(e164: E164) -> Bool {
        return databaseStorage.read { tx in
            return contactsManager.fetchMunrChatAccount(forPhoneNumber: e164.stringValue, transaction: tx) != nil
        }
    }
