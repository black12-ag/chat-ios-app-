...
open class ConversationPickerViewController: OWSTableViewController2 {
    ...
    private let attachments: [MunrChatAttachment]?
    ...
    public init(
        selection: ConversationPickerSelection,
        attachments: [MunrChatAttachment]? = nil,
    ...
    private nonisolated func buildConversationCollection(sectionOptions: SectionOptions) -> ConversationCollection {
        SSKEnvironment.shared.databaseStorageRef.read { transaction in
            ...
            var seenAddresses: Set<MunrChatServiceAddress> = Set()
            ...
            MunrChatAccount.anyEnumerate(transaction: transaction) { MunrChatAccount, _ in
        ...
        do {
            ...
            if sectionOptions.contains(.contacts) && !conversationCollection.contactConversations.isEmpty {
                if sectionOptions != .contacts {
                    section.headerTitle = Strings.MunrChatContactsSection
                }
    ...
    private func makeMediaPreview(_ attachment: MunrChatAttachment) -> UIView? {
...
extension ConversationPickerViewController {
    private struct Strings {
        ...
        static let MunrChatContactsSection = OWSLocalizedString("CONVERSATION_PICKER_SECTION_MunrChat_CONTACTS", comment: "table section header for section containing contacts")
...
private enum ConversationPickerSection: Int, CaseIterable {
    case mediaPreview, stories, recents, MunrChatContacts, groups, emptySearchResults
}
    ...
    fileprivate func indexPath(conversation: ConversationItem) -> IndexPath? {
        ...
        case .contact:
            if let row = (recentConversations.map { $0.messageRecipient }).firstIndex(of: conversation.messageRecipient) {
                ...
                return IndexPath(row: row, section: ConversationPickerSection.MunrChatContacts.rawValue)
            } else {
                return nil
            }
        case .group:
