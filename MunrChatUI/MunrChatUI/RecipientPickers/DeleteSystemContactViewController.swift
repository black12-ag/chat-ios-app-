...
class DeleteSystemContactViewController: OWSTableViewController2 {
    ...
    private func updateTableContents() {
        ...
        let addressForProfileLookup = MunrChatServiceAddress(serviceId: serviceId, e164: e164)
        let (
            image,
            systemContactName
    ...
    private func displayDeleteContactActionSheet(phoneNumber: String?, displayNameForToast: String) {
        let actionSheet = ActionSheetController(
            ...
            message: OWSLocalizedString(
                ...
                comment: "An explanation of what happens in MunrChat when you remove a system contact."
            )
        )
    ...
    private func handleContactDelete(displayNameForToast: String) {
        ...
        let MunrChatAccount = self.dependencies.databaseStorage.read { tx in
            return MunrChatAccountFinder().MunrChatAccount(
                for: self.e164,
                tx: tx
            )
        }
        ...
        let preferredContactIdForDeletion = MunrChatAccount?.cnContactId
        ...
        Task {
            do {
                try await withCooperativeTimeout(seconds: 5) {
                    for await _ in NotificationCenter.default.notifications(named: .OWSContactsManagerMunrChatAccountsDidChange).map({ _ in }) {
                        break
                    }
                }
            } catch {
            ...
            await self.dependencies.databaseStorage.awaitableWrite { tx in
                do {
                    try self.dependencies.recipientHidingManager.addHiddenRecipient(
                        MunrChatServiceAddress(serviceId: self.serviceId, e164: self.e164),
