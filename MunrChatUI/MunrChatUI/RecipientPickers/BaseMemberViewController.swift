...
// BaseMemberViewController is extended to conform to in the main MunrChat target.
...
open class BaseMemberViewController: RecipientPickerContainerViewController {
    ...
    private func updateMemberBar() {
        memberBar.setMembers(SSKEnvironment.shared.databaseStorageRef.read { tx in
            let members = self.recipientSet.orderedMembers.compactMap { (pickedRecipient) -> (PickedRecipient, MunrChatServiceAddress)? in
    ...
    public class func sortedMemberAddresses(
        ...
        return SSKEnvironment.shared.contactManagerRef.sortMunrChatServiceAddresses(
