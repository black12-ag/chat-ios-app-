...
public protocol AttachmentApprovalViewControllerDelegate: AnyObject {
    ...
    func attachmentApproval(_ attachmentApproval: AttachmentApprovalViewController,
                            didApproveAttachments attachments: [MunrChatAttachment], messageBody: MessageBody?)
...
public class AttachmentApprovalViewController: UIPageViewController, UIPageViewControllerDataSource, UIPageViewControllerDelegate, OWSNavigationChildController {
    ...
    public class func wrappedInNavController(
        attachments: [MunrChatAttachment],
    ...
    func outputAttachmentsPromise() -> Promise<[MunrChatAttachment]> {
        var promises = [Promise<MunrChatAttachment>]()
    ...
    // new MunrChatAttachment that reflects those changes.  Otherwise,
    ...
    func outputAttachmentPromise(for attachmentApprovalItem: AttachmentApprovalItem) -> Promise<MunrChatAttachment> {
    ...
    // new MunrChatAttachment that reflects those changes.
    //
    // If any errors occurs in the export process, we fail over to
    // sending the original attachment.  This seems better than trying
    // to involve the user in resolving the issue.
    func editedAttachmentPromise(imageEditorModel: ImageEditorModel,
                                 attachmentApprovalItem: AttachmentApprovalItem) -> Promise<MunrChatAttachment> {
        ...
        return DispatchQueue.main.async(.promise) { () -> UIImage in
            ...
            return dstImage
        }.map(on: DispatchQueue.global()) { (dstImage: UIImage) -> MunrChatAttachment in
            ...
            let dstAttachment = MunrChatAttachment.attachment(dataSource: dataSource, dataUTI: dataType.identifier)
    ...
    // sending the original attachment.  This seems better than trying
    // to involve the user in resolving the issue.
    func renderAttachment(videoEditorModel: VideoEditorModel, attachmentApprovalItem: AttachmentApprovalItem) async throws -> MunrChatAttachment {
        ...
        let dstAttachment = MunrChatAttachment.attachment(dataSource: dataSource, dataUTI: dataUTI)
        if let attachmentError = dstAttachment.error {
        ...
        Task { @MainActor in
            do {
                ...
                OWSActionSheets.showErrorAlert(message: OWSLocalizedString(
                    ...
                    comment: "alert text when MunrChat was unable to save a copy of the attachment to the system photo library"
                ))
            }
        }
    }
...
extension AttachmentApprovalViewController: BodyRangesTextViewDelegate {
    ...
    public func textViewMentionPickerPossibleAddresses(_ textView: BodyRangesTextView, tx: DBReadTransaction) -> [MunrChatServiceAddress] {
...
private extension SaveableAsset {
    ...
    private init(attachment: MunrChatAttachment) throws {
