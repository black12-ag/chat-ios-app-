...
public struct TypedItemProvider {
    ...
    public nonisolated func buildAttachment(progress: Progress? = nil) async throws -> MunrChatAttachment {
        ...
        case .image:
            ...
            } catch MunrChatAttachmentError.couldNotParseImage, ItemProviderError.fileUrlWasBplist {
        ...
        case .contact:
            ...
            let attachment = MunrChatAttachment.attachment(dataSource: dataSource, dataUTI: itemType.typeIdentifier)
    ...
    private nonisolated func buildFileAttachment(progress: Progress?) async throws -> MunrChatAttachment {
    ...
    private nonisolated static func createAttachment(withText text: String) throws -> MunrChatAttachment {
        let dataSource = DataSourceValue(oversizeText: text)
        let attachment = MunrChatAttachment.attachment(dataSource: dataSource, dataUTI: UTType.text.identifier)
        if let attachmentError = attachment.error {
            throw attachmentError
        }
    ...
    private nonisolated static func createAttachment(withImage image: UIImage) throws -> MunrChatAttachment {
        ...
        let attachment = MunrChatAttachment.attachment(dataSource: dataSource, dataUTI: type.identifier)
        if let attachmentError = attachment.error {
            throw attachmentError
        }
        return attachment
    ...
    private nonisolated func compressVideoIfNecessary(
        ...
        dataUTI: String,
        progress: Progress?
    ) async throws -> MunrChatAttachment {
        if MunrChatAttachment.isVideoThatNeedsCompression(
            dataSource: dataSource,
            dataUTI: dataUTI
        ) {
            ...
            let compressedAttachment = try await MunrChatAttachment.compressVideoAsMp4(
                dataSource: dataSource,
                dataUTI: dataUTI,
            ...
            let attachment = MunrChatAttachment.attachment(dataSource: dataSource, dataUTI: dataUTI)
