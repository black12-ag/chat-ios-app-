...
class MentionPicker: UIView {
    ...
    let style: Style
    let selectedAddressCallback: (MunrChatServiceAddress) -> Void

    init(
        mentionableAddresses: [MunrChatServiceAddress],
        style: Style,
        selectedAddressCallback: @escaping (MunrChatServiceAddress) -> Void
    ) {
        mentionableUsers = SSKEnvironment.shared.databaseStorageRef.read { transaction in
            let sortedAddresses = SSKEnvironment.shared.contactManagerImplRef.sortMunrChatServiceAddresses(
