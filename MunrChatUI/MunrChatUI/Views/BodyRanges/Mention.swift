...
public class Mention: NSObject {
    ...
    public class func withSneakyTransaction(address: MunrChatServiceAddress, style: Style) -> Mention {
        databaseStorage.read { transaction in
            Mention(address: address, style: style, transaction: transaction.unwrapGrdbRead)
        }
    }

    public convenience init(address: MunrChatServiceAddress, style: Style, transaction: GRDBReadTransaction) {
    ...
    private init(address: MunrChatServiceAddress, style: Style, text: String) {
    ...
    public var attributes: [NSAttributedString.Key: Any] {
        ...
        case .longMessageView:
            attributes[.backgroundColor] = Theme.isDarkThemeEnabled ? UIColor.ows_MunrChatBlueDark : UIColor.ows_blackAlpha20
            attributes[.foregroundColor] = Theme.primaryTextColor
        case .groupReply:
...
extension MessageBody {
    ...
    public func textValue(style: Mention.Style,
                          ...
                          shouldResolveAddress: (MunrChatServiceAddress) -> Bool,
    ...
    public func attributedBody(
        ...
        for (_, uuid) in orderedMentions {
            mentions[uuid] = mentions[uuid] ?? Mention(
                address: MunrChatServiceAddress(uuid: uuid),
                style: style,
                transaction: transaction
            )
        }
