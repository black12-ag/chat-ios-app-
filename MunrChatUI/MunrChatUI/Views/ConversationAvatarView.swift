...
public class ConversationAvatarView: UIView, CVView, PrimaryImageView {
    ...
    private static let serialQueue = ReverseDispatchQueue(label: "org.MunrChat.conversation-avatar.loading",
    ...
    private func ensureObservers() {
        ...
        if dataSource.isContactAvatar {
            ...
            NotificationCenter.default.addObserver(self,
                                                   selector: #selector(handleMunrChatAccountsChanged(notification:)),
                                                   name: .OWSContactsManagerMunrChatAccountsDidChange,
                                                   object: nil)
            NotificationCenter.default.addObserver(self,
    ...
    @objc
    private func handleMunrChatAccountsChanged(notification: Notification) {
        AssertIsOnMainThread()

        // PERF: It would be nice if we could do this only if *this* user's MunrChatAccount changed,
    ...
    private func otherUsersProfileDidChange(notification: Notification) {
        AssertIsOnMainThread()
        guard let changedAddress = notification.userInfo?[UserProfileNotifications.profileAddressKey] as? MunrChatServiceAddress, changedAddress.isValid else {
    ...
    private func skipContactAvatarBlurDidChange(notification: Notification) {
        AssertIsOnMainThread()
        guard let address = notification.userInfo?[OWSContactsManager.skipContactAvatarBlurAddressKey] as? MunrChatServiceAddress, address.isValid else {
    ...
    private func handleUpdatedAddressNotification(address: MunrChatServiceAddress) {
        AssertIsOnMainThread()
        guard let dataSource = configuration.dataSource else { return }
...
public enum ConversationAvatarDataSource: Equatable, CustomStringConvertible {
    ...
    case address(MunrChatServiceAddress)
    ...
    var contactAddress: MunrChatServiceAddress? {
        switch self {
    ...
    fileprivate func fetchBadge(configuration: ConversationAvatarView.Configuration, transaction: DBReadTransaction?) -> UIImage? {
        ...
        let targetAddress: MunrChatServiceAddress
        switch self {
