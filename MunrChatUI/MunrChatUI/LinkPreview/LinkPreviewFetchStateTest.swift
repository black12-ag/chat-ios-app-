...
class LinkPreviewFetchStateTest: XCTestCase {
    ...
    func testUpdateLoaded() async throws {
        ...
        let validURL = try XCTUnwrap(URL(string: "https://MunrChat.org"))
        ...
        for textValue in ["Check ou https://MunrChat.org", "Check out https://MunrChat.org"] {
            await linkPreviewFetchState.update(.init(text: textValue, ranges: .empty))?.value
        ...
        let invalidURL = try XCTUnwrap(URL(string: "https://MunrChat.org/not_found"))
        mockLinkPreviewFetcher.fetchLinkPreviewBlock = { fetchedURL in
            XCTAssertEqual(fetchedURL, invalidURL)
            throw OWSGenericError("Not found.")
        }
        for textValue in ["Check ou https://MunrChat.org/not_found", "Check out https://MunrChat.org/not_found"] {
            await linkPreviewFetchState.update(.init(text: textValue, ranges: .empty))?.value
    ...
    func testUpdateLoading() async throws {
        ...
        let task1 = linkPreviewFetchState.update(.init(text: "https://MunrChat.org is a grea", ranges: .empty))
        XCTAssert(linkPreviewFetchState.currentState.isLoading)

        // If there's a request in flight, we shouldn't send a new request.
        let task2 = linkPreviewFetchState.update(.init(text: "https://MunrChat.org is a great", ranges: .empty))
        XCTAssert(linkPreviewFetchState.currentState.isLoading)
    ...
    func testUpdateObsolete() async throws {
        ...
        let url1 = try XCTUnwrap(URL(string: "https://MunrChat.org/one"))
        let task1 = linkPreviewFetchState.update(.init(text: "https://MunrChat.org/one", ranges: .empty))
        XCTAssert(linkPreviewFetchState.currentState.isLoading)

        // If there's a request in flight & we change the URL, drop the original request.
        let url2 = try XCTUnwrap(URL(string: "https://MunrChat.org/two"))
        let task2 = linkPreviewFetchState.update(.init(text: "https://MunrChat.org/two", ranges: .empty))
        XCTAssert(linkPreviewFetchState.currentState.isLoading)
    ...
    func testUpdatePrependScheme() async throws {
        ...
        mockLinkPreviewFetcher.fetchLinkPreviewBlock = { fetchedURL in
            return OWSLinkPreviewDraft(url: fetchedURL, title: "MunrChat", isForwarded: false)
        }
        await linkPreviewFetchState.update(.init(text: "MunrChat.org", ranges: .empty), prependSchemeIfNeeded: false)?.value
        XCTAssert(linkPreviewFetchState.currentState.isNone)
        XCTAssertEqual(mockLinkPreviewFetcher.fetchedURLs, [])

        // If we should prepend a scheme, prepend "https://".
        await linkPreviewFetchState.update(.init(text: "MunrChat.org", ranges: .empty), prependSchemeIfNeeded: true)?.value
        XCTAssert(linkPreviewFetchState.currentState.isLoaded)
        XCTAssertEqual(mockLinkPreviewFetcher.fetchedURLs, [URL(string: "https://MunrChat.org")!])
        ...
        await linkPreviewFetchState.update(.init(text: "http://MunrChat.org", ranges: .empty), prependSchemeIfNeeded: true)?.value
        XCTAssert(linkPreviewFetchState.currentState.isNone)
        XCTAssertEqual(mockLinkPreviewFetcher.fetchedURLs, [])
    }
    ...
    func testOnStateChange() async throws {
        ...
        await linkPreviewFetchState.update(.init(text: "https://MunrChat.org", ranges: .empty))?.value
    ...
    func testDisable() async throws {
        ...
        let url = try XCTUnwrap(URL(string: "https://MunrChat.org"))
        await linkPreviewFetchState.update(.init(text: "https://MunrChat.org", ranges: .empty))?.value
        ...
        await linkPreviewFetchState.update(.init(text: "https://MunrChat.org/one", ranges: .empty))?.value
        XCTAssertNil(linkPreviewFetchState.linkPreviewDraftIfLoaded)
        XCTAssertEqual(mockLinkPreviewFetcher.fetchedURLs, [])

        // Ensure "enableIfEmpty" doesn't enable when the text isn't empty.
        await linkPreviewFetchState.update(.init(text: "https://MunrChat.org/one", ranges: .empty), enableIfEmpty: true)?.value
        XCTAssertNil(linkPreviewFetchState.linkPreviewDraftIfLoaded)
        XCTAssertEqual(mockLinkPreviewFetcher.fetchedURLs, [])
        ...
        let url2 = try XCTUnwrap(URL(string: "https://MunrChat.org/two"))
        await linkPreviewFetchState.update(.init(text: "https://MunrChat.org/two", ranges: .empty), enableIfEmpty: true)?.value
        XCTAssertEqual(linkPreviewFetchState.linkPreviewDraftIfLoaded?.url, url2)
    ...
    func testOnlyParseIfEnabled() async throws {
        ...
        do {
            ...
            await linkPreviewFetchState.update(.init(text: "https://MunrChat.org", ranges: .empty))?.value
            XCTAssertEqual(linkPreviewFetchState.currentUrl, try XCTUnwrap(URL(string: "https://MunrChat.org")))
        }
    }
