...
enum CallMode {
    ...
    var caller: MunrChatServiceAddress? {
        switch self {
        case .individual(let call):
...
enum CallError: Error {
    ...
    func shouldSilentlyDropCall() -> Bool {
        switch self {
        case .providerReset, .disconnected, .externalError, .timeout, .MunrChating:
            return false
...
class MunrChatCall: CallManagerCallReference {
    let mode: CallMode
    ...
    var caller: MunrChatServiceAddress? { mode.caller }
