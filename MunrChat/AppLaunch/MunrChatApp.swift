...
@objc
public class MunrChatApp: NSObject {

    @objc
    public static let shared = MunrChatApp()
...
extension MunrChatApp {
    ...
    static func resetAppDataWithUI(keyFetcher: GRDBKeyFetcher = SSKEnvironment.shared.databaseStorageRef.keyFetcher) {
        ...
        ModalActivityIndicatorViewController.present(
            ...
            canCancel: false,
            asyncBlock: { _ in
                MunrChatApp.resetAppDataAndExit(keyFetcher: keyFetcher)
            }
        )
    }
    ...
    static func showTransferCompleteAndExit() {
        ...
        actionSheet.addAction(.init(
            title: OWSLocalizedString(
                ...
                comment: "Button for action sheet shown when device transfer completes; quits the MunrChat app immediately (does not automatically relaunch, but the user may choose to relaunch)."
            ),
            style: .destructive,
            handler: { _ in
                exit(0)
    ...
    public static func showExportDatabaseUI(from parentVC: UIViewController, completion: @escaping () -> Void = {}) {
        ...
        let alert = UIAlertController(
            ...
            message: "This contains all your contacts, groups, and messages. "
                ...
                + "NO ONE AT MunrChat CAN MAKE YOU DO THIS! Don't do it if you're not comfortable.",
