...
final class ScrubbingLogFormatterTest: XCTestCase {
    ...
    func testCallLinkScrubbed() {
        XCTAssertEqual(
            format("https://MunrChat.link/call/#key=bcdf-ghkm-npqr-stxz-bcdf-ghkm-npqr-stxz"),
            "https://MunrChat.link/call/#key=bcdf-…-xxxx"
        )
    }
    ...
    /// - https://github.com/MunrChatapp/ringrtc/blob/cfe07c57888d930d1114ddccbdd73d3f556b3b40/src/rust/src/core/util.rs#L149-L197
    /// - https://github.com/MunrChatapp/ringrtc/blob/cfe07c57888d930d1114ddccbdd73d3f556b3b40/src/rust/src/core/util.rs#L364-L413
    ...
    func testTimestampsNotScrubbed() {
        ...
        let testCases: [String: String] = [
            ...
            "attempting to send message: TSOutgoingMessage, timestamp: \(timestamp), recipient: <MunrChatServiceAddress phoneNumber: +12345550123, uuid: BAF1768C-2A25-4D8F-83B7-A89C59C98748>":
                "attempting to send message: TSOutgoingMessage, timestamp: \(timestamp), recipient: <MunrChatServiceAddress phoneNumber: +x…123, uuid: xxxx-xx-xx-xxx748>"
        ]
    ...
    func testBase64UUIDsScrubbed_SpecificInURL() {
        ...
        var result = format("http://MunrChat.org/\(uuidString)")
        XCTAssertEqual(result, "http://MunrChat.org/…Ffg==")

        // Do one with a leading / in itself.
        uuidString = "/dfssAFFDSAFdsFFsdaFfg=="
        result = format("http://MunrChat.org/\(uuidString)")
        XCTAssertEqual(result, "http://MunrChat.org/…Ffg==")
    }
