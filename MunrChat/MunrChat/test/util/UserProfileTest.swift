...
class UserProfileTest: MunrChatBaseTest {
    override func setUp() {
        super.setUp()
    ...
    func testGetUserProfile() {
        let addresses: [MunrChatServiceAddress] = [
            MunrChatServiceAddress(Aci.randomForTesting()),
            MunrChatServiceAddress(Pni.randomForTesting()),
            MunrChatServiceAddress(phoneNumber: "+16505550101"),
        ]
        write { tx in
        ...
        read { tx in
            let actual = OWSUserProfile.getUserProfile(for: .otherUser(MunrChatServiceAddress.randomForTesting()), tx: tx)
            XCTAssertNil(actual)
        }
    }
    ...
    func testGetFor() {
        ...
        read { tx in
            let addressesToFetch: [MunrChatServiceAddress] = addresses + [MunrChatServiceAddress.randomForTesting()]
