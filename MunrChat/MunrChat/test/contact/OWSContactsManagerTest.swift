...
class OWSContactsManagerTest: MunrChatBaseTest {
    ...
    private func makeAndInsertRecipient(address: MunrChatServiceAddress) -> MunrChatRecipient {
        let recipient = MunrChatRecipient(
            aci: address.aci,
            pni: nil,
    ...
    private func createAccounts(_ accounts: [MunrChatAccount]) {
    ...
    func testGetDisplayNamesWithNicknames() throws {
        let aliceAddress = MunrChatServiceAddress(phoneNumber: "+16505550101")
        let bobAddress = MunrChatServiceAddress(serviceId: Aci.randomForTesting(), phoneNumber: "+16505550102")
    ...
    func testGetDisplayNamesWithCachedContactNames() {
        let addresses = [
            MunrChatServiceAddress(serviceId: Aci.randomForTesting(), phoneNumber: "+16505550100"),
            MunrChatServiceAddress(serviceId: Aci.randomForTesting(), phoneNumber: "+16505550101"),
        ]
    ...
    func testGetDisplayNamesWithProfileFullNames() {
        let addresses = [MunrChatServiceAddress.randomForTesting(), MunrChatServiceAddress.randomForTesting()]
    ...
    func testGetDisplayNamesWithPhoneNumbers() {
        let addresses = [
            MunrChatServiceAddress(phoneNumber: "+17035559900"),
            MunrChatServiceAddress(phoneNumber: "+17035559901")
        ]
    ...
    func testGetDisplayNamesWithUserNames() {
        ...
        let addresses = [MunrChatServiceAddress(aliceAci), MunrChatServiceAddress(bobAci)]
    ...
    func testGetDisplayNamesMixed() throws {
        let aliceAci = Aci.randomForTesting()
        let aliceAddress = MunrChatServiceAddress(serviceId: aliceAci, phoneNumber: "+16505550100")
        ...
        let bobAddress = MunrChatServiceAddress.randomForTesting()
        (SSKEnvironment.shared.profileManagerRef as! OWSFakeProfileManager).fakeUserProfiles = [
            bobAddress: makeUserProfile(givenName: "Bob", familyName: "Bobson"),
        ]

        let carolAddress = MunrChatServiceAddress(phoneNumber: "+17035559900")

        let daveAci = Aci.randomForTesting()
        let daveAddress = MunrChatServiceAddress(daveAci)
        dbV2.write { transaction in
            mockUsernameLookupMananger.saveUsername("dave", forAci: daveAci, transaction: transaction)
        }

        let eveAddress = MunrChatServiceAddress.randomForTesting()

        let feliciaAci = Aci.randomForTesting()
        let feliciaAddress = MunrChatServiceAddress(serviceId: feliciaAci, phoneNumber: "+16505550109")
    ...
    func testCachedContactNameMixed() {
        ...
        let chuckAddress = MunrChatServiceAddress(serviceId: chuckAci, phoneNumber: "+16505550101")
