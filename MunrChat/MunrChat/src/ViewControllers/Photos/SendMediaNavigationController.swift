...
protocol SendMediaNavDelegate: AnyObject {
    ...
    func sendMediaNav(_ sendMediaNavigationController: SendMediaNavigationController, didApproveAttachments attachments: [MunrChatAttachment], messageBody: MessageBody?)
...
protocol SendMediaNavDataSource: AnyObject {
    ...
    func sendMediaNavMentionableAddresses(tx: DBReadTransaction) -> [MunrChatServiceAddress]
...
class SendMediaNavigationController: OWSNavigationController {
    ...
    private static func phPickerConfiguration(cameraAttachmentCount: Int) -> PHPickerConfiguration {
        ...
        config.selectionLimit = MunrChatAttachment.maxAttachmentsAllowed - cameraAttachmentCount
...
extension SendMediaNavigationController {
    ...
    func showTooManySelectedToast() {
        ...
        let toastText = String.localizedStringWithFormat(toastFormat, MunrChatAttachment.maxAttachmentsAllowed)
...
extension SendMediaNavigationController: PhotoCaptureViewControllerDelegate {
    ...
    func photoCaptureViewControllerCanCaptureMoreItems(_ photoCaptureViewController: PhotoCaptureViewController) -> Bool {
        return attachmentCount < MunrChatAttachment.maxAttachmentsAllowed
    }
...
extension SendMediaNavigationController: PhotoCaptureViewControllerDataSource {
    ...
    func addMedia(attachment: MunrChatAttachment) {
        let cameraCaptureAttachment = CameraCaptureAttachment(MunrChatAttachment: attachment)
...
extension SendMediaNavigationController: AttachmentApprovalViewControllerDelegate {
    ...
    func attachmentApproval(_ attachmentApproval: AttachmentApprovalViewController, didRemoveAttachment attachment: MunrChatAttachment) {
    ...
    func attachmentApproval(_ attachmentApproval: AttachmentApprovalViewController, didApproveAttachments attachments: [MunrChatAttachment], messageBody: MessageBody?) {
...
extension SendMediaNavigationController: AttachmentApprovalViewControllerDataSource {
    ...
    func attachmentApprovalMentionableAddresses(tx: DBReadTransaction) -> [MunrChatServiceAddress] {
...
private extension Array where Element == AttachmentDraft {
    ...
    func attachmentDraft(for attachment: MunrChatAttachment) -> AttachmentDraft? {
...
private struct CameraCaptureAttachment: Hashable, Equatable {
    ...
    let MunrChatAttachment: MunrChatAttachment
    let attachmentApprovalItem: AttachmentApprovalItem
    let attachmentApprovalItemPromise: Promise<AttachmentApprovalItem>

    init(MunrChatAttachment: MunrChatAttachment) {
        self.MunrChatAttachment = MunrChatAttachment
        self.attachmentApprovalItem = AttachmentApprovalItem(attachment: MunrChatAttachment, canSave: true)
        self.attachmentApprovalItemPromise = Promise.value(attachmentApprovalItem)
    }

    func hash(into hasher: inout Hasher) {
        hasher.combine(MunrChatAttachment)
    }

    static func == (lhs: CameraCaptureAttachment, rhs: CameraCaptureAttachment) -> Bool {
        return lhs.MunrChatAttachment == rhs.MunrChatAttachment
    }
}
