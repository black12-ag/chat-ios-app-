...
class PhotoAlbumContents {
    ...
    private func requestVideoDataSource(for asset: PHAsset) -> Promise<MunrChatAttachment> {
        return Promise { future in
            ...
            _ = imageManager.requestAVAsset(forVideo: asset, options: options) { video, _, info in
                ...
                if let onDiskVideo = video as? AVURLAsset {
                    ...
                    if let dataSource = try? DataSourcePath(fileUrl: url, shouldDeleteOnDeallocation: false) {
                        if !MunrChatAttachment.isVideoThatNeedsCompression(dataSource: dataSource, dataUTI: dataUTI) {
                            future.resolve(MunrChatAttachment.attachment(dataSource: dataSource, dataUTI: dataUTI))
                            return
                        }
                    }
                ...
                Task {
                    do {
                        future.resolve(try await MunrChatAttachment.compressVideoAsMp4(asset: video, baseFilename: baseFilename, dataUTI: dataUTI))
                    } catch {
                        future.reject(error)
                    }
                }
    ...
    func outgoingAttachment(for asset: PHAsset) -> Promise<MunrChatAttachment> {
        switch asset.mediaType {
        case .image:
            return requestImageDataSource(for: asset).map(on: DispatchQueue.global()) { (dataSource: DataSource, dataUTI: String) in
                return MunrChatAttachment.attachment(dataSource: dataSource, dataUTI: dataUTI)
            }
        case .video:
