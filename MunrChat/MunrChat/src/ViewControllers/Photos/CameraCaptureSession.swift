...
protocol CameraCaptureSessionDelegate: AnyObject {
    ...
    func cameraCaptureSession(_ session: CameraCaptureSession, didFinishProcessing attachment: MunrChatAttachment)
...
class CameraCaptureSession: NSObject {
    ...
    private static let sessionQueue = DispatchQueue(label: "org.MunrChat.capture.camera")
    ...
    private func handleVideoRecording(at outputUrl: URL) {
        ...
        let attachment = MunrChatAttachment.attachment(dataSource: dataSource, dataUTI: UTType.mpeg4Movie.identifier)
...
extension CameraCaptureSession: PhotoCaptureDelegate {
    ...
    fileprivate func photoCaptureDidProduce(result: Result<Data, Error>) {
        ...
        case .success(let photoData):
            ...
            let attachment = MunrChatAttachment.attachment(dataSource: dataSource, dataUTI: UTType.jpeg.identifier)
            delegate.cameraCaptureSession(self, didFinishProcessing: attachment)
        }
    }
}
...
private class VideoCapture: NSObject, AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate {
    ...
    private let videoCaptureQueue = DispatchQueue(label: "org.MunrChat.capture.video", qos: .userInteractive)
    private let audioCaptureQueue = DispatchQueue(label: "org.MunrChat.capture.audio", qos: .userInteractive)
    private let recordingQueue = DispatchQueue(label: "org.MunrChat.capture.recording", qos: .userInteractive)
    ...
    func beginRecording(aspectRatio: CGFloat, includeAudio: Bool) throws {
        ...
        // video specs from MunrChat-Android: 2Mbps video 192K audio, 720P 30 FPS
