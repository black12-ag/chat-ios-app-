...
class StoryInfoSheet: OWSTableSheetViewController {
    ...
    private func buildStatusSections(for recipientStates: [ServiceId: StoryRecipientState]) -> [OWSTableSection] {
        ...
        for state in orderedSendingStates {
            ...
            let sortedRecipientAddresses = SSKEnvironment.shared.databaseStorageRef.read { tx in
                return SSKEnvironment.shared.contactManagerImplRef.sortMunrChatServiceAddresses(
                    recipients.map { MunrChatServiceAddress($0.key) },
                    transaction: tx
                )
            }
    ...
    private func statusMessage(for state: OWSOutgoingMessageRecipientStatus) -> String {
        ...
        case .skipped:
            return OWSLocalizedString(
                ...
                comment: "message status if message delivery to a recipient is skipped. We skip delivering group messages to users who have left the group or unregistered their MunrChat account."
            )
        }
    }
    ...
    private func contactItem(for address: MunrChatServiceAddress, accessoryText: String) -> OWSTableItem {
