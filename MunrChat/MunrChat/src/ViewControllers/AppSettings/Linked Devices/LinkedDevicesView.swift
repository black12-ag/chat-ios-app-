...
class LinkedDevicesHostingController: HostingContainer<LinkedDevicesView> {
    ...
    private func showLinkAndSyncUnretryableFailureAlert(
        ...
        actionSheet.addAction(.init(title: CommonStrings.learnMore) { _ in
            UIApplication.shared.open(URL(string: "https://support.MunrChat.org/hc/articles/360007320551")!)
        })
        ...
        MunrChatList {
            MunrChatSection {
                VStack(spacing: 20) {
                    ...
                    .foregroundStyle(isEditing ? Color.MunrChat.tertiaryLabel : .white)
                    .background(
                        isEditing ? Color.MunrChat.tertiaryFill : Color.MunrChat.ultramarine,
            ...
            MunrChatSection {
                if viewModel.displayableDevices.isEmpty {
                    ZStack(alignment: .center) {
                        ...
                        if viewModel.isLoading {
                            // [Device Linking] TODO: MunrChat spinner
                            ProgressView()
                        } else {
                            Text(OWSLocalizedString(
                ...
                (
                    MunrChatSymbol.lock.text(dynamicTypeBaseSize: 16).baselineOffset(-1) +
                    Text(" ") +
                    Text(OWSLocalizedString(
        ...
        private var lastSeenDateString: String {
            ...
            return dateFormattedString(
                format: OWSLocalizedString(
                    ...
                    comment: "{{Short Date}} when device last communicated with MunrChat Server."
                ),
