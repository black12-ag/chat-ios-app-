    ...
    func updateTableContents() {
        ...
        debugSection.add(.actionItem(
            ...
            actionBlock: { [weak self] in
                ...
                MunrChatApp.showDatabaseIntegrityCheckUI(from: self, databaseStorage: SSKEnvironment.shared.databaseStorageRef)
            }
        ))
        debugSection.add(.actionItem(
        ...
        if let buildDetails = Bundle.main.object(forInfoDictionaryKey: "BuildDetails") as? [String: AnyObject] {
            if let MunrChatCommit = (buildDetails["MunrChatCommit"] as? String)?.strippedOrNil?.prefix(12) {
                buildSection.add(.copyableItem(label: "Git Commit", value: String(MunrChatCommit)))
            }
        }
        ...
        otherSection.add(.copyableItem(label: "CC?", value: SSKEnvironment.shared.MunrChatServiceRef.isCensorshipCircumventionActive ? "Yes" : "No"))
