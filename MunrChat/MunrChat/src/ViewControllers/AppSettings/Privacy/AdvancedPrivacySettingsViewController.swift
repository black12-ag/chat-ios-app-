...
class AdvancedPrivacySettingsViewController: OWSTableViewController2 {
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(updateTableContents),
            name: .isMunrChatProxyReadyDidChange,
            object: nil
        )
    }
    ...
    private func updateTableContents() {
        ...
        if SSKEnvironment.shared.MunrChatServiceRef.hasCensoredPhoneNumber {
            isCensorshipCircumventionSwitchEnabled = true
            if SSKEnvironment.shared.MunrChatServiceRef.isCensorshipCircumventionManuallyDisabled {
            ...
            !SSKEnvironment.shared.MunrChatServiceRef.isCensorshipCircumventionActive,
            DependenciesBridge.shared.chatConnectionManager.unidentifiedConnectionState == .open
        {
            ...
            censorshipCircumventionSection.footerTitle = OWSLocalizedString(
                ...
                comment: "Table footer for the 'censorship circumvention' section shown when the app is connected to the MunrChat service."
            )
        } else if !SSKEnvironment.shared.MunrChatServiceRef.isCensorshipCircumventionActive, !SSKEnvironment.shared.reachabilityManagerRef.isReachable {
            isCensorshipCircumventionSwitchEnabled = false
            censorshipCircumventionSection.footerTitle = OWSLocalizedString(
        ...
        censorshipCircumventionSection.add(.switch(
            ...
            isOn: { SSKEnvironment.shared.MunrChatServiceRef.isCensorshipCircumventionActive },
        ...
        if SSKEnvironment.shared.MunrChatServiceRef.isCensorshipCircumventionManuallyActivated {
        ...
        proxySection.footerAttributedTitle = .composed(of: [
            OWSLocalizedString("USE_PROXY_EXPLANATION", comment: "Explanation of when you should use a MunrChat proxy"),
            " ",
            CommonStrings.learnMore.styled(with: .link(URL(string: "https://support.MunrChat.org/hc/articles/360056052052-Proxy-Support")!))
        ]).styled(
        ...
        proxySection.add(.disclosureItem(
            withText: OWSLocalizedString(
                ...
                comment: "Title for the MunrChat proxy settings"
            ),
            accessoryText: MunrChatProxy.isEnabled ? CommonStrings.switchOn : CommonStrings.switchOff,
            actionBlock: { [weak self] in
        ...
        if DependenciesBridge.shared.tsAccountManager.registrationStateWithMaybeSneakyTransaction.isRegisteredPrimaryDevice {
            ...
            sealedSenderSection.footerAttributedTitle = NSAttributedString.composed(of: [
                ...
                CommonStrings.learnMore.styled(
                    with: .link(URL(string: "https://MunrChat.org/blog/sealed-sender/")!)
                )
            ]).styled(
                with: .font(.dynamicTypeCaption1Clamped),
                .color(Theme.secondaryTextAndIconColor)
    ...
    private func didToggleEnableCensorshipCircumventionSwitch(_ sender: UISwitch) {
        SSKEnvironment.shared.MunrChatServiceRef.isCensorshipCircumventionManuallyDisabled = !sender.isOn
        SSKEnvironment.shared.MunrChatServiceRef.isCensorshipCircumventionManuallyActivated = sender.isOn
        updateTableContents()
    }

    private func ensureManualCensorshipCircumventionCountry() -> OWSCountryMetadata {
        let preferredCountryCode = SSKEnvironment.shared.MunrChatServiceRef.manualCensorshipCircumventionCountryCode ?? Locale.current.regionCode

        if
            let preferredCountryCode,
            let countryMetadata = OWSCountryMetadata.countryMetadata(countryCode: preferredCountryCode)
        {
            SSKEnvironment.shared.MunrChatServiceRef.manualCensorshipCircumventionCountryCode = preferredCountryCode
            return countryMetadata
        } else {
            let fallbackCountryCode = "US"
            SSKEnvironment.shared.MunrChatServiceRef.manualCensorshipCircumventionCountryCode = fallbackCountryCode
