...
class ProxySettingsViewController: OWSTableViewController2 {
    private var useProxy = MunrChatProxy.useProxy
    ...
    private var hasPendingChanges: Bool {
        useProxy != MunrChatProxy.useProxy || host != MunrChatProxy.host
    }
    ...
    private lazy var hostTextField: UITextField = {
        ...
        textField.text = MunrChatProxy.host
        textField.font = .dynamicTypeBody
        textField.backgroundColor = .clear
        textField.placeholder = OWSLocalizedString(
            "PROXY_PLACEHOLDER",
            comment: "Placeholder text for MunrChat proxy host"
        )
    ...
    private func updateTableContents() {
        ...
        useProxySection.add(.switch(
            withText: OWSLocalizedString("USE_PROXY_BUTTON", comment: "Button to activate the MunrChat proxy"),
            isOn: { [weak self] in
                self?.useProxy ?? false
            },
            target: self,
        ...
        proxyAddressSection.headerAttributedTitle = OWSLocalizedString("PROXY_ADDRESS", comment: "The title for the address of the MunrChat proxy").styled(
        ...
        shareSection.add(.init(
            ...
            actionBlock: { [weak self] in
                ...
                AttachmentSharing.showShareUI(for: URL(string: "https://MunrChat.tube/#\(self.host ?? "")")!, sender: self.view)
            }
        ))
    ...
    private func notifyForInvalidHostIfNecessary() -> Bool {
        guard !MunrChatProxy.isValidProxyFragment(host) else { return false }
    ...
    private func didTapSave() {
        ...
        SSKEnvironment.shared.databaseStorageRef.write { transaction in
            MunrChatProxy.setProxyHost(host: self.host, useProxy: self.useProxy, transaction: transaction)
        }
        updateNavigationBar()
        ...
        ModalActivityIndicatorViewController.present(fromViewController: self, canCancel: true, asyncBlock: { modal in
            ...
            modal.dismiss {
                if connected {
                    ...
                    SSKEnvironment.shared.databaseStorageRef.write { transaction in
                        MunrChatProxy.setProxyHost(host: self.host, useProxy: false, transaction: transaction)
                    }
