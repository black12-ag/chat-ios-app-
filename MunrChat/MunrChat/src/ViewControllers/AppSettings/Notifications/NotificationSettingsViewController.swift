    ...
    func updateTableContents() {
        ...
        soundsSection.add(.switch(
            withText: OWSLocalizedString(
                ...
                comment: "Table cell switch label. When disabled, MunrChat will not play notification sounds while the app is in the foreground."
            ),
        ...
        notifyWhenSection.add(.switch(
            withText: OWSLocalizedString(
                "SETTINGS_NOTIFICATION_EVENTS_CONTACT_JOINED_MunrChat",
                comment: "When the local device discovers a contact has recently installed MunrChat, the app can generates a message encouraging the local user to say hello. Turning this switch off disables that feature."
            ),
            isOn: {
