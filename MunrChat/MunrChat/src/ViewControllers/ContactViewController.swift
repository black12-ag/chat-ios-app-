...
class ContactViewController: OWSTableViewController2 {
    ...
    private enum ContactViewMode {
        case systemContactWithMunrChat
        case systemContactWithoutMunrChat
    ...
    private static func viewMode(for phoneNumberPartition: OWSContact.PhoneNumberPartition) -> ContactViewMode {
        return phoneNumberPartition.map(
            ifSendablePhoneNumbers: { _ in .systemContactWithMunrChat },
            elseIfInvitablePhoneNumbers: { _ in .systemContactWithoutMunrChat },
    ...
    private func showInviteToMunrChat() -> Bool {
        switch viewMode {
        case .systemContactWithoutMunrChat, .nonSystemContact:
            return true
        default:
            return false
        }
    ...
    private func updateContent() {
        ...
        // Message, Video, Audio buttons for MunrChat contacts as a horizontal stack of buttons
        if viewMode == .systemContactWithMunrChat {
            ...
            if isRegistered {
                func addAction(title: String, action: ConversationViewAction) {
                    actionSheet.addAction(ActionSheetAction(
                        ...
                        style: .default,
                        handler: { _ in
                            let address = MunrChatServiceAddress(phoneNumber: phoneNumber)
                            MunrChatApp.shared.presentConversationForAddress(address, action: action, animated: true)
                        }
                    ))
                }
