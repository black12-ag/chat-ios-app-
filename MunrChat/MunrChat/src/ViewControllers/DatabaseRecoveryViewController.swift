...
class DatabaseRecoveryViewController<SetupResult>: OWSViewController {
    ...
    private func didTapToExportDatabase() {
        ...
        MunrChatApp.showExportDatabaseUI(from: self)
    }
    ...
    @objc
    private func didTapToResetMunrChat() {
        OWSActionSheets.showConfirmationAlert(
            ...
            MunrChatApp.resetAppDataWithUI(keyFetcher: GRDBKeyFetcher(keychainStorage: keychainStorage))
        }
    }
    ...
    private func renderDeviceSpaceWarning() {
        ...
        headlineLabel.text = OWSLocalizedString(
            ...
            comment: "On the database recovery screen, if the user's device storage is nearly full, MunrChat will not be able to recover the database. A warning screen, which can be bypassed if the user wishes, will be shown. This is the title of that screen."
        )

        descriptionLabel.text = {
            let labelFormat = OWSLocalizedString(
                "DATABASE_RECOVERY_MORE_STORAGE_SPACE_NEEDED_DESCRIPTION",
                comment: "On the database recovery screen, if the user's device storage is nearly full, MunrChat will not be able to recover the database. A warning screen, which can be bypassed if the user wishes, will be shown. This is the line of text on that screen. Embeds an amount like \"2GB\"."
            )
        ...
        let continueButton = button(
            title: OWSLocalizedString(
                ...
                comment: "On the database recovery screen, if the user's device storage is nearly full, MunrChat will not be able to recover the database. A warning screen, which can be bypassed if the user wishes, will be shown. This is the text on the button to bypass the warning."
            ),
    ...
    private func renderRecoveryFailed() {
        ...
        let resetMunrChatButton = self.button(
            title: OWSLocalizedString(
            ...
            selector: #selector(didTapToResetMunrChat),
        ...
        resetMunrChatButton.autoPinWidthToSuperviewMargins()
