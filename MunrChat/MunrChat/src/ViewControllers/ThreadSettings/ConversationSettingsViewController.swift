...
class ConversationSettingsViewController: OWSTableViewController2, BadgeCollectionDataSource {
    ...
    private(set) var groupMemberStateMap = [MunrChatServiceAddress: VerificationState]()
    private(set) var sortedGroupMembers = [MunrChatServiceAddress]()
    func updateGroupMembers(transaction tx: DBReadTransaction) {
        ...
        var allMembersSorted = [MunrChatServiceAddress]()
        var verificationStateMap = [MunrChatServiceAddress: VerificationState]()
        ...
        allMembersSorted = SSKEnvironment.shared.contactManagerImplRef.sortMunrChatServiceAddresses(allMembers, transaction: tx)

        var membersToRender = [MunrChatServiceAddress]()
    ...
    func reloadThreadAndUpdateContent() {
        let didUpdate = SSKEnvironment.shared.databaseStorageRef.read { tx -> Bool in
            ...
            self.isSystemContact = {
                ...
                return SSKEnvironment.shared.contactManagerRef.fetchMunrChatAccount(for: address, transaction: tx) != nil
            }()
    ...
    func didSelectGroupMember(_ memberAddress: MunrChatServiceAddress) {
    ...
    func presentCreateOrEditContactViewController(address: MunrChatServiceAddress, editImmediately: Bool) {
    ...
    private func presentAddToExistingContactFlow(address: MunrChatServiceAddress) {
        SUIEnvironment.shared.contactsViewHelperRef.presentSystemContactsFlow(
    ...
    func showReplaceAdminView(candidates: Set<MunrChatServiceAddress>) {
    ...
    private var replacementAdminCandidates: Set<MunrChatServiceAddress> {
        guard let groupThread = thread as? TSGroupThread else {
            owsFailDebug("Invalid thread.")
            return []
        }
    ...
    private func otherUsersProfileDidChange(notification: Notification) {
        ...
        guard let address = notification.userInfo?[UserProfileNotifications.profileAddressKey] as? MunrChatServiceAddress,
    ...
    private func profileWhitelistDidChange(notification: Notification) {
        ...
        if let address = notification.userInfo?[UserProfileNotifications.profileAddressKey] as? MunrChatServiceAddress,
