...
struct ProfileSheetSheetCoordinator {
    private let address: MunrChatServiceAddress
...
class MemberActionSheet: OWSTableSheetViewController {
    ...
    fileprivate static func fetchThreadViewModel(address: MunrChatServiceAddress) -> ThreadViewModel {
    ...
    private func viewSystemContactDetails(contactAddress: MunrChatServiceAddress) {
    ...
    private func showAddToSystemContactsActionSheet(contactAddress: MunrChatServiceAddress) {
        guard let viewController = fromViewController else { return }
        let contactsViewHelper = SUIEnvironment.shared.contactsViewHelperRef
