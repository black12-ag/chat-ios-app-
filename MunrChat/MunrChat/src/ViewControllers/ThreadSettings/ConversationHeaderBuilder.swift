...
struct ConversationHeaderBuilder {
    ...
    static func buildHeaderForContact(
        ...
        if isVerified {
            ...
            subtitle.append(MunrChatSymbol.safetyNumber.attributedString(for: .subheadline, clamped: true))
            subtitle.append(" ")
        ...
        if options.contains(.message) {
            buttons.append(buildIconButton(
                ...
                action: { [weak delegate] in
                    ...
                    MunrChatApp.shared.presentConversationForThread(
    ...
    static func threadAttributedString(
        ...
        if isSystemContact {
            let contactIcon = MunrChatSymbol.personCircle.attributedString(
        ...
        if canTap {
            let chevron = MunrChatSymbol.chevronTrailing(for: threadName).attributedString(
                dynamicTypeBaseSize: 24,
                weight: .bold,
                leadingCharacter: .nonBreakingSpace,
                attributes: [.foregroundColor: UIColor.MunrChat.secondaryLabel]
            )
...
extension ConversationHeaderDelegate {
    ...
    func threadAttributedString(renderLocalUserAsNoteToSelf: Bool, tx: DBReadTransaction) -> NSAttributedString {
        ...
        if let contactThread = self.thread as? TSContactThread {
            SSKEnvironment.shared.contactManagerRef.fetchMunrChatAccount(
