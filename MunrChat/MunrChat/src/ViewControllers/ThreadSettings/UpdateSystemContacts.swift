...
class AddToExistingContactFlow: SystemContactsFlow {
    ...
    init(address: MunrChatServiceAddress, nameComponents: PersonNameComponents? = nil) {
        self.address = address
        self.nameComponents = nameComponents
    }
}
...
extension ContactsViewHelper {
    ...
    fileprivate func contactViewController(for systemContactsFlow: SystemContactsFlow) -> CNContactViewController {
        ...
        let MunrChatAccount = SSKEnvironment.shared.databaseStorageRef.read { tx in
            return SSKEnvironment.shared.contactManagerRef.fetchMunrChatAccount(for: address, transaction: tx)
        }
        ...
        if cnContact == nil, let cnContactId = MunrChatAccount?.cnContactId {
