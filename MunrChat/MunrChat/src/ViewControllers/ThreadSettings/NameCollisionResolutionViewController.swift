...
class NameCollisionResolutionViewController: OWSTableViewController2 {
    ...
    private func unblock(address: MunrChatServiceAddress) {
    ...
    private func blockAddress(_ address: MunrChatServiceAddress) {
    ...
    private func removeFromGroup(_ address: MunrChatServiceAddress) {
    ...
    private func presentUpdateContactViewController(for address: MunrChatServiceAddress) {
...
extension NameCollisionResolutionViewController: ContactsViewHelperObserver {
    ...
    func shouldShowContactUpdateAction(for address: MunrChatServiceAddress) -> Bool {
        guard SSKEnvironment.shared.contactManagerImplRef.isEditingAllowed else {
            return false
        }
        return SSKEnvironment.shared.databaseStorageRef.read { transaction in
            return SSKEnvironment.shared.contactManagerRef.fetchMunrChatAccount(for: address, transaction: transaction) != nil
        }
    }
