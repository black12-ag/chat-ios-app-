    ...
    private func updateTableContents() {
        ...
        SSKEnvironment.shared.databaseStorageRef.read { transaction in
            ...
            if let contactThread = thread as? TSContactThread {
                ...
                let MunrChatRecipient = recipientDatabaseTable.fetchRecipient(address: address, tx: transaction)

                section.add(.copyableItem(
                    label: "ACI",
                    value: MunrChatRecipient?.aci?.serviceIdString
                ))

                section.add(.copyableItem(
                    label: "Phone Number",
                    value: MunrChatRecipient?.phoneNumber?.stringValue
                ))

                section.add(.copyableItem(
                    label: "PNI",
                    value: MunrChatRecipient?.pni?.serviceIdString
                ))

                section.add(.copyableItem(
                    label: "Discoverable Phone Number?",
                    value: MunrChatRecipient?.phoneNumber?.isDiscoverable == true ? "Yes" : "No"
                ))
        ...
        if let contactThread = thread as? TSContactThread {
            ...
            sessionSection.add(.actionItem(withText: "Delete Session") {
                SSKEnvironment.shared.databaseStorageRef.write { transaction in
                    let aciStore = DependenciesBridge.shared.MunrChatProtocolStoreManager.MunrChatProtocolStore(for: .aci)
