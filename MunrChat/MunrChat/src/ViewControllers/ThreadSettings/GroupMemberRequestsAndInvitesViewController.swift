...
public class GroupMemberRequestsAndInvitesViewController: OWSTableViewController2 {
    ...
    private func addContentsForMemberRequests(contents: OWSTableContents) {
        ...
        let requestingMembersSorted = SSKEnvironment.shared.databaseStorageRef.read { tx in
            SSKEnvironment.shared.contactManagerImplRef.sortMunrChatServiceAddresses(groupMembership.requestingMembers, transaction: tx)
        }
    ...
    private func buildMemberRequestButtons(address: MunrChatServiceAddress) -> ContactCellAccessoryView {
    ...
    private func approveMemberRequest(address: MunrChatServiceAddress) {
        showAcceptMemberRequestUI(address: address)
    }

    private func denyMemberRequest(address: MunrChatServiceAddress) {
    ...
    private func addContentsForPendingInvites(contents: OWSTableContents) {
        ...
        let allPendingMembersSorted = SSKEnvironment.shared.databaseStorageRef.read { tx in
            SSKEnvironment.shared.contactManagerImplRef.sortMunrChatServiceAddresses(groupMembership.invitedMembers, transaction: tx)
        }

        // Note that these collections retain their sorting from above.
        var membersInvitedByLocalUser = [MunrChatServiceAddress]()
        var membersInvitedByOtherUsers = [MunrChatServiceAddress: [MunrChatServiceAddress]]()
        for invitedAddress in allPendingMembersSorted {
            ...
            if inviterAci == localAci {
                ...
                var invitedMembers = membersInvitedByOtherUsers[MunrChatServiceAddress(inviterAci)] ?? []
                invitedMembers.append(invitedAddress)
                membersInvitedByOtherUsers[MunrChatServiceAddress(inviterAci)] = invitedMembers
            }
        }
        ...
        if membersInvitedByOtherUsers.count > 0 {
            let inviterAddresses = SSKEnvironment.shared.databaseStorageRef.read { tx in
                SSKEnvironment.shared.contactManagerImplRef.sortMunrChatServiceAddresses(membersInvitedByOtherUsers.keys, transaction: tx)
            }
    ...
    private func showRevokePendingInviteFromLocalUserConfirmation(invitedAddress: MunrChatServiceAddress) {
    ...
    private func showRevokePendingInviteFromOtherUserConfirmation(invitedAddresses: [MunrChatServiceAddress],
                                                                  inviterAddress: MunrChatServiceAddress) {
    ...
    private func inviteFromLocalUserWasTapped(_ address: MunrChatServiceAddress,
                                              canRevoke: Bool) {
        if canRevoke {
    ...
    private func invitesFromOtherUserWasTapped(invitedAddresses: [MunrChatServiceAddress],
                                               inviterAddress: MunrChatServiceAddress,
                                               canRevoke: Bool) {
        if canRevoke {
    ...
    private func showMemberActionSheet(for address: MunrChatServiceAddress) {
    ...
    private func presentRequestApprovedToast(address: MunrChatServiceAddress) {
    ...
    private func presentRequestDeniedToast(address: MunrChatServiceAddress) {
...
private extension GroupMemberRequestsAndInvitesViewController {
    ...
    func revokePendingInvites(addresses: [MunrChatServiceAddress]) {
...
fileprivate extension GroupMemberRequestsAndInvitesViewController {
    ...
    func showAcceptMemberRequestUI(address: MunrChatServiceAddress) {
    ...
    func showDenyMemberRequestUI(address: MunrChatServiceAddress) {
    ...
    func acceptOrDenyMemberRequests(address: MunrChatServiceAddress, shouldAccept: Bool) {
