...
class ReplaceAdminViewController: OWSTableViewController2 {
    ...
    private let candidates: Set<MunrChatServiceAddress>

    init(candidates: Set<MunrChatServiceAddress>,
    ...
    private func updateTableContents() {
        ...
        let sortedCandidates = SSKEnvironment.shared.databaseStorageRef.read { tx in
            SSKEnvironment.shared.contactManagerImplRef.sortMunrChatServiceAddresses(self.candidates, transaction: tx)
        }
    ...
    private func candidateWasSelected(candidate: MunrChatServiceAddress) {
