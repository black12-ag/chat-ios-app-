...
extension GroupViewHelper {
    ...
    func memberActionSheetCanMakeGroupAdmin(address: MunrChatServiceAddress) -> Bool {
    ...
    @MainActor
    func memberActionSheetMakeGroupAdminWasSelected(address: MunrChatServiceAddress) {
    ...
    func memberActionSheetCanRevokeGroupAdmin(address: MunrChatServiceAddress) -> Bool {
        guard let groupThread = thread as? TSGroupThread,
            groupThread.isGroupV2Thread else {
                return false
        }
    ...
    @MainActor
    func memberActionSheetRevokeGroupAdminWasSelected(address: MunrChatServiceAddress) {
    ...
    func canRemoveFromGroup(address: MunrChatServiceAddress) -> Bool {
        guard let groupThread = thread as? TSGroupThread,
            groupThread.isGroupV2Thread else {
                return false
        }
    ...
    @MainActor
    func presentRemoveFromGroupActionSheet(address: MunrChatServiceAddress) {
