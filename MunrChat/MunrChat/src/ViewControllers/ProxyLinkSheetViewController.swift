...
class ProxyLinkSheetViewController: OWSTableSheetViewController {
    ...
    init?(url: URL) {
        guard MunrChatProxy.isValidProxyLink(url) else { return nil }
        self.url = url
        super.init()
    }
    ...
    override func updateTableContents(shouldReload: Bool = true) {
        ...
        addressSection.headerTitle = OWSLocalizedString("PROXY_ADDRESS", comment: "The title for the address of the MunrChat proxy")
        ...
        actionsSection.add(.init(customCellBlock: { [weak self] in
            ...
            let stackView = UIStackView(arrangedSubviews: [
                ...
                self.button(
                    title: OWSLocalizedString("USE_PROXY_BUTTON", comment: "Button to activate the MunrChat proxy"),
                    titleColor: .ows_accentBlue,
                    touchHandler: { [weak self] in
                        SSKEnvironment.shared.databaseStorageRef.write {
                            MunrChatProxy.setProxyHost(host: proxyHost, useProxy: true, transaction: $0)
                        }
                        ...
                        _ = Task(priority: .userInitiated) {
                            if await ProxyConnectionChecker(chatConnectionManager: DependenciesBridge.shared.chatConnectionManager).checkConnection() {
                                ...
                                SSKEnvironment.shared.databaseStorageRef.write { transaction in
                                    MunrChatProxy.setProxyHost(host: proxyHost, useProxy: false, transaction: transaction)
                                }
                            }
                        }
