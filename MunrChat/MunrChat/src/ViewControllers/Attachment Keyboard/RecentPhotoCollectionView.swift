...
protocol RecentPhotosDelegate: AnyObject {
    func didSelectRecentPhoto(asset: PHAsset, attachment: MunrChatAttachment)
}
...
class RecentPhotosCollectionView: UICollectionView {
    ...
    private func noAccessToPhotosView() -> UIView {
        let textLabel = textLabel(text: OWSLocalizedString(
            ...
            comment: "Text in chat attachment panel explaining that user needs to give MunrChat permission to access photos."
        ))
        let button = button(title: OWSLocalizedString(
            "ATTACHMENT_KEYBOARD_OPEN_SETTINGS",
            comment: "Button in chat attachment panel to let user open Settings app and give MunrChat persmission to access photos."
        ))
        button.block = {
