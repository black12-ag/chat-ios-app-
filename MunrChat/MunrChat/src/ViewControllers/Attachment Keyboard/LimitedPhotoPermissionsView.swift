...
class LimitedPhotoPermissionsView: UIView {
    private let button: UIButton = {
        let selectMoreAction = UIAction(
            title: OWSLocalizedString(
                ...
                comment: "Button in a context menu from the 'manage' button in attachment panel that allows to select more photos/videos to give MunrChat access to"
            ),
        ...
        let settingsAction = UIAction(
            title: OWSLocalizedString(
                ...
                comment: "Button in a context menu from the 'manage' button in attachment panel that opens the iOS system settings for MunrChat to update access permissions"
            ),
        ...
        buttonConfig.baseForegroundColor = UIColor.MunrChat.label
        buttonConfig.baseBackgroundColor = UIColor.MunrChat.tertiaryFill
        var titleAttributedString = AttributedString(
            OWSLocalizedString(
                "ATTACHMENT_KEYBOARD_BUTTON_MANAGE",
                comment: "Button in chat attachment panel that allows to select photos/videos MunrChat has access to."
            )
        )
    ...
    init() {
        ...
        let label = UILabel()
        label.textColor = UIColor.MunrChat.secondaryLabel
        label.font = .dynamicTypeBody2Clamped
        label.numberOfLines = 2
        label.setContentHuggingHorizontalLow()
        label.text = OWSLocalizedString(
            "ATTACHMENT_KEYBOARD_LIMITED_ACCESS",
            comment: "Text in chat attachment panel when MunrChat only has access to some photos/videos. This string is in a compact horizontal space, so it should be short if possible."
        )
