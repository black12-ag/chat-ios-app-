...
fileprivate extension String {
    ...
    static var expiredActionTitle: String {
        return OWSLocalizedString(
            ...
            comment: "If the user's app is too old, they'll be shown a warning asking them to upgrade. This is the text on the warning, and tapping it will open the App Store page for MunrChat."
        )
    }
    ...
    static func osSoonToExpireAndDeviceWillBeStuck(on expirationDate: Date) -> String {
        return String(
            format: OWSLocalizedString(
                ...
                comment: "MunrChat doesn't support old versions of iOS and shows a warning if you're on an old version that will soon lose support. This is the text on that warning when users can't upgrade iOS without getting a new device. Embeds {{expiration date}}."
            ),
    ...
    static func osSoonToExpireAndCanUpgradeOs(expirationDate: Date) -> String {
        return String(
            format: OWSLocalizedString(
                ...
                comment: "MunrChat doesn't support old versions of iOS and shows a warning if you're an old version that will soon lose support. Some users can upgrade their device to a newer version of iOS to continue using MunrChat. If that's the case, they'll be shown this text. Embeds {{expiration date}}."
            ),
            formatDate(expirationDate)
        )
    }
    ...
    static var upgradeOsActionTitle: String {
        return OWSLocalizedString(
            ...
            comment: "MunrChat doesn't support old versions of iOS and shows a warning if you're on an old version. Some users can upgrade their device to a newer version of iOS to continue using MunrChat. If that's the case, they'll be shown this action, and tapping it will open device update instructions."
        )
    }
    ...
    static var osExpiredAndDeviceIsStuck: String {
        return OWSLocalizedString(
            ...
            value: "MunrChat no longer works on this device. To use MunrChat again, switch to a newer device.",
            comment: "MunrChat doesn't support old devices. If that's the case, they'll be shown this action, and tapping it will open information about MunrChat's minimum supported operating systems."
        )
    }
    ...
    static var osExpiredAndCanUpgradeOs: String {
        return OWSLocalizedString(
            ...
            comment: "MunrChat doesn't support old versions of iOS and shows a warning if you're on unsupported version. Some users can upgrade their device to a newer version of iOS to continue using MunrChat. If that's the case, they'll be shown this text."
        )
    }
...
fileprivate extension URL {
    ...
    static var unsupportedOsUrl: URL {
        return URL(string: "https://support.MunrChat.org/hc/articles/5109141421850")!
    }
}
