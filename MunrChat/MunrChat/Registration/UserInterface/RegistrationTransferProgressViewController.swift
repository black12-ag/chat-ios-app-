        ...
        case .backgroundedDevice:
            progressView.renderError(
                text: OWSLocalizedString(
                    ...
                    comment: "An error indicating that the other device closed MunrChat mid-transfer and it could not complete"
                )
            )
        case .cancel:
    ...
    override public func viewDidLoad() {
        ...
        exitButton.setTitle(
            OWSLocalizedString(
                ...
                comment: "Button for bottom sheet shown when device transfer completes on the receiving device. Tapping will terminate the MunrChat app and trigger a notification to relaunch."
            ),
            for: .normal
        )
