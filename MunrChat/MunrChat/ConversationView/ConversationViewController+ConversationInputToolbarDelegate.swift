...
extension ConversationViewController: ConversationInputToolbarDelegate {
    ...
    func tryToSendAttachments(
        _ attachments: [MunrChatAttachment],
    ...
    enum SendAttachmentError: Error {
        ...
        case invalidAttachment(MunrChatAttachment)
    }
    ...
    public func didSelectRecentPhoto(asset: PHAsset, attachment: MunrChatAttachment) {
        AssertIsOnMainThread()
...
public extension ConversationViewController {
    ...
    func showApprovalDialog(forAttachments attachments: [MunrChatAttachment]) {
        AssertIsOnMainThread()
...
extension ConversationViewController: LocationPickerDelegate {
    ...
    public func didPickLocation(_ locationPicker: LocationPicker, location: Location) {
        ...
        Task { @MainActor in
            let attachment: MunrChatAttachment
            do {
...
extension ConversationViewController: UIDocumentPickerDelegate {
    ...
    public func documentPicker(_ controller: UIDocumentPickerViewController, didPickDocumentAt url: URL) {
        ...
        if MunrChatAttachment.isVideoThatNeedsCompression(dataSource: dataSource,
                                                        dataUTI: contentType.identifier) {
            self.showApprovalDialogAfterProcessingVideoURL(url, filename: filename)
            return
        }

        let attachment = MunrChatAttachment.attachment(dataSource: dataSource, dataUTI: contentType.identifier)
    ...
    private func showApprovalDialogAfterProcessingVideoURL(_ movieURL: URL, filename: String?) {
        ...
        ModalActivityIndicatorViewController.present(fromViewController: self,
            ...
            let promise = Promise.wrapAsync({
                return try await MunrChatAttachment.compressVideoAsMp4(dataSource: dataSource,
                                                                     dataUTI: UTType.mpeg4Movie.identifier)
            })
            promise.done(on: DispatchQueue.main) { (attachment: MunrChatAttachment) in
...
extension ConversationViewController: SendMediaNavDelegate {
    ...
    func sendMediaNav(
        ...
        didApproveAttachments attachments: [MunrChatAttachment],
        messageBody: MessageBody?
    ) {
        Task { @MainActor in
