...
struct CVItemModelBuilder: CVItemBuilding {
    ...
    private static func configureItemViewState(item: ItemBuilder,
        ...
        if let outgoingMessage = interaction as? TSOutgoingMessage {
            ...
            let incomingSenderAddress: MunrChatServiceAddress = incomingMessage.authorAddress
            ...
            if thread.isGroupThread {
                ...
                if let nextItem = nextItem,
                   let nextIncomingMessage = nextItem.interaction as? TSIncomingMessage {
                    let nextIncomingSenderAddress: MunrChatServiceAddress = nextIncomingMessage.authorAddress
...
class DisplayNameCache {
    ...
    private func _displayName(for address: MunrChatServiceAddress, tx: DBReadTransaction) -> DisplayName {
    ...
    func shortDisplayName(address: MunrChatServiceAddress, transaction: DBReadTransaction) -> String {
        return _displayName(for: address, tx: transaction).resolvedValue(useShortNameIfAvailable: true)
    }

    func displayName(address: MunrChatServiceAddress, transaction: DBReadTransaction) -> String {
