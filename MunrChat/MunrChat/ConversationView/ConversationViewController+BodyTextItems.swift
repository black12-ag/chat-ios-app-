...
extension ConversationViewController {
    ...
    private func didLongPressLink(dataItem: TextCheckingDataItem) {
        ...
        } else if MunrChatProxy.isValidProxyLink(dataItem.url) {
            actionSheet.addAction(ActionSheetAction(
                title: OWSLocalizedString("MESSAGE_ACTION_LINK_OPEN_PROXY", comment: "Label for button to open a MunrChat proxy."),
                style: .default,
                handler: { [weak self] _ in
    ...
    private func didLongPressPhoneNumber(dataItem: TextCheckingDataItem) {
        ...
        let blockedAddress = MunrChatServiceAddress(phoneNumber: phoneNumber)
    ...
    private func didLongPressEmail(dataItem: TextCheckingDataItem) {
        ...
        // TODO: We could show (Send MunrChat Message/MunrChat call) actions for this email address.
        //       Ideally we could detect whether this email address corresponds to a system contact
        //       which is a registered MunrChat user.
    ...
    private func didTapOrLongPressMention(_ mentionAci: Aci) {
        ...
        showMemberActionSheet(forAddress: MunrChatServiceAddress(mentionAci), withHapticFeedback: true)
    }
