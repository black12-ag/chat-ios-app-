...
extension ConversationViewController: CVComponentDelegate {
    ...
    func handleUrl(_ url: URL) {
        ...
        if MunrChatProxy.isValidProxyLink(url) {
            didTapProxyLink(url: url)
            return
        }

        if MunrChatDotMePhoneNumberLink.isPossibleUrl(url) {
            cvc_didTapMunrChatMeLink(url: url)
            return
        }
    ...
    public func cvc_didTapMunrChatMeLink(url: URL) {
        MunrChatDotMePhoneNumberLink.openChat(url: url, fromViewController: self)
    }
    ...
    public func didTapUsernameLink(usernameLink: Usernames.UsernameLink) {
        SSKEnvironment.shared.databaseStorageRef.read { tx in
            UsernameQuerier().queryForUsernameLink(
                ...
                tx: tx,
                onSuccess: { _, aci in
                    MunrChatApp.shared.presentConversationForAddress(
    ...
    public func didTapPreviouslyVerifiedIdentityChange(_ address: MunrChatServiceAddress) {
        AssertIsOnMainThread()

        showFingerprint(address: address)
    }

    public func showFingerprint(address: MunrChatServiceAddress) {
        AssertIsOnMainThread()
    ...
    public func didTapUnverifiedIdentityChange(_ address: MunrChatServiceAddress) {
        AssertIsOnMainThread()
    ...
    public func didTapSessionRefreshMessage(_ message: TSErrorMessage) {
        ...
        OWSActionSheets.showContactSupportActionSheet(
            ...
            emailFilter: .custom("MunrChat iOS Session Refresh"),
            fromViewController: self
        )
    }
    ...
    public func didTapShowFingerprint(_ address: MunrChatServiceAddress) {
        AssertIsOnMainThread()
        ...
        alert.addAction(
            ActionSheetAction(
                title: OWSLocalizedString(
                    ...
                    comment: "Action to block contact in MunrChat for sheet shown when the user taps a missed call from a contact blocked in iOS settings."
                ),
                style: .destructive
            ) { [weak self] _ in
    ...
    public func didTapUpdateSystemContact(_ address: MunrChatServiceAddress, newNameComponents: PersonNameComponents) {
    ...
    public func didTapPhoneNumberChange(aci: Aci, phoneNumberOld: String, phoneNumberNew: String) {
        SUIEnvironment.shared.contactsViewHelperRef.checkEditAuthorization(
            performWhenAllowed: {
                ...
                let address = MunrChatServiceAddress(serviceId: aci, phoneNumber: phoneNumberNew)
                SUIEnvironment.shared.contactsViewHelperRef.presentSystemContactsFlow(
    ...
    public func didTapActivatePayments() {
        AssertIsOnMainThread()
        MunrChatApp.shared.showAppSettings(mode: .payments)
    }
