...
class BlockingAnnouncementOnlyView: UIStackView {
    ...
    private func fetchGroupAdminAddresses(tx: DBReadTransaction) -> [MunrChatServiceAddress] {
        guard
        ...
        return contactManager.sortMunrChatServiceAddresses(groupAdminAddresses, transaction: tx)
...
class MessageUserSubsetSheet: OWSTableSheetViewController {
    private let addresses: [MunrChatServiceAddress]
    private let forceDarkMode: Bool

    init(addresses: [MunrChatServiceAddress], forceDarkMode: Bool) {
