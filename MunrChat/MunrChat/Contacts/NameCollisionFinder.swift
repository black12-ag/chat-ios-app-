...
/// thread recipient with all known MunrChat accounts.
public class ContactThreadNameCollisionFinder: NameCollisionFinder {
    ...
    public func findCollisions(transaction: DBReadTransaction) -> [NameCollision] {
        ...
        let candidateAddresses = { () -> [MunrChatServiceAddress] in
            var result = Set<MunrChatServiceAddress>()
            result.formUnion(SSKEnvironment.shared.profileManagerRef.allWhitelistedRegisteredAddresses(tx: transaction))
            // Include all MunrChatAccounts as well (even though most are redundant) to
            // ensure we check against blocked system contact names.
            result.formUnion(MunrChatAccount.anyFetchAll(transaction: transaction).map { $0.recipientAddress })
...
public class GroupMembershipNameCollisionFinder: NameCollisionFinder {
    ...
    let lock = UnfairLock()
    private var recentProfileUpdateMessages: [MunrChatServiceAddress: [TSInfoMessage]]?
    ...
    private func fetchRecentProfileUpdates(transaction: DBReadTransaction) -> [MunrChatServiceAddress: [TSInfoMessage]] {
        lock.withLock {
            ...
            // Build a map from (MunrChatServiceAddress) -> (List of recent profile update messages)
            let results: [MunrChatServiceAddress: [TSInfoMessage]] = finder
    ...
    func standardSort(readTx: DBReadTransaction) -> NameCollision {
        NameCollision(
            elements.sorted { lhs, rhs in
                ...
                // - MunrChatServiceAddress UUID otherwise, to ensure stable sorting
