...
final class CallLinkViewController: OWSTableViewController2 {
    ...
    private func buildTableContents() -> OWSTableContents {
        ...
        let sharingSection = OWSTableSection(items: [
            .item(
                icon: .buttonForward,
                name: CallStrings.shareLinkViaMunrChat,
                actionBlock: { [unowned self] in self.shareCallLinkViaMunrChat() }
            ),
            .item(
                icon: .buttonCopy,
...
private extension Theme {
    class var joinButtonTextColor: UIColor {
        ...
        let darkThemeColor = UIColor.MunrChat.ultramarine.resolvedColor(with: darkThemeTraits)
        let lightThemeColor = UIColor.MunrChat.ultramarine.resolvedColor(with: UITraitCollection(userInterfaceStyle: .light))
