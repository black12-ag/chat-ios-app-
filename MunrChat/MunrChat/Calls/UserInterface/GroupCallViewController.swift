...
final class GroupCallViewController: UIViewController {
    ...
    private let call: MunrChatCall
    private let groupCall: GroupCall
    private let ringRtcCall: MunrChatRingRTC.GroupCall
    ...
    private var membersAtJoin: Set<MunrChatServiceAddress>?
    ...
    static func load(call: MunrChatCall, groupCall: GroupCall, tx: DBReadTransaction) -> GroupCallViewController {
    ...
    private static func _prepareLobby(
        ...
        buildAndStartConnecting: () async throws -> (MunrChatCall, GroupCall)?
    ) async rethrows -> (() -> Void)? {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if let splitViewSnapshot = MunrChatApp.shared.snapshotSplitViewController(afterScreenUpdates: false) {
    ...
    private func didHangupCall() {
        ...
        guard
            let splitViewSnapshot = MunrChatApp.shared.snapshotSplitViewController(afterScreenUpdates: false),
    ...
    private func otherUsersProfileChanged(notification: Notification) {
        ...
        guard let changedAddress = notification.userInfo?[UserProfileNotifications.profileAddressKey] as? MunrChatServiceAddress,
        ...
        if let peekInfo = self.ringRtcCall.peekInfo {
            ...
            if joinedAndPendingMembers.contains(where: { uuid in
                changedAddress == MunrChatServiceAddress(Aci(fromUUID: uuid))
            }) {
...
extension GroupCallViewController: CallViewControllerWindowReference {
    ...
    var remoteVideoAddress: MunrChatServiceAddress {
    ...
    public func returnFromPip(pipWindow: UIWindow) {
        ...
        guard let splitViewSnapshot = MunrChatApp.shared.snapshotSplitViewController(afterScreenUpdates: false) else {
    ...
    private func safetyNumberMismatchAddresses(untrustedThreshold: Date?) -> [MunrChatServiceAddress] {
        SSKEnvironment.shared.databaseStorageRef.read { transaction in
            let addressesToCheck: [MunrChatServiceAddress]
            if
                case .groupThread(let groupThreadCall) = groupCall.concreteType,
