...
final class CallKitCallUIAdaptee: NSObject, CallUIAdaptee, @preconcurrency CXProviderDelegate {
    ...
    class func buildProviderConfiguration(useSystemCallLog: Bool) -> CXProviderConfiguration {
        ...
        let iconMaskImage = #imageLiteral(resourceName: "MunrChat-logo-128")
    ...
    private func localizedCallerNameWithSneakyTransaction(for call: MunrChatCall) -> String {
        switch call.mode {
    ...
    @MainActor
    private func endCallOnceReported(_ call: MunrChatCall, reason: CXCallEndedReason) {
        Self.providerReadyFlag.runNowOrWhenDidBecomeReadySync {
    ...
    public func provider(_ provider: CXProvider, perform action: CXEndCallAction) {
        ...
        // MunrChat to the system that the action has been successfully performed.
        action.fulfill()
