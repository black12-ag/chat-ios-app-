...
class CallHeader: UIView {
    ...
    private func incomingRingText(caller: MunrChatServiceAddress) -> String {
    ...
    private func whoIsHereText(joinedMembers: [UUID?]) -> String {
        ...
        let upToTwoKnownMemberNames: [String] = SSKEnvironment.shared.databaseStorageRef.read { tx -> [String] in
            joinedMembers
                .lazy
                .compactMap { $0 }
                .map { SSKEnvironment.shared.contactManagerRef.displayName(for: MunrChatServiceAddress(Aci(fromUUID: $0)), tx: tx) }
    ...
    private func callTitleLabelText() -> String {
            ...
            firstMember.presenting == true
        {
            let presentingName = SSKEnvironment.shared.databaseStorageRef.read { tx in
                SSKEnvironment.shared.contactManagerRef.displayName(for: MunrChatServiceAddress(Aci(fromUUID: firstMember.userId)), tx: tx).resolvedValue(useShortNameIfAvailable: true)
            }
            let formatString = OWSLocalizedString(
