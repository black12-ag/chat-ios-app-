...
public class CallUIAdapter: NSObject {
    ...
    @MainActor
    internal func reportIncomingCall(_ call: MunrChatCall) {
    ...
    @MainActor
    internal func reportMissedCall(_ call: MunrChatCall, individualCall: IndividualCall) {
    ...
    @MainActor
    internal func startOutgoingCall(call: MunrChatCall) {
        // make sure we don't terminate audio session during call
        _ = SUIEnvironment.shared.audioSessionRef.startAudioActivity(call.commonState.audioActivity)
    ...
    @MainActor
    internal func answerCall(_ call: MunrChatCall) {
    ...
    @MainActor
    internal func remoteDidHangupCall(_ call: MunrChatCall) {
        adaptee.remoteDidHangupCall(call)
    }

    @MainActor
    internal func remoteBusy(_ call: MunrChatCall) {
        adaptee.remoteBusy(call)
    }

    @MainActor
    internal func didAnswerElsewhere(call: MunrChatCall) {
        adaptee.didAnswerElsewhere(call: call)
    }

    @MainActor
    internal func didDeclineElsewhere(call: MunrChatCall) {
        adaptee.didDeclineElsewhere(call: call)
    }

    @MainActor
    internal func wasBusyElsewhere(call: MunrChatCall) {
        adaptee.wasBusyElsewhere(call: call)
    }

    @MainActor
    internal func localHangupCall(_ call: MunrChatCall) {
        adaptee.localHangupCall(call)
    }

    @MainActor
    internal func failCall(_ call: MunrChatCall, error: CallError) {
        adaptee.failCall(call, error: error)
    }

    @MainActor
    private func showCall(_ call: MunrChatCall) {
    ...
    @MainActor
    internal func setIsMuted(call: MunrChatCall, isMuted: Bool) {
    ...
    @MainActor
    internal func setHasLocalVideo(call: MunrChatCall, hasLocalVideo: Bool) {
        adaptee.setHasLocalVideo(call: call, hasLocalVideo: hasLocalVideo)
    }

    @MainActor
    internal func setCameraSource(call: MunrChatCall, isUsingFrontCamera: Bool) {
