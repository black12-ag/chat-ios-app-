...
class CallsListViewController: OWSViewController, HomeTabViewController, CallServiceStateObserver, GroupCallObserver {
    ...
    func didUpdateCall(from oldValue: MunrChatCall?, to newValue: MunrChatCall?) {
        // When a MunrChatCall ends, reload it if we know its callId.
        ...
        // When a MunrChatCall starts, reload it as soon as we learn its callId.
        if let newValue {
    ...
    private let clearMissedCallQueue = DispatchQueue(label: "org.MunrChat.calls-list-clear-missed")
