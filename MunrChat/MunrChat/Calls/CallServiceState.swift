...
class CallServiceState {
    ...
    init(currentCall: AtomicValue<MunrChatCall?>) {
    ...
    private let _currentCall: AtomicValue<MunrChatCall?>

    /// Represents the call currently occuring on this device.
    var currentCall: MunrChatCall? { _currentCall.get() }

    @MainActor
    func setCurrentCall(_ currentCall: MunrChatCall?) {
     ...
     */
    @MainActor
    func terminateCall(_ call: MunrChatCall) {
