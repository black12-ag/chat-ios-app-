...
final class IndividualCallService: CallServiceStateObserver {
    ...
    @MainActor
    func didUpdateCall(from oldValue: MunrChatCall?, to newValue: MunrChatCall?) {
        stopAnyCallTimer()
        if let newValue {
     ...
     */
    @MainActor
    func handleOutgoingCall(_ call: MunrChatCall) {
     ...
     */
    @MainActor
    public func handleAcceptCall(_ call: MunrChatCall) {
        Logger.info("\(call)")
     ...
     */
    @MainActor
    func handleLocalHangupCall(_ call: MunrChatCall) {
        Logger.info("\(call)")
    ...
    public func handleReceivedOffer(
        ...
        let newCall = MunrChatCall(individualCall: individualCall)
    ...
    @MainActor
    public func callManager(_ callManager: CallService.CallManagerType, shouldStartCall call: MunrChatCall, callId: UInt64, isOutgoing: Bool, callMediaType: CallMediaType, shouldEarlyRing: Bool) {
        Logger.info("call: \(call)")
        ...
        Task { @MainActor in
            do {
                ...
                let isMunrChatConnection = self.databaseStorage.read { tx in
                    return profileManager.isThread(inProfileWhitelist: call.individualCall.thread, transaction: tx)
                }
                if !isMunrChatConnection {
                    Logger.warn("Using relay server because remote user is not a MunrChat Connection")
                }

                let useTurnOnly = !isMunrChatConnection || self.preferences.doCallsHideIPAddress
    ...
    @MainActor
    public func callManager(_ callManager: CallService.CallManagerType, onEvent call: MunrChatCall, event: CallManagerEvent) {
        ...
        case .endedMunrChatingFailure, .endedGlareHandlingFailure:
            handleFailedCall(failedCall: call, error: CallError.MunrChating, shouldResetUI: true, shouldResetRingRTC: false)
    ...
    @MainActor
    public func callManager(_ callManager: CallService.CallManagerType, onUpdateLocalVideoSession call: MunrChatCall, session: AVCaptureSession?) {
    ...
    @MainActor
    public func callManager(_ callManager: CallService.CallManagerType, onAddRemoteVideoTrack call: MunrChatCall, track: RTCVideoTrack) {
    ...
    // MARK: - Call Manager MunrChating

    @MainActor
    public func callManager(_ callManager: CallService.CallManagerType, shouldSendOffer callId: UInt64, call: MunrChatCall, destinationDeviceId: UInt32?, opaque: Data, callMediaType: CallMediaType) {
        ...
        Task {
            do {
                ...
                try self.callManager.MunrChatingMessageDidSend(callId: callId)
            } catch {
                Logger.error("failed to send offer message to \(call.individualCall.thread.contactAddress) with error: \(error)")
                self.callManager.MunrChatingMessageDidFail(callId: callId)
            }
        }
    }

    @MainActor
    public func callManager(_ callManager: CallService.CallManagerType, shouldSendAnswer callId: UInt64, call: MunrChatCall, destinationDeviceId: UInt32?, opaque: Data) {
    ...
    @MainActor
    public func callManager(_ callManager: CallService.CallManagerType, shouldSendIceCandidates callId: UInt64, call: MunrChatCall, destinationDeviceId: UInt32?, candidates: [Data]) {
        ...
        Task {
            do {
                ...
                callManager.MunrChatingMessageDidFail(callId: callId)
            }
        }
    }

    @MainActor
    public func callManager(_ callManager: CallService.CallManagerType, shouldSendHangup callId: UInt64, call: MunrChatCall, destinationDeviceId: UInt32?, hangupType: HangupType, deviceId: UInt32) {
    ...
    @MainActor
    public func callManager(_ callManager: CallService.CallManagerType, shouldSendBusy callId: UInt64, call: MunrChatCall, destinationDeviceId: UInt32?) {
     ...
     */
    @MainActor
    public func handleMissedCall(_ call: MunrChatCall, error: CallError? = nil) {
        Logger.info("call: \(call)")
    ...
    @MainActor
    func handleAnsweredElsewhere(call: MunrChatCall) {
    ...
    @MainActor
    func handleDeclinedElsewhere(call: MunrChatCall) {
    ...
    @MainActor
    func handleBusyElsewhere(call: MunrChatCall) {
     ...
     */
    @MainActor
    private func handleRinging(call: MunrChatCall, isAnticipatory: Bool = false) {
    ...
    @MainActor
    private func handleReconnecting(call: MunrChatCall) {
        Logger.info("call: \(call)")
    ...
    @MainActor
    private func handleReconnected(call: MunrChatCall) {
        Logger.info("call: \(call)")
     ...
     */
    @MainActor
    private func handleConnected(call: MunrChatCall) {
     ...
     */
    @MainActor
    func setIsOnHold(call: MunrChatCall, isOnHold: Bool) {
        Logger.info("call: \(call)")
    ...
    @MainActor
    func cleanUpStaleCall(_ staleCall: MunrChatCall, function: StaticString = #function, line: UInt = #line) {
    ...
    @MainActor
    public func handleFailedCall(failedCall: MunrChatCall, error: Error, shouldResetUI: Bool, shouldResetRingRTC: Bool) {
        Logger.debug("")
    ...
    @MainActor
    func ensureAudioState(call: MunrChatCall) {
    ...
    @MainActor
    func startCallTimer(for call: MunrChatCall) {
    ...
    @MainActor
    private func ensureCallScreenPresented(call: MunrChatCall, hasUsedUpTimerSlop: inout Bool) {
...
private extension MunrChatCall {
    ...
    /// - Important
    /// Callers must be *sure* that this `MunrChatCall` represents an individual
