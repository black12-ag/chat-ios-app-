...
class CallLinkManagerImpl: CallLinkManager {
    ...
    private let sfuClient: MunrChatRingRTC.SFUClient
    ...
    struct CreateResult {
        ...
        var callLinkState: MunrChatServiceKit.CallLinkState
    }
    ...
    func createCallLink(rootKey: CallLinkRootKey) async throws -> CreateResult {
        ...
        let callLinkState = MunrChatServiceKit.CallLinkState(try await self.sfuClient.createCallLink(
            sfuUrl: sfuUrl,
            ...
            restrictions: MunrChatServiceKit.CallLinkState.Constants.defaultRequiresAdminApproval ? .adminApproval : .none
        ).unwrap())
    ...
    func updateCallLinkName(
        ...
        let secretParams = CallLinkSecretParams.deriveFromRootKey(rootKey.bytes)
        let authCredentialPresentation = authCredential.present(callLinkParams: secretParams)
        return MunrChatServiceKit.CallLinkState(try await self.sfuClient.updateCallLinkName(
            sfuUrl: sfuUrl,
            authCredentialPresentation: [UInt8](authCredentialPresentation.serialize()),
            linkRootKey: rootKey,
            adminPasskey: adminPasskey,
    ...
    func updateCallLinkRestrictions(
        ...
        let secretParams = CallLinkSecretParams.deriveFromRootKey(rootKey.bytes)
        let authCredentialPresentation = authCredential.present(callLinkParams: secretParams)
        return MunrChatServiceKit.CallLinkState(try await self.sfuClient.updateCallLinkRestrictions(
            sfuUrl: sfuUrl,
            authCredentialPresentation: [UInt8](authCredentialPresentation.serialize()),
            linkRootKey: rootKey,
            adminPasskey: adminPasskey,
