...
final class CallService: CallServiceStateObserver, CallServiceStateDelegate {
    public typealias CallManagerType = CallManager<MunrChatCall, CallService>
    ...
    public init(
        ...
        mutableCurrentCall: AtomicValue<MunrChatCall?>,
        ...
        self.callManager = CallManager<MunrChatCall, CallService>(
    ...
    func callServiceState(_ callServiceState: CallServiceState, didTerminateCall call: MunrChatCall) {
    ...
    private func updateIsLocalAudioMutedWithMicrophonePermission(call: MunrChatCall, isLocalAudioMuted: Bool) {
    ...
    private func updateIsLocalVideoMutedWithCameraPermissions(call: MunrChatCall, isLocalVideoMuted: Bool) {
        owsPrecondition(call === callServiceState.currentCall)
    ...
    func updateCameraSource(call: MunrChatCall, isUsingFrontCamera: Bool) {
    ...
    public func handleFailedCall(failedCall: MunrChatCall, error: Error) {
    ...
    func buildAndConnectGroupCall(for groupId: GroupIdentifier, isVideoMuted: Bool) -> (MunrChatCall, GroupThreadCall)? {
        return _buildAndConnectGroupCall(isOutgoingVideoMuted: isVideoMuted) { () -> (MunrChatCall, GroupThreadCall)? in
            ...
            return (MunrChatCall(groupThreadCall: groupThreadCall), groupThreadCall)
        }
    }
    ...
    enum CallLinkStateRetrievalStrategy {
        case reuse(MunrChatServiceKit.CallLinkState)
        case fetch
    }
    ...
    func buildAndConnectCallLinkCall(
    ...
    ) async throws -> (MunrChatCall, CallLinkCall)? {
        let state: MunrChatServiceKit.CallLinkState
        ...
        return _buildAndConnectGroupCall(isOutgoingVideoMuted: false) { () -> (MunrChatCall, CallLinkCall)? in
            let videoCaptureController = VideoCaptureController()
            let sfuUrl = DebugFlags.callingUseTestSFU.get() ? TSConstants.sfuTestURL : TSConstants.sfuURL
            ...
            return (MunrChatCall(callLinkCall: callLinkCall), callLinkCall)
        }
    }

    private func _buildAndConnectGroupCall<T: GroupCall>(
        isOutgoingVideoMuted: Bool,
        createCall: () -> (MunrChatCall, T)?
    ) -> (MunrChatCall, T)? {
    ...
    func joinGroupCallIfNecessary(_ call: MunrChatCall, groupCall: GroupCall) {
    ...
    private func leaveAndTerminateGroupCall(_ call: MunrChatCall, groupCall: GroupCall) {
        if groupCall.hasInvokedConnectMethod {
    ...
    func buildOutgoingIndividualCallIfPossible(thread: TSContactThread, localDeviceId: DeviceId, hasVideo: Bool) -> (MunrChatCall, IndividualCall)? {
        ...
        let call = MunrChatCall(individualCall: individualCall)
    ...
    private func shouldReorientUI(for call: MunrChatCall) -> Bool {
...
extension CallService: CallManagerDelegate {
    public typealias CallManagerDelegateCallType = MunrChatCall
    ...
    public func callManager(
        _ callManager: CallManager<MunrChatCall, CallService>,
    ...
    private func sendCallMessage(
        ...
        to recipientAci: Aci,
        callAtStart: MunrChatCall?
    ) async {
        do {
            let sendPromise = await databaseStorage.awaitableWrite { transaction in
                let thread = TSContactThread.getOrCreateThread(
                    withContactAddress: MunrChatServiceAddress(recipientAci),
    ...
    private func publishUntrustedIdentityErrorIfNeeded(_ error: any Error, callAtStart: MunrChatCall?) {
    ...
    public nonisolated func callManager(
        _ callManager: CallManager<MunrChatCall, CallService>,
        shouldCompareCalls call1: MunrChatCall,
        call2: MunrChatCall
    ) -> Bool {
        Logger.info("")
    ...
    public func callManager(
        _ callManager: CallManager<MunrChatCall, CallService>,
        shouldStartCall call: MunrChatCall,
        callId: UInt64,
        isOutgoing: Bool,
    ...
    public func callManager(
        _ callManager: CallManager<MunrChatCall, CallService>,
        onNetworkRouteChangedFor call: MunrChatCall,
    ...
    public nonisolated func callManager(
        _ callManager: CallManager<MunrChatCall, CallService>,
        onAudioLevelsFor call: MunrChatCall,
    ...
    public func callManager(
        _ callManager: CallManager<MunrChatCall, CallService>,
        onLowBandwidthForVideoFor call: MunrChatCall,
        recovered: Bool
    ) {
    ...
    public func callManager(
        _ callManager: CallManager<MunrChatCall, CallService>,
        onUpdateLocalVideoSession call: MunrChatCall,
    ...
    public func callManager(
        _ callManager: CallManager<MunrChatCall, CallService>,
        onAddRemoteVideoTrack call: MunrChatCall,
        track: RTCVideoTrack
    ) {
        individualCallService.callManager(
            callManager,
    ...
    public func callManager(
        ...
        case .ring(let groupId):
            ...
            groupThreadCall.groupCallRingState = .incomingRing(caller: MunrChatServiceAddress(senderAci), ringId: ringId)
