...
actor CallLinkStateUpdater {
    ...
    func updateExclusively(
        ...
        updateAndFetch: (CallLinkAuthCredential) async throws -> MunrChatServiceKit.CallLinkState
    ...
    private enum UpdateAction {
        case update(MunrChatServiceKit.CallLinkState)
        case notFound
        case delete
    }
    ...
    private func _updateExclusively(
        rootKey: CallLinkRootKey,
        updateAndFetch: (CallLinkAuthCredential) async throws -> MunrChatServiceKit.CallLinkState?
    ) async throws -> Result<MunrChatServiceKit.CallLinkState, CallLinkNotFoundError>? {
        ...
        let returnResult: Result<MunrChatServiceKit.CallLinkState, CallLinkNotFoundError>?
    ...
    func readCallLink(rootKey: CallLinkRootKey) async throws -> Result<MunrChatServiceKit.CallLinkState, CallLinkNotFoundError> {
