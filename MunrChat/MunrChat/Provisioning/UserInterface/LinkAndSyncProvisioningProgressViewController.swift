    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            Text("DEBUG: " + (viewModel.currentProgressStep?.rawValue ?? "none") + "\n\(viewModel.taskProgress)")
                .padding(.top)
                .foregroundStyle(Color.MunrChat.quaternaryLabel)
            ...
            Group {
                MunrChatSymbol.lock.text(dynamicTypeBaseSize: 20)
                    .padding(.bottom, 6)
                ...
                .appendLink(CommonStrings.learnMore) {
                    let vc = SFSafariViewController(url: URL(string: "https://support.MunrChat.org/hc/articles/360007320551")!)
