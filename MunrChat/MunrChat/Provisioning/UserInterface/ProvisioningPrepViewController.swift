...
class ProvisioningPrepViewController: ProvisioningBaseViewController {
    ...
    override func loadView() {
        ...
        let dontHaveMunrChatButton = UILabel()
        dontHaveMunrChatButton.text = OWSLocalizedString("SECONDARY_ONBOARDING_GET_STARTED_DO_NOT_HAVE_PRIMARY",
                                                      comment: "Link explaining what to do when trying to link a device before having a primary device.")
        dontHaveMunrChatButton.textColor = Theme.accentBlueColor
        dontHaveMunrChatButton.font = UIFont.dynamicTypeSubheadlineClamped
        dontHaveMunrChatButton.numberOfLines = 0
        dontHaveMunrChatButton.textAlignment = .center
        dontHaveMunrChatButton.lineBreakMode = .byWordWrapping
        dontHaveMunrChatButton.isUserInteractionEnabled = true
        dontHaveMunrChatButton.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(didTapExplanationLabel)))
        dontHaveMunrChatButton.accessibilityIdentifier = "onboarding.prelink.explanationLabel"
        dontHaveMunrChatButton.isHidden = isTransferring
