    ...
    public func showAppUpgradeNagIfNecessary() {
        ...
        Task {
            do {
                ...
                // Only failDebug if we're looking up the true org.munrchatsystems.MunrChat app store record
                // If someone is building MunrChat with their own bundleID, it's less important that this succeeds.
                if error.isNetworkFailureOrTimeout || !bundleIdentifier.hasPrefix("org.munrchatsystems.") {
                    Logger.warn("failed with error: \(error)")
                } else {
                    owsFailDebug("Failed to find MunrChat app store record")
                }
            }
        }
    }
