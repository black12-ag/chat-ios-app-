...
final class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    private let flushQueue = DispatchQueue(label: "org.MunrChat.flush", qos: .utility)
    ...
    func application(
        ...
        DebugLogger.registerLibMunrChat()
    ...
    private func setUpMainAppEnvironment(
        ...
        let _currentCall = AtomicValue<MunrChatCall?>(nil, lock: .init())
    ...
    private func setAppIsReady(
        ...
        MunrChatApp.shared.performInitialSetup(appReadiness: appReadiness)
        ...
        MunrChatApp.shared.showLaunchInterface(
            launchInterface,
            appReadiness: appReadiness,
    ...
    private func presentLaunchFailureActionSheet(
        ...
        if DebugFlags.internalSettings {
            actionSheet.addAction(.init(title: "Export Database (internal)") { [unowned viewController] _ in
                MunrChatApp.showExportDatabaseUI(from: viewController) {
        ...
        for action in actions {
            ...
            case .submitDebugLogsWithDatabaseIntegrityCheckAndCrash(let databaseStorage):
                addSubmitDebugLogsAction { [unowned viewController] in
                    MunrChatApp.showDatabaseIntegrityCheckUI(from: viewController, databaseStorage: databaseStorage) {
                        DebugLogs.submitLogs(supportTag: supportTag, dumper: logDumper) {
                            owsFail("Exiting after submitting debug logs")
                        }
                    }
    ...
    private func refreshConnection(isAppActive: Bool) {
        ...
        if isAppActive {
            ...
            let isPastRegistration = MunrChatApp.shared.conversationSplitViewController != nil
    ...
    func application(
        ...
        case "INSendMessageIntent":
            ...
            appReadiness.runNowOrWhenAppDidBecomeReadySync {
                ...
                MunrChatApp.shared.presentConversationAndScrollToFirstUnreadMessage(
    ...
    func application(
        ...
        appReadiness.runNowOrWhenUIDidBecomeReadySync {
            ...
            MunrChatApp.shared.showNewConversationView()
    ...
    static func applicationShortcutItems(isRegistered: Bool) -> [UIApplicationShortcutItem] {
        ...
        return [.init(
            ...
            localizedTitle: OWSLocalizedString(
                ...
                comment: "On the iOS home screen, if you tap and hold the MunrChat icon, this shortcut will appear. Tapping it will let users send a new message. You may want to refer to similar behavior in other iOS apps, such as Messages, for equivalent strings."
            ),
