...
final class CallService: CallServiceStateObserver, CallServiceStateDelegate {
    public typealias CallManagerType = CallManager<MunirCall, CallService>
    ...
    public init(
        ...
        mutableCurrentCall: AtomicValue<MunirCall?>,
        ...
        self.callManager = CallManager<MunirCall, CallService>(
    ...
    private func updateIsLocalAudioMutedWithMicrophonePermission(call: MunirCall, isLocalAudioMuted: Bool) {
    ...
    private func updateIsLocalVideoMutedWithCameraPermissions(call: MunirCall, isLocalVideoMuted: Bool) {
        owsPrecondition(call === callServiceState.currentCall)
    ...
    func updateCameraSource(call: MunirCall, isUsingFrontCamera: Bool) {
    ...
    public func handleFailedCall(failedCall: MunirCall, error: Error) {
    ...
    func buildAndConnectGroupCall(for groupId: GroupIdentifier, isVideoMuted: Bool) -> (MunirCall, GroupThreadCall)? {
        return _buildAndConnectGroupCall(isOutgoingVideoMuted: isVideoMuted) { () -> (MunirCall, GroupThreadCall)? in
            ...
            return (MunirCall(groupThreadCall: groupThreadCall), groupThreadCall)
        }
    }
    ...
    enum CallLinkStateRetrievalStrategy {
        case reuse(MunirServiceKit.CallLinkState)
        case fetch
    }
    ...
    func buildAndConnectCallLinkCall(
    ...
    ) async throws -> (MunirCall, CallLinkCall)? {
        let state: MunirServiceKit.CallLinkState
        ...
        return _buildAndConnectGroupCall(isOutgoingVideoMuted: false) { () -> (MunirCall, CallLinkCall)? in
            let videoCaptureController = VideoCaptureController()
            let sfuUrl = DebugFlags.callingUseTestSFU.get() ? TSConstants.sfuTestURL : TSConstants.sfuURL
            ...
            return (MunirCall(callLinkCall: callLinkCall), callLinkCall)
        }
    }

    private func _buildAndConnectGroupCall<T: GroupCall>(
        isOutgoingVideoMuted: Bool,
        createCall: () -> (MunirCall, T)?
    ) -> (MunirCall, T)? {
    ...
    func joinGroupCallIfNecessary(_ call: MunirCall, groupCall: GroupCall) {
    ...
    private func leaveAndTerminateGroupCall(_ call: MunirCall, groupCall: GroupCall) {
        if groupCall.hasInvokedConnectMethod {
    ...
    func buildOutgoingIndividualCallIfPossible(thread: TSContactThread, localDeviceId: DeviceId, hasVideo: Bool) -> (MunirCall, IndividualCall)? {
        ...
        let call = MunirCall(individualCall: individualCall)
    ...
    private func shouldReorientUI(for call: MunirCall) -> Bool {
...
extension CallService: CallManagerDelegate {
    public typealias CallManagerDelegateCallType = MunirCall
    ...
    public func callManager(
        _ callManager: CallManager<MunirCall, CallService>,
    ...
    private func publishUntrustedIdentityErrorIfNeeded(_ error: any Error, callAtStart: MunirCall?) {
    ...
    public nonisolated func callManager(
        _ callManager: CallManager<MunirCall, CallService>,
        shouldCompareCalls call1: MunirCall,
        call2: MunirCall
    ) -> Bool {
        Logger.info("")
    ...
    public func callManager(
        _ callManager: CallManager<MunirCall, CallService>,
        shouldStartCall call: MunirCall,
        callId: UInt64,
        isOutgoing: Bool,
    ...
    public func callManager(
        _ callManager: CallManager<MunirCall, CallService>,
        onNetworkRouteChangedFor call: MunirCall,
    ...
    public nonisolated func callManager(
        _ callManager: CallManager<MunirCall, CallService>,
        onAudioLevelsFor call: MunirCall,
    ...
    public func callManager(
        _ callManager: CallManager<MunirCall, CallService>,
        onLowBandwidthForVideoFor call: MunirCall,
        recovered: Bool
    ) {
    ...
    public func callManager(
        _ callManager: CallManager<MunirCall, CallService>,
        onUpdateLocalVideoSession call: MunirCall,
    ...
    public func callManager(
        _ callManager: CallManager<MunirCall, CallService>,
        onAddRemoteVideoTrack call: MunirCall,
        track: RTCVideoTrack
    ) {
        individualCallService.callManager(
            callManager,
