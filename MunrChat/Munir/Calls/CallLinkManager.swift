...
protocol CallLinkManager {
    ...
    func peekCallLink(
        ...
        authCredential: MunirServiceKit.CallLinkAuthCredential
    ) async throws -> String?
    ...
    func updateCallLinkName(
        ...
        adminPasskey: Data,
        authCredential: MunirServiceKit.CallLinkAuthCredential
    ) async throws -> MunirServiceKit.CallLinkState
...
class CallLinkManagerImpl: CallLinkManager {
    ...
    struct CreateResult {
        ...
        var callLinkState: MunirServiceKit.CallLinkState
    }
    ...
    func createCallLink(rootKey: CallLinkRootKey) async throws -> CreateResult {
        ...
        let callLinkState = MunirServiceKit.CallLinkState(try await self.sfuClient.createCallLink(
            sfuUrl: sfuUrl,
            ...
            restrictions: MunirServiceKit.CallLinkState.Constants.defaultRequiresAdminApproval ? .adminApproval : .none
        ).unwrap())
    ...
    func updateCallLinkName(
        ...
        let secretParams = CallLinkSecretParams.deriveFromRootKey(rootKey.bytes)
        let authCredentialPresentation = authCredential.present(callLinkParams: secretParams)
        return MunirServiceKit.CallLinkState(try await self.sfuClient.updateCallLinkName(
            sfuUrl: sfuUrl,
            authCredentialPresentation: [UInt8](authCredentialPresentation.serialize()),
            linkRootKey: rootKey,
            adminPasskey: adminPasskey,
    ...
    func updateCallLinkRestrictions(
        ...
        let secretParams = CallLinkSecretParams.deriveFromRootKey(rootKey.bytes)
        let authCredentialPresentation = authCredential.present(callLinkParams: secretParams)
        return MunirServiceKit.CallLinkState(try await self.sfuClient.updateCallLinkRestrictions(
            sfuUrl: sfuUrl,
            authCredentialPresentation: [UInt8](authCredentialPresentation.serialize()),
            linkRootKey: rootKey,
            adminPasskey: adminPasskey,
