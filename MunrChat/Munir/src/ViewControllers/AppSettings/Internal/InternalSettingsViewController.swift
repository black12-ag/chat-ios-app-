    ...
    func updateTableContents() {
        ...
        debugSection.add(.actionItem(
            ...
            actionBlock: { [weak self] in
                ...
                MunirApp.showExportDatabaseUI(from: self)
            }
        ))
        debugSection.add(.actionItem(
        ...
        debugSection.add(.actionItem(
            ...
            actionBlock: { [weak self] in
                ...
                MunirApp.showDatabaseIntegrityCheckUI(from: self, databaseStorage: SSKEnvironment.shared.databaseStorageRef)
            }
        ))
        debugSection.add(.actionItem(
        ...
        if let buildDetails = Bundle.main.object(forInfoDictionaryKey: "BuildDetails") as? [String: AnyObject] {
            if let signalCommit = (buildDetails["Munir'sCommit"] as? String)?.strippedOrNil?.prefix(12) {
