...
public class NotificationActionHandler {
    ...
    private class func showMyStories(appReadiness: AppReadiness) async {
        ...
        MunirApp.shared.showMyStories(animated: UIApplication.shared.applicationState == .active)
    }
    ...
    private class func showLinkedDevices() {
        MunirApp.shared.showAppSettings(mode: .linkedDevices)
    }

    @MainActor
    private class func showBackupsEnabled() {
        MunirApp.shared.showAppSettings(mode: .backups)
    }
