...
final class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    private func setUpMainAppEnvironment(
        ...
        let _currentCall = AtomicValue<MunirCall?>(nil, lock: .init())
    ...
    private func setAppIsReady(
        ...
        MunirApp.shared.performInitialSetup(appReadiness: appReadiness)
        ...
        MunirApp.shared.showLaunchInterface(
            launchInterface,
            appReadiness: appReadiness,
    ...
    private func presentLaunchFailureActionSheet(
        ...
        if DebugFlags.internalSettings {
            actionSheet.addAction(.init(title: "Export Database (internal)") { [unowned viewController] _ in
                MunirApp.showExportDatabaseUI(from: viewController) {
        ...
        for action in actions {
            ...
            case .submitDebugLogsWithDatabaseIntegrityCheckAndCrash(let databaseStorage):
                addSubmitDebugLogsAction { [unowned viewController] in
                    MunirApp.showDatabaseIntegrityCheckUI(from: viewController, databaseStorage: databaseStorage) {
                        DebugLogs.submitLogs(supportTag: supportTag, dumper: logDumper) {
                            owsFail("Exiting after submitting debug logs")
                        }
                    }
    ...
    private func refreshConnection(isAppActive: Bool) {
        ...
        if isAppActive {
            ...
            let isPastRegistration = MunirApp.shared.conversationSplitViewController != nil
    ...
    func application(
        ...
        case "INSendMessageIntent":
            ...
            appReadiness.runNowOrWhenAppDidBecomeReadySync {
                ...
                MunirApp.shared.presentConversationAndScrollToFirstUnreadMessage(
    ...
    func application(
        ...
        appReadiness.runNowOrWhenUIDidBecomeReadySync {
            ...
            MunirApp.shared.showNewConversationView()
    ...
    static func applicationShortcutItems(isRegistered: Bool) -> [UIApplicationShortcutItem] {
        ...
        return [.init(
            ...
            localizedTitle: OWSLocalizedString(
                ...
                comment: "On the iOS home screen, if you tap and hold the Munirs icon, this shortcut will appear. Tapping it will let users send a new message. You may want to refer to similar behavior in other iOS apps, such as Messages, for equivalent strings."
            ),
